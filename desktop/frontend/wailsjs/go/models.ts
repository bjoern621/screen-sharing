export namespace capabilities {
	
	export class Gap {
	    engine: string;
	    option: string;
	    value: string;
	    reason: string;
	
	    static createFrom(source: any = {}) {
	        return new Gap(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.engine = source["engine"];
	        this.option = source["option"];
	        this.value = source["value"];
	        this.reason = source["reason"];
	    }
	}
	export class AudioEncoder {
	    engine: string;
	    element: string;
	    parser: string;
	
	    static createFrom(source: any = {}) {
	        return new AudioEncoder(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.engine = source["engine"];
	        this.element = source["element"];
	        this.parser = source["parser"];
	    }
	}
	export class AudioCodec {
	    name: string;
	    format: string;
	    rate: number;
	    bitrateK: number;
	    encoders: AudioEncoder[];
	    gaps: Gap[];
	
	    static createFrom(source: any = {}) {
	        return new AudioCodec(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.name = source["name"];
	        this.format = source["format"];
	        this.rate = source["rate"];
	        this.bitrateK = source["bitrateK"];
	        this.encoders = this.convertValues(source["encoders"], AudioEncoder);
	        this.gaps = this.convertValues(source["gaps"], Gap);
	    }
	
		convertValues(a: any, classs: any, asMap: boolean = false): any {
		    if (!a) {
		        return a;
		    }
		    if (a.slice && a.map) {
		        return (a as any[]).map(elem => this.convertValues(elem, classs));
		    } else if ("object" === typeof a) {
		        if (asMap) {
		            for (const key of Object.keys(a)) {
		                a[key] = new classs(a[key]);
		            }
		            return a;
		        }
		        return new classs(a);
		    }
		    return a;
		}
	}
	
	export class Codec {
	    name: string;
	    family: string;
	    format: string;
	    implemented: boolean;
	    chromas: string[];
	    cqMax: Record<string, number>;
	    bitrateLimitM: Record<string, number>;
	    gaps: Gap[];
	
	    static createFrom(source: any = {}) {
	        return new Codec(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.name = source["name"];
	        this.family = source["family"];
	        this.format = source["format"];
	        this.implemented = source["implemented"];
	        this.chromas = source["chromas"];
	        this.cqMax = source["cqMax"];
	        this.bitrateLimitM = source["bitrateLimitM"];
	        this.gaps = this.convertValues(source["gaps"], Gap);
	    }
	
		convertValues(a: any, classs: any, asMap: boolean = false): any {
		    if (!a) {
		        return a;
		    }
		    if (a.slice && a.map) {
		        return (a as any[]).map(elem => this.convertValues(elem, classs));
		    } else if ("object" === typeof a) {
		        if (asMap) {
		            for (const key of Object.keys(a)) {
		                a[key] = new classs(a[key]);
		            }
		            return a;
		        }
		        return new classs(a);
		    }
		    return a;
		}
	}
	export class Decoder {
	    element: string;
	    family: string;
	    format: string;
	    chromas: string[];
	    reason: string;
	
	    static createFrom(source: any = {}) {
	        return new Decoder(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.element = source["element"];
	        this.family = source["family"];
	        this.format = source["format"];
	        this.chromas = source["chromas"];
	        this.reason = source["reason"];
	    }
	}

}

export namespace display {
	
	export class Monitor {
	    index: number;
	    width: number;
	    height: number;
	    offsetX: number;
	    offsetY: number;
	    primary: boolean;
	    refreshHz: number;
	
	    static createFrom(source: any = {}) {
	        return new Monitor(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.index = source["index"];
	        this.width = source["width"];
	        this.height = source["height"];
	        this.offsetX = source["offsetX"];
	        this.offsetY = source["offsetY"];
	        this.primary = source["primary"];
	        this.refreshHz = source["refreshHz"];
	    }
	}

}

export namespace encoderate {
	
	export class Rate {
	    lowFps: number;
	    highFps: number;
	    lowBounded: boolean;
	    highBounded: boolean;
	
	    static createFrom(source: any = {}) {
	        return new Rate(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.lowFps = source["lowFps"];
	        this.highFps = source["highFps"];
	        this.lowBounded = source["lowBounded"];
	        this.highBounded = source["highBounded"];
	    }
	}

}

export namespace encoders {
	
	export class Availability {
	    usable: Record<string, any>;
	    unprobed: Record<string, string>;
	
	    static createFrom(source: any = {}) {
	        return new Availability(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.usable = source["usable"];
	        this.unprobed = source["unprobed"];
	    }
	}

}

export namespace gpupath {
	
	export class Path {
	    engine: string;
	    capture: string;
	    family: string;
	    import: string;
	
	    static createFrom(source: any = {}) {
	        return new Path(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.engine = source["engine"];
	        this.capture = source["capture"];
	        this.family = source["family"];
	        this.import = source["import"];
	    }
	}

}

export namespace main {
	
	export class PublishStateEvent {
	    publishing: boolean;
	    settings?: settings.Stream;
	    pending: boolean;
	    retrying: boolean;
	    attempt: number;
	    budget: number;

	    static createFrom(source: any = {}) {
	        return new PublishStateEvent(source);
	    }

	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.publishing = source["publishing"];
	        this.settings = this.convertValues(source["settings"], settings.Stream);
	        this.pending = source["pending"];
	        this.retrying = source["retrying"];
	        this.attempt = source["attempt"];
	        this.budget = source["budget"];
	    }
	
		convertValues(a: any, classs: any, asMap: boolean = false): any {
		    if (!a) {
		        return a;
		    }
		    if (a.slice && a.map) {
		        return (a as any[]).map(elem => this.convertValues(elem, classs));
		    } else if ("object" === typeof a) {
		        if (asMap) {
		            for (const key of Object.keys(a)) {
		                a[key] = new classs(a[key]);
		            }
		            return a;
		        }
		        return new classs(a);
		    }
		    return a;
		}
	}
	export class TransportCarriage {
	    name: string;
	    leg: string;
	    engine: string;
	    video: string[];
	    audio: string[];
	
	    static createFrom(source: any = {}) {
	        return new TransportCarriage(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.name = source["name"];
	        this.leg = source["leg"];
	        this.engine = source["engine"];
	        this.video = source["video"];
	        this.audio = source["audio"];
	    }
	}
	export class WatchKey {
	    name: string;
	    transport: string;
	
	    static createFrom(source: any = {}) {
	        return new WatchKey(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.name = source["name"];
	        this.transport = source["transport"];
	    }
	}

}

export namespace platform {
	
	export class Info {
	    os: string;
	    display: string;
	
	    static createFrom(source: any = {}) {
	        return new Info(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.os = source["os"];
	        this.display = source["display"];
	    }
	}

}

export namespace relay {
	
	export class Path {
	    name: string;
	    ready: boolean;
	    tracks: string;
	    format: string;
	    readers: number;
	    inMbps: number;
	
	    static createFrom(source: any = {}) {
	        return new Path(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.name = source["name"];
	        this.ready = source["ready"];
	        this.tracks = source["tracks"];
	        this.format = source["format"];
	        this.readers = source["readers"];
	        this.inMbps = source["inMbps"];
	    }
	}
	export class Status {
	    reachable: boolean;
	    error?: string;
	    paths: Path[];
	
	    static createFrom(source: any = {}) {
	        return new Status(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.reachable = source["reachable"];
	        this.error = source["error"];
	        this.paths = this.convertValues(source["paths"], Path);
	    }
	
		convertValues(a: any, classs: any, asMap: boolean = false): any {
		    if (!a) {
		        return a;
		    }
		    if (a.slice && a.map) {
		        return (a as any[]).map(elem => this.convertValues(elem, classs));
		    } else if ("object" === typeof a) {
		        if (asMap) {
		            for (const key of Object.keys(a)) {
		                a[key] = new classs(a[key]);
		            }
		            return a;
		        }
		        return new classs(a);
		    }
		    return a;
		}
	}

}

export namespace settings {
	
	export class Stream {
	    name: string;
	    relayHost: string;
	    relayPort: number;
	    apiPort: number;
	    rtspPort: number;
	    webrtcPort: number;
	    rtmpPort: number;
	    hlsPort: number;
	    transport: string;
	    codec: string;
	    mode: string;
	    chroma: string;
	    colorRange: string;
	    fps: number;
	    cq: number;
	    bitrateM: number;
	    maxrateM: number;
	    vbvMs: number;
	    gop: number;
	    bframes: number;
	    encPreset: string;
	    capture: string;
	    audio: string;
	    audioCodec: string;
	    drmMap: string;
	    monitor: number;
	    captureMemory: string;
	    srtPublishLatencyMs: number;
	    srtWatchLatencyMs: number;
	    rtspPublishProtocol: string;
	    rtspWatchProtocol: string;
	    rtspWatchLatencyMs: number;
	    uplinkMbps: number;
	    watchTransport: string;
	    gridTransport: string;
	
	    static createFrom(source: any = {}) {
	        return new Stream(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.name = source["name"];
	        this.relayHost = source["relayHost"];
	        this.relayPort = source["relayPort"];
	        this.apiPort = source["apiPort"];
	        this.rtspPort = source["rtspPort"];
	        this.webrtcPort = source["webrtcPort"];
	        this.rtmpPort = source["rtmpPort"];
	        this.hlsPort = source["hlsPort"];
	        this.transport = source["transport"];
	        this.codec = source["codec"];
	        this.mode = source["mode"];
	        this.chroma = source["chroma"];
	        this.colorRange = source["colorRange"];
	        this.fps = source["fps"];
	        this.cq = source["cq"];
	        this.bitrateM = source["bitrateM"];
	        this.maxrateM = source["maxrateM"];
	        this.vbvMs = source["vbvMs"];
	        this.gop = source["gop"];
	        this.bframes = source["bframes"];
	        this.encPreset = source["encPreset"];
	        this.capture = source["capture"];
	        this.audio = source["audio"];
	        this.audioCodec = source["audioCodec"];
	        this.drmMap = source["drmMap"];
	        this.monitor = source["monitor"];
	        this.captureMemory = source["captureMemory"];
	        this.srtPublishLatencyMs = source["srtPublishLatencyMs"];
	        this.srtWatchLatencyMs = source["srtWatchLatencyMs"];
	        this.rtspPublishProtocol = source["rtspPublishProtocol"];
	        this.rtspWatchProtocol = source["rtspWatchProtocol"];
	        this.rtspWatchLatencyMs = source["rtspWatchLatencyMs"];
	        this.uplinkMbps = source["uplinkMbps"];
	        this.watchTransport = source["watchTransport"];
	        this.gridTransport = source["gridTransport"];
	    }
	}
	export class Preset {
	    name: string;
	    settings: Stream;
	
	    static createFrom(source: any = {}) {
	        return new Preset(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.name = source["name"];
	        this.settings = this.convertValues(source["settings"], Stream);
	    }
	
		convertValues(a: any, classs: any, asMap: boolean = false): any {
		    if (!a) {
		        return a;
		    }
		    if (a.slice && a.map) {
		        return (a as any[]).map(elem => this.convertValues(elem, classs));
		    } else if ("object" === typeof a) {
		        if (asMap) {
		            for (const key of Object.keys(a)) {
		                a[key] = new classs(a[key]);
		            }
		            return a;
		        }
		        return new classs(a);
		    }
		    return a;
		}
	}

}

