...
/*
 * Vendored from MediaMTX: internal/servers/moq/reader.js.
 * MIT License, Copyright (c) 2019 aler9. https://github.com/bluenviron/mediamtx
 *
 * It is vendored rather than loaded from the relay because the app's origin
 * cannot fetch it: a self-signed relay certificate fails a cross-origin script
 * load the same way it fails the fingerprint fetch (see the Go moq package).
 *
 * Six local modifications, each marked LOCAL below. Re-apply them when
 * re-vendoring; everything else is upstream and should stay that way:
 *
 *   1. `export` on the class, so it can be imported rather than assigned to a
 *      global by a <script> tag.
 *   2. `conf.fingerprint` accepted as a hex string, skipping #fetchFingerprint.
 *      The app fetches it in Go, where the verification decision can be made.
 *   3. `close()`, a terminal teardown. Upstream only ever restarts, which a sink
 *      the grid has disconnected must not do.
 *   4. `stats()` plus the two counters behind it, for the tile's stats overlay.
 *   5. `conf.onFrame`, called on each drawn frame. The sink promotes to connected
 *      on the first one, since onSubscribed fires before there is a picture.
 *   6. A gain node, `setMuted` and `setVolume`. Upstream connects every buffer
 *      source straight to the destination and offers `unmute()` alone, which is
 *      not enough to back the tile's volume control.
 */
...
export class MediaMTXMoQReader {
  ...
  #audioDecoder = null;
  #audioReorderer = null;
  // LOCAL 4: counters behind stats().
  #framesDecoded = 0;
  #bytesReceived = 0;
  // LOCAL 6: the volume the tile set, held so it survives the reconnect that
  // builds a new AudioContext and a new gain node.
  #audioGain = null;
  #volume = 1;
  ...
  /**
   * LOCAL 6: mute or unmute. The AudioContext starts suspended without a user
   * gesture, which is upstream's muted state, so mute is the same suspension
  ...
  setMuted(muted) {
    ...
    const done = () => {
      if (this.#conf.onAudioMuted !== undefined) {
        this.#conf.onAudioMuted(muted);
      }
    };
    if (muted) {
      this.#audioCtx.suspend().then(done);
    } else {
      this.#audioCtx.resume().then(done);
    }
  }

  /**
   * LOCAL 6: set the playback volume, 0..1. Held across reconnects, since each
   * one builds a new AudioContext that would otherwise come back at full volume.
   *
   * @param {number} volume
   */
  setVolume(volume) {
  ...
  /**
   * LOCAL 3: terminal teardown. Upstream has no counterpart: every failure path
   * goes through #handleError, which restarts after a pause. A sink the grid has
   * disconnected must stop instead, so this releases the same resources and
   * leaves #state somewhere #handleError will not act on.
   *
   * Idempotent: calling it twice releases nothing a second time.
   */
  close() {
  ...
  /**
   * LOCAL 4: decode figures for a stats overlay, or null before a frame has been
   * drawn. Counters are cumulative; rates are the caller's to derive.
   *
   * @returns {?{codec: string, width: number, height: number, framesDecoded: number, bytesReceived: number, hasAudio: boolean}}
   */
  stats() {
    if (this.#videoCanvas === null || this.#framesDecoded === 0) {
      return null;
    }
    return {
      codec: this.#videoTrack !== null ? this.#videoTrack.codec : "",
  ...
  async #fetchFingerprint() {
    // LOCAL 2: a fingerprint supplied by the caller is used as-is. The app
    // fetches it in Go so a self-signed relay can be pinned; the fetch below is
    // upstream's path and still serves a caller that passes no fingerprint.
    const hex =
      this.#conf.fingerprint !== undefined && this.#conf.fingerprint !== ""
        ? this.#conf.fingerprint
        : await fetch(this.#conf.fingerprintUrl, {
    ...
    if (requestId === MediaMTXMoQReader.#VIDEO_REQUEST_ID) {
      ...
      this.#videoDecoder = new VideoDecoder({
        output: (frame) => {
          ...
          // LOCAL 4 and 5: a drawn frame is what the sink calls connected, so it
          // is counted and announced here rather than at subscription time.
          this.#framesDecoded++;
          if (this.#conf.onFrame !== undefined) {
            this.#conf.onFrame();
          }
        },
      ...
      // LOCAL 6: a gain node between the buffer sources and the destination.
      // Upstream connects each source straight to the destination, which leaves
      // no volume to set; the tile's volume control needs one.
        ...
        output: async (data) => {
          try {
            if (this.#audioCtx.state === "running") {
              ...
              src.buffer = buf;
              src.connect(this.#audioGain); // LOCAL 6
  ...
  #decodeVideo(data, groupId) {
    // LOCAL 4: counted before the queue check, so the figure stays a measure of
    // what arrived rather than of what the decoder had room for.
