...
type Engine interface {
	...
	// streamName in a viewer window, receiving it over the named transport.
	Command(s settings.Stream, streamName, transportName string) (args, env []string, err error)
}
...
// Select returns the viewer engine for the named watch transport. The transport
// is chosen per viewer, not taken from the publish settings, so a stream can be
// received over any transport the relay serves it on.
...
// WindowTitle returns the title a viewer sets on its window. It carries the
// transport as well as the stream name so a user watching one stream over
// several transports at once can tell the windows apart.
