...
// Stats is one encoder progress sample. Both publish engines emit it, each
// measuring it from what its own pipeline offers, so the meaning of a field is
// fixed here rather than by whichever engine filled it:
//
...
type Stats struct {
	...
	// which is how often the encoder emitted one. The two differ wherever a
	// backend paces its output independently of its source: the portal path
	// repeats the newest damage frame at the configured rate, so its Fps equals
	// the target whatever the screen does, and this is the figure a starved
	// capture or a static screen shows up in. A pipeline built without the rate
	// probe marks it missing instead of reporting a zero rate.
	CaptureFps float64
	Missing    Missing
}

// Missing is the set of Stats figures a sample carries no measurement for.
// ffmpeg reports "N/A" until the first packet is muxed, a per-interval figure has
// no value on the first sample of a run, and an engine instruments only what its
// pipeline exposes; none of the three is the measured zero that marks a stalled
// encoder. The zero value marks nothing missing, so an engine that measures a
// figure leaves its flag alone.
type Missing struct {
	Fps        bool
	CaptureFps bool
	SizeKiB    bool
...
// MarshalJSON writes every figure Missing marks as null, which is what keeps an
// unmeasured figure out of the UI's numbers. encoding/json has no per-field
// presence for a float64, hence the pointer-shaped wire struct.
...
// stdoutLineInitial and stdoutLineMax size the scanner behind onLine. A line is
// one message, and the roster the native grid answers grows with the stream
// count, so the limit is generous rather than tuned.
const (
...
// sample per ffmpeg -progress block. onLine, when non-nil, receives every line
// the child writes to stdout, for a child that talks back rather than only
// logging; both read the same pipe, so a caller passes one or the other.
// onExit fires once when the child exits, with a non-nil error only on an
// unexpected failure, the tail of stderr, and the path of the full run log.
func Start(
	exe string,
	args []string,
	hideWindow bool,
	...
	if onLine != nil {
		...
		go func() {
			defer readers.Done()
			// Mirrored into the run log a line at a time, so what the child said
			// is in the log the way it would be without a reader on the pipe.
