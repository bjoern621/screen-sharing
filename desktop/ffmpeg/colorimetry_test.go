...
func TestPublishedFullRangeStaysFullRangeThroughTheColourTag(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.colorRange, func(t *testing.T) {
			s := baseStream()
			s.Codec, s.Chroma, s.ColorRange = "libx264", "yuv420p", tc.colorRange
			options, filters := publishColour(t, s)
			...
			args = append(args, options...)
			args = append(args, "-pix_fmt", s.Chroma, "-f", "h264", stream)
			...
			decode := exec.CommandContext(ctx, exe, "-hide_banner", "-loglevel", "error",
				"-i", stream, "-frames:v", "1", "-pix_fmt", s.Chroma, "-f", "rawvideo", "-")
...
func publishColour(t *testing.T, s settings.Stream) (options, filters []string) {
	t.Helper()
