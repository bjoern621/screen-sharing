...
func TestALiveStreamCarriesNoRetryFigures(t *testing.T) {
	a := &App{run: &publishRun{settings: settings.Stream{Name: "bob"}, handle: liveHandle{}}}
...
func TestAPendingRetryCarriesTheAttemptAndTheBudget(t *testing.T) {
	a := &App{retry: &publishRetry{settings: settings.Stream{Name: "bob"}, attempts: 2}}
...
// TestAClosedGridHasATileOnNothing: the window's two halves are read under one lock and
// the closed case clears the set, because the report that clears it arrives only once
// the child's stdout is drained. A read taken in that gap would name tiles nobody can
// see.
//
// The window is the frontend's alone now - the control contract describes no grid,
// because how a viewer arranges what it receives is the shell's - so the rule is held
// by the getter that surface polls rather than by an assertion on the way to the wire.
func TestAClosedGridHasATileOnNothing(t *testing.T) {
	a := &App{nativeGridWatching: []string{"bob", "test-1"}}

	if a.NativeGridRunning() {
		t.Errorf("running = true with no window open, want false")
	}
	if watching := a.NativeGridWatching(); len(watching) != 0 {
		t.Errorf("watching = %v with no window open, want nothing", watching)
	}
}
