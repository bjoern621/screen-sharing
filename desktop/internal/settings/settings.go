...
type Stream struct {
	...
	// MoqPort is the relay's Media-over-QUIC listener, one number for two
	// listeners: MediaMTX serves the fingerprint endpoint on TCP there and the
	// WebTransport endpoint on UDP on the same number, which is its own default.
	// It is a watch-leg port with no publish counterpart, since no engine this app
	// drives publishes MoQ (viewer-architecture.md, "Reading Media over QUIC").
