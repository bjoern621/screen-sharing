...
// The empty setting is the capture's own size reaching the encoder unscaled. It is a
// value and not an absence, and the two answers are separated so a caller cannot mistake
...
func TestASetOutputResolutionParsesToItsTwoFigures(t *testing.T) {
	s := Stream{OutputResolution: "1920x1080"}
...
// A size crosses the wire as a string and comes back through the same spelling, so the
// two directions have to be each other's inverse: a value the option list wrote and the
...
// FormatSize is what an option list builds an entry's value with, and it has to write the
// same spelling the parser reads.
...
// Every value that reaches the parser was written by this side, so a malformed one is a
// caller that made it up. Each refusal names which part failed, because the caller that
// made it up is the one reading the message.
func TestAMalformedOutputResolutionIsRefused(t *testing.T) {
	cases := []struct {
		...
		{"below the floor", "8x8"},
		// Every chroma subsampling this app encodes in needs an even picture, so an odd
		// side is refused rather than rounded: a size the run silently changed is a size
		// no form can show back.
		{"odd width", "1921x1080"},
...
// A stored settings file carrying a size nothing can parse is an environment condition
// rather than a bug, so it travels as an error and the run refuses with it named.
func TestAMalformedStoredResolutionReachesTheCallerAsAnError(t *testing.T) {
	s := Stream{OutputResolution: "not-a-size"}
