...
func diagnostics(d Deps, s settings.Stream, est *screensharev1.Estimate) []*screensharev1.Diagnostic {
...
// a lower quality.
func diagnosticsAboutTheLine(s settings.Stream, est *screensharev1.Estimate) []*screensharev1.Diagnostic {
	if s.UplinkMbps <= 0 {
	...
	if est.GetHeadroomMbps() < 0 {
		out = append(out, diagnosticFor(screensharev1.Severity_SEVERITY_WARNING, KeyUplinkMbps,
			say(uplinkBelowPrediction,
				argBitrateMbps(est.GetBitrateMbps()), argUplinkMbps(s.UplinkMbps))))
		return out
	}
	...
	if !spreads || high <= float64(s.UplinkMbps) {
		return out
	}
	anchor, declared := warningPeaks[s.Mode]
	assert.Assert(declared, "a mode whose rate spreads states where its burst belongs", s.Mode)
	out = append(out, diagnosticFor(screensharev1.Severity_SEVERITY_WARNING, anchor,
		say(burstAboveUplink,
			argLowMbps(low), argHighMbps(high), argUplinkMbps(s.UplinkMbps), argMode(s.Mode))))
	return out
}

// diagnosticsAboutTheCapture says where the machine will not give the pipeline what the
// settings ask of it.
func diagnosticsAboutTheCapture(d Deps, s settings.Stream) []*screensharev1.Diagnostic {
	var out []*screensharev1.Diagnostic
	...
	if m, found := estimateMonitor(d, s); found && m.RefreshHz > 0 && s.Fps > m.RefreshHz {
		out = append(out, diagnosticFor(screensharev1.Severity_SEVERITY_WARNING, KeyFps,
			say(fpsAboveRefresh, argFps(s.Fps), argRefreshHz(m.RefreshHz))))
	}
...
func diagnosticsAboutTheViewer(s settings.Stream) []*screensharev1.Diagnostic {
	decode, err := capabilities.DecodeOf(s.Codec, s.Chroma)
	if err != nil {
	...
	if len(decode.Hardware) == 0 {
		return []*screensharev1.Diagnostic{
			diagnosticFor(screensharev1.Severity_SEVERITY_WARNING, KeyChroma,
				say(decodesOnCPU,
					argFormat(format), argChroma(s.Chroma),
...
func diagnosticsAboutThePrediction(d Deps, s settings.Stream, est *screensharev1.Estimate) []*screensharev1.Diagnostic {
	if est == nil {
		if _, found := estimateMonitor(d, s); !found {
			return []*screensharev1.Diagnostic{
				diagnosticFor(screensharev1.Severity_SEVERITY_INFO, KeyMonitor,
					say(monitorNotPriced, argMonitor(s.Monitor))),
			}
		}
		return []*screensharev1.Diagnostic{
