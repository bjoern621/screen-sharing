...
// The machine every draft below is judged against: one 1080p output at 60 Hz, which is
// what makes a 120 fps target a statement about the monitor rather than about an
...
// the software H.264 encoder over SRT at a bitrate target, on the one capture backend
// whose command asks nothing of the host it is rendered on, so a healthy configuration
// stays healthy on every platform this test runs on.
func diagnosticTestStream() settings.Stream {
	s := settings.Defaults()
	s.Capture = "gdigrab"
	s.Codec = "libx264"
	s.Mode = "cbr"
...
// diagnosticTestOfRank returns the diagnostics of one rank, so a case can say how many of them
// it expects rather than filtering at each site.
...
// diagnosticTestDrafts is the spread every whole-list property below is held against. Each
// one reaches a different diagnostic, so a rule that fires on one of them is a rule the
// invariants have seen.
func diagnosticTestDrafts() map[string]settings.Stream {
	drafts := map[string]settings.Stream{}

	drafts["a healthy configuration"] = diagnosticTestStream()

	unencodable := diagnosticTestStream()
	unencodable.Chroma = "gbrp"
	...
	// The colour range moves with it because libvpx codes tv range alone, and a gap on
	// that axis would refuse the command before the leg was ever asked about.
	uncarriable := diagnosticTestStream()
	uncarriable.Codec = "libvpx"
	uncarriable.ColorRange = "tv"
	uncarriable.Transport = "rtmp"
...
// A combination no engine can build is refused once, and the sentence that refuses it is
// the sentence the summary shows. Two answers to one question is the fork the whole
// contract exists to remove: a form that says one thing and a start button that fails
// with another leaves the user with nothing to act on.
func TestAnUnbuildableCombinationRefusesAndSaysWhy(t *testing.T) {
	d := diagnosticTestDeps()
	s := diagnosticTestStream()
	// x264 codes no planar RGB: the format is absent from this codec's chroma list, so
	// the combination is one no engine can build rather than one a machine cannot run.
	s.Chroma = "gbrp"
	...
	// The diagnostic states that the publish was refused and the summary carries the
	// refusal itself, which is the one raw string on this contract a diagnostic points
	// at rather than repeating (api/proto/screenshare/v1/text.proto).
...
// The healthy case is the one worth stating explicitly: nothing in the settings refuses
// the publish, so the start button is live and the summary carries the line it would run.
...
// A diagnostic anchors beside a control or beside nothing, and a key no shell has a widget
// for would render the diagnostic nowhere at all. The failure is silent, which is why the
// whole spread is held against the declared set rather than each rule against its own.
...
// Every diagnostic is ranked and says something. An unranked one sorts as though it were
// the least important and reads as though the backend had no opinion, and an empty one
// occupies a line in the form to say nothing.
...
// The list is ranked so a shell renders it in the order it is given. A shell that had to
// sort it would be ranking diagnostics itself, which is the judgement the backend is here
// to make.
...
// A line under the prediction is the failure that announces itself least: nothing slows
// down, the transport drops what it cannot ship, and the viewer sees a stall. It is a
// diagnostic and not a refusal, since the stream does run.
...
func TestAPixelFormatNoGpuDecodesCostsRatherThanRefuses(t *testing.T) {
	d := diagnosticTestDeps()
	s := diagnosticTestStream()
	// No vendor put H.264's High 4:4:4 Predictive profile in silicon, so this pair has no
	// hardware decoder anywhere.
	s.Chroma = "yuv444p"
...
// The estimate the summary shows is the one the diagnostics were ranked against. Computing
// it a second time would be a second answer, and the two would part the first time either
...
// It is the whole of what it states now: the shorthand for the configuration and the one
// per group used to be composed here, and both were screen copy - a separator, an
// abbreviation and a length, chosen where none of the three is visible.
...
// Resolve runs on every keystroke, so a diagnostic list that moved between two identical
// drafts would reorder the form under a user who changed nothing.
