...
func estimate(d Deps, s settings.Stream) *screensharev1.Estimate {
	m, found := estimateMonitor(d, s)
	if !found || m.Width <= 0 || m.Height <= 0 || s.Fps <= 0 {
		return nil
	}
	chroma, known := estimateChromas[s.Chroma]
	if !known {
		return nil
	}

	pixelRate := float64(m.Width) * float64(m.Height) * float64(s.Fps)
	...
	est := &screensharev1.Estimate{
		BitrateMbps:  coded,
		RawMbps:      raw,
		HeadroomMbps: float64(s.UplinkMbps) - coded,
	}
	...
	assert.Assert(est.GetHeadroomMbps() == float64(s.UplinkMbps)-est.GetBitrateMbps(),
...
func estimateCoded(s settings.Stream, pixelRate, raw float64, chroma estimateChromaCost) (float64, bool) {
	switch s.Mode {
	case capabilities.ModeCbr, capabilities.ModeAbr, capabilities.ModeVbr:
		...
		return float64(s.BitrateM), true
...
func estimateConstantQuality(s settings.Stream, pixelRate float64, chroma estimateChromaCost) (float64, bool) {
	c, known := capabilities.Get(s.Codec)
	if !known {
		return 0, false
	}
	...
	if engine, err := publish.EngineFor(s.Capture); err == nil {
		if scale := c.CqMaxOn(engine); scale > 0 {
			cq = float64(s.Cq) * estimateAnchorCqMax / float64(scale)
		}
	}
...
func estimateSpread(s settings.Stream, est *screensharev1.Estimate) (low, high float64, spreads bool) {
	if est == nil {
		return 0, 0, false
	}
	switch s.Mode {
	...
	case capabilities.ModeVbr:
		...
		if s.MaxrateM > s.BitrateM {
			return float64(s.BitrateM), float64(s.MaxrateM), true
		}
	}
...
func estimateMonitor(d Deps, s settings.Stream) (display.Monitor, bool) {
