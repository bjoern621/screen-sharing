...
func fieldState(d Deps, s settings.Stream, key string) state {
...
func optionState(d Deps, s settings.Stream, key, value string) (enabled bool, reason *screensharev1.Text) {
...
var availabilityRules = map[string]func(availability) state{
	...
	KeyRelayPort: func(av availability) state { return availabilityShownFor(av.s.Transport == availabilitySrt) },
	KeyRtspPort:  func(av availability) state { return availabilityShownFor(av.s.Transport == availabilityRtsp) },
	KeyWebrtcPort: func(av availability) state {
		return availabilityShownFor(av.s.Transport == availabilityWebrtc)
	},
	KeyRtmpPort: func(av availability) state { return availabilityShownFor(av.s.Transport == availabilityRtmp) },
	...
	KeyHlsPort: func(av availability) state {
		return availabilityShownFor(av.s.WatchTransport == availabilityHls)
	},
	// MoQ is gated on neither leg the others follow. No engine publishes it and no
	// player opens it, so it is reachable from the web grid alone and shown whatever
	// either dropdown says.
	KeyMoqPort: func(availability) state { return availabilityLive() },
	...
	KeyColorRange: func(av availability) state {
		...
		if slices.Contains(availabilityFullRangeChromas, av.s.Chroma) {
	...
	KeyBitrateM: func(av availability) state {
		return av.knob(KeyBitrateM, av.mode().usesBitrate, say(bitrateNotInMode, argMode(av.s.Mode)))
	},
	...
	KeyBframes: func(av availability) state {
		...
		reason := say(bframesOffInMode, argMode(av.s.Mode))
		if usesBframes {
	...
	KeyEncPreset: func(av availability) state {
		...
		if takesPreset && mode.pinsPreset {
			reason = say(presetPinnedByMode, argMode(av.s.Mode), argEncPreset(mode.pinnedPreset))
		}
	...
	KeyAudioCodec: func(av availability) state {
		if av.s.Audio == "" || av.s.Audio == platform.AudioSourceNone {
	...
	KeyDrmMap: func(av availability) state {
		if av.s.Capture != availabilityKmsgrab {
	...
	KeyMonitor: func(av availability) state {
		if slices.Contains(availabilityMonitorless, av.s.Capture) {
			return availabilityDisabled(say(captureTakesNoMonitor, argCapture(av.s.Capture)))
		}
		return availabilityLive()
	},
	...
	KeySrtPublishLatencyMs: func(av availability) state {
		return availabilityShownFor(av.s.Transport == availabilitySrt)
	},
	KeyRtspPublishProtocol: func(av availability) state {
		return availabilityShownFor(av.s.Transport == availabilityRtsp)
	},
	// The watch-leg knobs follow the legs that read them.
	KeySrtWatchLatencyMs: func(av availability) state {
		return availabilityShownFor(av.s.WatchTransport == availabilitySrt)
	},
	KeyRtspWatchProtocol: func(av availability) state {
		return availabilityShownFor(av.s.WatchTransport == availabilityRtsp)
	},

	KeyUplinkMbps:     func(availability) state { return availabilityLive() },
	KeyWatchTransport: func(availability) state { return availabilityLive() },
...
var availabilityOptionRules = map[string]func(availability, string) *screensharev1.Text{
	...
	// external player opens a URL through libavformat, and the tile grid decodes
	// through a GStreamer pipeline, so the two reach different protocol sets.
	KeyWatchTransport: func(av availability, value string) *screensharev1.Text {
...
func availabilityOf(d Deps, s settings.Stream) availability {
	av := availability{deps: d, s: s}
	if engine, err := publish.EngineFor(s.Capture); err == nil {
		av.engine = engine
	}
	av.codec, av.knownCodec = capabilities.Get(s.Codec)
	if av.engine != "" && av.knownCodec {
		av.path, av.onDevicePath = gpupath.For(av.engine, s.Capture, av.codec.Family)
	}
	return av
}
...
func (av availability) mode() availabilityMode {
	return availabilityModes[av.s.Mode]
}
...
func (av availability) transportReason(name string) *screensharev1.Text {
	allowed, err := publish.TransportsFor(av.s.Capture)
	if err != nil {
		return nil
	}
	assert.Assert(av.engine != "", "a capture backend the app runs names a publish engine", av.s.Capture)
	if slices.Contains(allowed, name) {
		return nil
	}
	return say(engineHasNoPublishSink,
		argCapture(av.s.Capture), argEngine(av.engine), argTransport(name))
}
...
func (av availability) codecReason(codec string) *screensharev1.Text {
	...
	if !transport.CanPublish(av.s.Transport, av.engine) {
		return nil
	}
	if transport.CanPublishFormat(av.s.Transport, av.engine, c.Format) {
		return nil
	}
...
func (av availability) carryBlockReason(c capabilities.Codec) *screensharev1.Text {
	...
	otherEngine := ""
	if transport.CanPublishFormat(av.s.Transport, other, c.Format) {
		otherEngine = other
	}
	return say(transportCarriesNoCodec,
		argTransport(av.s.Transport),
		argCodec(c.Name),
		argEngine(av.engine),
...
func (av availability) audioCodecReason(name string) *screensharev1.Text {
	...
	carriage, ok := transport.PublishCarriage(av.s.Transport, av.engine)
...
func (av availability) frameMemoryReason(memory string) *screensharev1.Text {
	...
	if !av.onDevicePath {
		...
		return say(pairHasNoDeviceMemory,
			argCapture(av.s.Capture), argCodec(av.s.Codec), argEngine(av.engine))
	}
	if !av.path.Colour.TradesColour() {
		...
		return say(pairConvertsOnDevice,
			argCapture(av.s.Capture), argCodec(av.s.Codec), argMemory(gpupath.MemoryGpu))
	}
	if memory != gpupath.MemoryGpu {
		return nil
	}
	return say(pairTradesColour,
		argCapture(av.s.Capture),
		argCodec(av.s.Codec),
		argEngine(av.engine),
...
func (av availability) outputResolutionReason(value string) *screensharev1.Text {
	...
	return say(devicePathHasNoScaler,
		argCapture(av.s.Capture), argCodec(av.s.Codec), argMemory(gpupath.MemorySystem))
}
...
func (av availability) engineRule(key string) (availabilityEngineRule, bool) {
	for _, r := range availabilityEngineRules {
		...
		if len(r.modes) > 0 && !slices.Contains(r.modes, av.s.Mode) {
			continue
		}
		...
		if slices.Contains(r.codecs, av.s.Codec) {
			return r, true
		}
...
func (av availability) vaapiCeilingNote() *screensharev1.Text {
	if av.engine != capabilities.EngineGst || av.s.Mode != capabilities.ModeVbr {
		return nil
	}
	if !av.knownCodec || av.codec.Family != capabilities.FamilyVaapi {
		return nil
	}
	if av.s.BitrateM <= 0 || av.s.MaxrateM <= av.s.BitrateM*2 {
		return nil
	}
	return say(vaapiCeilingBound, argMaxrateMbps(av.s.BitrateM*2), argBitrateTarget(av.s.BitrateM))
}
...
func (av availability) staysOnDevice() bool {
	if !av.onDevicePath || av.s.CaptureMemory == gpupath.MemorySystem {
		return false
	}
	return !(av.s.CaptureMemory == gpupath.MemoryAuto && av.path.Colour.TradesColour())
}
...
func (av availability) encoderColourReason() *screensharev1.Text {
	if !av.onDevicePath || av.s.CaptureMemory != gpupath.MemoryGpuEncoderColor {
		return nil
	}
	if !av.path.Colour.TradesColour() {
		return nil
...
func (av availability) decodeNote() *screensharev1.Text {
	...
	for _, d := range capabilities.Decoders {
		if d.Format != av.codec.Format || !d.Hardware() || !slices.Contains(d.Chromas, av.s.Chroma) {
			continue
		}
	...
	for _, d := range capabilities.Decoders {
		...
		if d.Hardware() && limited == "" && !slices.Contains(d.Chromas, av.s.Chroma) {
			limited = d.Family
		}
	}
	return say(decodesOnCPU,
		argFormat(av.codec.Format), argChroma(av.s.Chroma), argDecoder(software), argDecodeFamily(limited))
}
