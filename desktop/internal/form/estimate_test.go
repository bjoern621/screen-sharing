...
func estimateTestStream() settings.Stream {
	s := settings.Defaults()
	s.Capture = "x11grab"
	s.Codec = "libx264"
	s.Mode = "crf"
...
func TestTheQuantizerIsPlacedOnTheAnchorScale(t *testing.T) {
	...
	onFiftyOne.Codec = "libx265"
	onFiftyOne.Cq = 51

	onSixtyThree := estimateTestStream()
	onSixtyThree.Codec = "libvpx-vp9"
	onSixtyThree.Cq = 63
...
func TestAnUnpriceableSettingPredictsNothing(t *testing.T) {
	cases := map[string]func(*settings.Stream){
		"a pixel format no codec encodes":  func(s *settings.Stream) { s.Chroma = "nv12" },
		"a rate-control mode that is none": func(s *settings.Stream) { s.Mode = "magic" },
		"a codec the table does not hold":  func(s *settings.Stream) { s.Codec = "libnope" },
		"no frame rate at all":             func(s *settings.Stream) { s.Fps = 0 },
	}
