...
func TestTheEncoderPresetLadderIsContiguousFromTheFirstStep(t *testing.T) {
	...
	if !slices.Contains(offered, settings.Defaults().EncPreset) {
		t.Errorf("the ladder %v does not carry the default preset %q", offered, settings.Defaults().EncPreset)
	}
}
...
func TestTheDefaultsAreValuesTheFormOffers(t *testing.T) {
	...
	cases := map[string]string{
		KeyCapture:             s.Capture,
		KeyCaptureMemory:       s.CaptureMemory,
		KeyDrmMap:              s.DrmMap,
		KeyCodec:               s.Codec,
		KeyChroma:              s.Chroma,
		KeyColorRange:          s.ColorRange,
		KeyMode:                s.Mode,
		KeyEncPreset:           s.EncPreset,
		KeyAudio:               s.Audio,
		KeyAudioCodec:          s.AudioCodec,
		KeyTransport:           s.Transport,
		KeyWatchTransport:      s.WatchTransport,
		KeyRtspPublishProtocol: s.RtspPublishProtocol,
		KeyRtspWatchProtocol:   s.RtspWatchProtocol,
	}
