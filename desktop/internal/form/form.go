...
type field struct {
	...
	value func(settings.Stream) *screensharev1.FieldValue
	...
	// take no options.
	options func(Deps, settings.Stream) []*screensharev1.FieldOption
	// bounds sizes a number or a slider, nil on the controls that take no range.
	bounds func(Deps, settings.Stream) *screensharev1.NumericRange
}
...
// exists to remove.
func Resolve(d Deps, draft settings.Stream) *screensharev1.Form {
...
func resolveGroups(d Deps, s settings.Stream) []*screensharev1.FieldGroup {
...
// verdict on it.
func resolveField(d Deps, s settings.Stream, f *field) *screensharev1.Field {
