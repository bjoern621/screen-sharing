...
func draftOf(m *screensharev1.StreamSettings, verb string) (settings.Stream, error) {
	if m == nil || proto.Equal(m, &screensharev1.StreamSettings{}) {
		return settings.Stream{}, invalidArgument("no settings to %s", verb)
	}
	return wire.StreamSettings(m), nil
}
...
func (s *Server) SavePreset(ctx context.Context, req *screensharev1.SavePresetRequest) (*screensharev1.SavePresetResponse, error) {
	...
	draft, err := draftOf(req.GetSettings(), "save as a preset")
	if err != nil {
		return nil, err
	}

	if err := settings.SavePreset(name, draft); err != nil {
