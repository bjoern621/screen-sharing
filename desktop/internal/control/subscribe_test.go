...
//
// grpc.ServerStream is embedded and left nil: the two methods Subscribe uses are
// Context and Send, and a call to any other would be a call this test wants to hear
// about as a panic rather than to have quietly satisfied. The buffer on sent is what
// keeps Send from blocking after a test has stopped reading, so a test that fails still
// lets the call under test return instead of deadlocking on its way out.
...
// unknownKind is an EventKind value no build declares, which is what a shell generated
// against a later minor version would send. It is a number rather than a misspelt name
// because the kinds are an enum now: the mistake a shell can still make is naming a
// kind that exists somewhere and not here.
const unknownKind = screensharev1.EventKind(9999)

// TestAnUnknownEventKindIsRefusedRatherThanIgnored: a kind this build has none of would
// otherwise leave the shell holding an open stream that never delivers, which reads as
// a backend where nothing is happening rather than as a name that was got wrong. The
// refusal has to name the kind, because the request may have carried several and only
...
// TestANarrowedSubscriptionReceivesOnlyItsKinds: narrowing exists for a surface that
// wants the state changes without the per-second statistics, so a filter that leaked
// would hand that surface exactly the traffic it asked not to have.
func TestANarrowedSubscriptionReceivesOnlyItsKinds(t *testing.T) {
	...
	// The subscription opens on another goroutine and the broker offers no signal for
	// when it has been registered, so both kinds are published until something arrives
	// rather than once behind a sleep. Publishing both every round is what makes the
	// arrival conclusive: a filter that let the wrong kind through would have delivered
	// it by the round the right one is delivered on.
...
// TestASubscriptionEndsWithTheClientThatOpenedIt: the broker sends on a channel it holds
// until the cancel removes it, so a call that outlived its client would leave a
// subscriber nothing reads and every publish still has to walk past. The call returning
// is the observable half of that release - the only way out of the loop is through the
// deferred cancel - and it is what a shell closing its window has to produce.
