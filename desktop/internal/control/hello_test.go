...
func (f *fakeBackend) Settings() settings.Stream                      { return settings.Stream{} }
...
func (f *fakeBackend) MoqCert(context.Context, string) (moq.Cert, string, error) {
	return moq.Cert{}, "", f.err
}

func (f *fakeBackend) MeasureUplink(context.Context) (float64, error) { return 0, f.err }
func (f *fakeBackend) MeasureEncodeRate(context.Context, settings.Stream) (encoderate.Rate, error) {
	return encoderate.Rate{}, f.err
}

func (f *fakeBackend) SaveSettings(settings.Stream) error  { return f.err }
func (f *fakeBackend) StartPublish(settings.Stream) error  { return f.err }
func (f *fakeBackend) ApplyToStream(settings.Stream) error { return f.err }
