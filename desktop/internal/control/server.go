...
type Backend interface {
	...
	MaxTestStreams() int
	// MoqCert reads the relay's Media-over-QUIC certificate fingerprint for one stream,
	// and the endpoint the pin belongs to.
	//
	// The URL travels with the certificate rather than being rebuilt by the caller,
	// because it is built from the relay host and port the backend holds: a caller that
	// composed it from settings it read separately would be a second definition of the
	// endpoint, and one of the two would eventually name a port the other had moved off.
	MoqCert(ctx context.Context, streamName string) (cert moq.Cert, url string, err error)
	...
	MeasureEncodeRate(ctx context.Context, s settings.Stream) (encoderate.Rate, error)
