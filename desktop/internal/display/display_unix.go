...
// A Wayland session falls back to the X11 enumerator so XWayland-backed setups
// still report their outputs.
// The empty server is what platform.Detect reports off Linux and in a headless
// session, and it has no enumerator: List answers with its placeholder.
//
...
// and answering it with no enumerator would put a real session behind the
// placeholder a headless one gets.
