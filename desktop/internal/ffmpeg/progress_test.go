...
// progressBlock renders one -progress block from the fields a test sets, in
// ffmpeg's own order and spelling.
func progressBlock(fields ...string) string {
	return strings.Join(fields, "\n") + "\nprogress=continue\n"
}

// newTestParser returns a parser whose clock the test advances by hand, so the
// per-interval figures are measured against a known interval.
...
// The parser takes the cumulative counters off a block and derives the
// per-interval figures against the previous block. The first block has no
// predecessor, so it carries no rate; the second carries them, measured against
...
func TestParseProgressSamples(t *testing.T) {
	...
	wantFirst := Stats{
		...
		Speed:   1,
		AvgMbps: 2,
		Missing: Missing{Fps: true, CaptureFps: true, InstMbps: true},
	}
	if first != wantFirst {
...
// Fps is the rate over the last interval, not the run: ffmpeg's own fps field
// still reports the 60 fps the run averaged while the frame counter advances by
// five in the second the capture collapsed.
...
// "N/A" is ffmpeg saying it has no figure yet, which is not a figure of zero.
// The block after it has no byte baseline either, so its bitrate stays
// unmeasured rather than counting the whole output as one interval's worth.
func TestParseProgressUnmeasuredFields(t *testing.T) {
	...
	want := Missing{Fps: true, CaptureFps: true, SizeKiB: true, TimeSec: true, Speed: true, InstMbps: true, AvgMbps: true}
...
// The wire form separates the two states the UI has to tell apart: an
// unmeasured figure is null, a measured zero is 0.
