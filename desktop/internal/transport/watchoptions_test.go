...
func watchSettings() settings.Stream {
...
// A transport with no watch form of its own declares no knobs, and neither does
...
func TestSetWatchOption(t *testing.T) {
	...
	if s.SrtWatchLatencyMs != 800 {
		t.Errorf("latency = %d, want 800", s.SrtWatchLatencyMs)
	}
	if err := SetWatchOption("rtsp", &s, "rtspWatchProtocol", "udp"); err != nil {
		t.Fatal(err)
	}
	if s.RtspWatchProtocol != "udp" {
		t.Errorf("protocol = %q, want udp", s.RtspWatchProtocol)
	}
}

// A refused value leaves the settings as they were: the caller is told, rather
// than the knob quietly taking something else.
...
// Every option a transport declares can be written back, which is what the
// viewer does with the values it read.
