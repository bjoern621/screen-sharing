...
//
// Windows walks the continue handlers even when an exception handler already said the
// exception was handled, and the Go runtime's last continue handler crashes the process on
// anything it does not recognise.
// An exception handler alone is therefore answered and then killed anyway, and a continue
// handler alone is a resume from an exception nothing handled, which Windows ends the process
// over.
// Both go in first, ahead of the runtime's own pair, the shape the runtime uses for the
// exceptions it handles itself.
...
//
// GLib names every thread it starts, and on Windows that is spelled as raising exception
// 0x406D1388 with a debugger expected to be listening.
...
//
// Answered here rather than avoided upstream because there is nothing upstream to avoid: the raise
// is how thread naming is spelled on this platform, and every GStreamer thread this backend starts
// does it.
// Declining the exception is the process saying it has no debugger and needs none.
//
// Called before Init, because the threads that raise it start during the registry scan.
