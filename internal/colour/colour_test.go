...
// The two forms of the field have to answer alike, because callers hold one against the
// other: a capture negotiates a name and a capsfilter pins four numbers. This is the case
// that says so for every curve the enum carries, rather than for the two anybody remembered.
func TestBothSpellingsOfOneCurveAnswerAlike(t *testing.T) {
	...
	for number, want := range transferNicks {
		// A capsfilter pins range, matrix, transfer and primaries. The other three are what
		// they are: only the third is read.
		value := "1:1:" + number + ":1"
		if got := TransferOfColorimetry(value); got != want {
...
// A signal carrying no colorimetry at all carries no transfer, which is what makes "no
// transfer" a readable answer rather than a gap. Every caller acts on it: a capture stating
// none narrows nothing, and a decode stating none is standard range.
...
// A value this table has never seen is answered with itself rather than with a guess. It is
// the case a new GStreamer produces: a curve added to the enum reaches a reader as whatever
...
// The verdict is the two BT.2100 curves and nothing else. Everything else in the enum
// describes a standard-range picture whatever its primaries are, which is why a wide-gamut
// SDR desktop is not HDR and the primaries are read nowhere.
func TestOnlyTheTwoBT2100CurvesAreHDR(t *testing.T) {
	...
	// The nicks the two names resolve to are the ones the constants spell, which is what
	// lets a caller compare a transfer it read off a pipeline against them.
