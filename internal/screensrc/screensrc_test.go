...
// unenumerated is an index no machine running this test has an output for, which is what
// makes the heads below the same on every machine. What a head does with an index the
// enumeration carries is the enumeration's business and differs per machine; what it does
// with one it does not is this package's, and is the shape asserted here.
const unenumerated = 4096

// The two tables describe one set of elements. init asserts it at load, and this says so as
// a test as well, because a build that fails at load fails every test with one message and
// this one names which row is wrong.
...
// Which sessions can show one screen, and what the others are told to say instead. The set is
// small and stated here as well as in the table, because the table's rows are the whole of the
// feature's reach: a row added or dropped changes what a wizard on that platform offers.
func TestWhichSessionsReadOneScreen(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		// The portal is Wayland's only way to a screen and it answers with whatever the
		// picker was told, so there is no picture of one named output to take.
		...
		// avfvideosrc chooses its own display, which is also why the monitor setting does
		// not reach the macOS publish pipeline.
		...
		t.Run(name, func(t *testing.T) {
			...
			// The sentence is the shell's and is written per platform, so the statement has
			// to carry which platform it is about.
...
// The invariant the package exists for: the picture the wizard offers a screen by is taken
// from the same element and the same rectangle the stream would carry, because both are built
// from one head.
...
// A preview is paced and reduced before anything downstream sees it. Both are the reason it
// costs a wizard tile rather than a stream: the default render chain on Linux writes no size
// bound at all, so a preview that did not reduce its own frames would upload whole desktops.
...
// rectangle, which is the same answer x11grab gives. The Windows head names the index whatever
// the enumeration knows, because the index is the Windows enumeration's own.
