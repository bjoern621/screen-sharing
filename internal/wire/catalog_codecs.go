...
// The codec half of the catalog: the three capability tables and the gaps that narrow them per
// engine.
//
// They sit beside the machine-side conversions rather than inside them because they are a
// projection of one page - docs/domain-model.md - and reading them together is how a reader checks
// that the projection lost nothing.
// Every row crosses whole: a row narrowed on the way over would be a second,
// quieter definition of the same codec.

// catalogCodecs converts the video capability table.
//
// Gaps cross alongside the row rather than being applied to it.
// Narrowing Chromas to what both engines encode would take a pixel format away from the engine that
// has it, with no reason shown anywhere; carrying the gap instead is what lets a form offer the
...
// hardware rather than this machine's, because a stream is published once and watched on whatever
// the watchers have.
...
// They cross so that a shell can state what a track will cost without a table of its own,
// and so that the figure it shows is the one both engines build their branch from.
...
// catalogGaps converts a row's gaps, normalizing the option each one names to the spelling a form
// control binds by (gapOptions).
//
// The reason crosses verbatim.
// It is written to name the limit and which side has it, so a shell shows it where the option would
// have been and the reader learns what to change rather than only that the option is gone.
...
//
// capabilities.Options names an option by the Go JSON tag settings.Settings carries ("chroma",
// "mode", "colorRange"); a form control names the same option by its proto field name,
// which is what StreamSettings spells it and what form/keys.go binds a widget by ("chroma", "mode",
// "color_range").
// docs/ipc-api.md promises a shell that receives a gap can grey the matching control with no
// mapping of its own, so the normalization happens here, once, and both the gaps and the
// capability_options list are emitted through it, which is what keeps the list and the gaps from
// disagreeing.
...
// The empty string passes through, because a gap naming no option is the gap that takes the codec
// off an engine altogether and has no control to point at.
...
// catalogOptions lists the settings fields a Gap may name, in table order and in the spelling the
// gaps themselves arrive in.
...
// engineLimits converts the two per-engine numeric columns into one row per engine that bounds the
// codec by either of them.
//
// They were two maps keyed by engine name and are one typed row, because the contract carries the
// engine as an enum everywhere else and a map takes no enum key.
// Merging them here is what makes the row a fact about an engine rather than a column heading:
...
// Absence is how the table says there is no quantizer scale or no bitrate ceiling,
// and zero is a number the encoder would have accepted.
