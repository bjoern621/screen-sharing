...
// That is the only answer a launcher can give, nothing in a launched pipeline knowing what the
// capture turned out to be, and it is the wrong one for a publish: a surface with a transfer
...
// table, written into the description it handed over, which of them the surface carries is the
...
// Deliberate rather than a fallback: caps carrying no transfer mean a standard-range surface,
// and the parent leads its table with the standard-range row.
...
// A caps event crosses that pad on its way downstream, so the narrowing lands ahead of the
// negotiation it is about rather than after an encoder has been told a colour.
//
// Sources are found by shape, as everywhere else here: an element with no sink pad produces frames
// from outside the pipeline, and a name would live both here and in whichever backend built the
// description.
func narrowToSurface(pipeline gst.Pipeline) {
	for v := range pipeline.IterateSources().Values() {
		...
		pad.AddProbe(gst.PadProbeTypeEventDownstream, func(_ gst.Pad, info *gst.PadProbeInfo) gst.PadProbeReturn {
			...
			// and a second narrowing is the same edit twice.
...
// The last keeps this from producing an empty capsfilter, which would fail the run with a
// negotiation error in place of the refusal the parent makes on the same caps, naming both ends.
...
// does.
//
// Each structure is copied with its own caps features, the feature deciding which pads can link at
// all: one that arrived naming device memory and left naming none pins the frames back into the
// system-memory round trip the whole path exists to avoid.
...
// The capture produces one format, so several structures are caps nothing has negotiated yet.
