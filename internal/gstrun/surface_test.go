...
// A capsfilter's alternatives are narrowed to the surface's own colour, so what the encoder
// is handed is what the desktop had.
//
// The pipelines below stand in for the publish chain: a source claiming a colour, the
// videoconvert every capture chain carries, and the encoder input stating each colour this
// publish accepts. What replaces the encoder is a fakesink, because what is under test is
// the negotiation ahead of it and an encoder would only add a plugin this may not carry.
//
// The claim matters because the untouched behaviour is silent. videoconvert fixates a
// capsfilter to its first structure whatever the frames carry, so without the narrowing an
// HDR desktop would be converted into the standard-range row and every viewer would be told
// a colour the surface never had.

// surfaceRows is the colours the encoder input accepts, in the order a publish states them:
// standard range first, then the two BT.2100 curves, all at full range
// (publish/gstpipeline.go, gstColorimetries).
...
func TestTheEncoderInputTakesTheSurfacesOwnTransfer(t *testing.T) {
	for _, tc := range []struct {
		...
		// The two HDR curves: each wins its own row, which is the whole point. Neither
		// leads the rows, so an answer of the leading one is the failure this exists for.
		{surface: "bt2100-pq", want: "1:6:14:7"},
		{surface: "bt2100-hlg", want: "1:6:15:7"},
		// A standard-range desktop takes the leading row, which is what it would take with
		// no narrowing at all: the answer does not move, because the row already described
		// the surface.
		{surface: "bt709", want: "1:3:5:1"},
		// A transfer no row names narrows nothing, so the leading row stands and the parent
		// refuses the run on the same caps, in words that name both ends (publish/gsthdr.go).
		// An empty capsfilter here would fail with a negotiation error instead, which names
		// neither.
...
// Mastering display metadata reaches the encoder where the capture states it, and is
// absent where it does not.
//
// It rides through because nothing names it: the encoder input pins the memory, the format,
// the colorimetry and the size, so every other field the capture put on its caps survives
// the intersection untouched. That is the whole mechanism, and this is what says it stays
// true - a row that started naming these fields would drop the ones it did not match, and a
// stream would lose the grading it was mastered against with nothing saying so.
func TestMasteringMetadataReachesTheEncoder(t *testing.T) {
	...
	// A capture stating neither hands the encoder neither, rather than a default somebody
	// made up: a grading nobody measured is worse than none at all.
...
// encoderInput plays one description to its end the way a run does, and returns the
// colorimetry the last element was handed.
//
// The pipeline is driven here rather than through Run because the answer is on a pad inside
// it: a run reports what the capture negotiated, and what this is about is the far end of
// the conversion. Everything a run does to the pipeline before it plays is done here in the
// same order.
...
// negotiatedCaps plays one description to its end and returns the caps the element named
// sink was handed.
