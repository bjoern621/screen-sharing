...
// GetReceiveState returns the streams the backend is decoding, one entry per stream and
// leg it is received over. A shell reads it when it mounts and receives the same message
// on the event stream thereafter.
