...
//
// The contract version is in the name rather than negotiated on the connection,
// which is what makes a v2 a second pipe: two backends on different majors can then run side by
// side, and a shell that opens the wrong one fails to connect instead of connecting and being
// turned away at Hello.
const pipeName = `\\.\pipe\screenshare-control-v1`

// Listen opens the named pipe this backend serves the control contract on.
//
// There is no stale endpoint to clean up here, unlike on Unix: a named pipe exists only while a
// process holds it and disappears when that process dies, however it dies.
// The same property is what makes a second backend's Listen fail rather than steal the name,
// so the error this returns is also the app's "another instance is already running" signal.
func Listen() (net.Listener, error) {
	...
	if err != nil {
		// A name already held answers ERROR_ACCESS_DENIED, which the syscall package maps to this:
		// the first instance of a pipe owns the name, and a second creation of it is refused rather than
		// queued.
		// A name held by another user's process refuses the same way, and the conclusion is the same one
		// - something else is serving this endpoint and this process will not be reached
		// (ErrAddressInUse).
...
// pipeSecurity builds the pipe's security descriptor: nobody but the user running this backend may
// open it, and not from another machine.
//
...
// right are not.
// CREATOR OWNER is substituted only through inheritance and grants nothing on an object created
// with it named directly, and any group this user belongs to would also admit every other member of
// that group.
//
// Nothing else appears - no Administrators ACE, no SYSTEM ACE - and the omission is deliberate.
// An administrator can take ownership of the pipe whether or not they are named,
// so naming them buys nothing and widens the list to every account that can elevate on this
// machine.
