...
// a rule binds where the facts match and lands on the values that match, and writing the second as
// a bare list would leave a numeric control unable to say "everything above this".
// A codec's bitrate ceiling is therefore a rule like every other constraint,
// rather than a column with a consumer of its own.
//
// The zero Match is everything.
// In When that would be an axis the rule does not care about, which is written by leaving the axis
// out instead, and registration refuses it.
...
type Match struct {
	...
	any []string
	// low and high bound a numeric match, inclusive, each read only where its flag is set.
	// Two flags rather than a sentinel: zero is a legal bitrate and a legal quantizer,
	// so a sentinel would make one real value unmatchable.
	low     int
	high    int
	hasLow  bool
	hasHigh bool
	// numeric records which of the two halves above was written, so a match built for a numeric
	// control cannot be silently read as an empty list of identifiers.
	numeric bool
}
...
// everything reports whether this match is the zero one, which names no value and therefore stands
// for all of them.
...
//
// It is what turns a numeric refusal into the ends a control is offered between,
// so the slider and the refusal cannot disagree.
// A band that clips neither end would leave a hole in the middle, which no offered range can
// express, and it is asserted against rather than approximated: a control silently offering a value
// the publish refuses is the failure this whole system exists to remove.
func (m Match) narrow(low, high int) (int, int) {
	...
	case m.low <= low && m.high >= high:
		// The band covers the whole range.
		// Nothing is left, which the caller reads as a control with no legal value rather than as an
		// inverted range.
		return low, low - 1
	case m.low <= low:
