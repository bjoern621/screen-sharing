...
// The rules and the gaps are one table read two ways, so they answer the same for every
// codec, engine, option and value there is.
//
// It is exhaustive rather than sampled on purpose. The conversion exists so the gap
// mechanism can be retired, and a migration that agrees on the rows somebody thought to
// list is a migration nobody can retire anything on.
...
// A codec an engine has no encoder for is refused as an entry of the codec control
// itself, which is the one gap that names no option.
...
// A gap belonging to the format or the library binds on both engines, and a gap belonging
// to one wrapper binds only there. It is the distinction the engine column carried, and
// the rules keep it by naming the axis or leaving it out.
func TestAnEngineWideFactBindsOnBothEngines(t *testing.T) {
	// libvpx cannot signal a colour range on either engine: VP8's keyframe header has no
	// field for it, which is the format's limit rather than one builder's.
	for _, engine := range Engines {
	...
	// x265 codes planar RGB through ffmpeg and no GStreamer element takes it, so the same
	// value is one engine's alone.
...
// Every reason a rule carries is the one its row declared, and it arrives naming the
// codec it is about. A statement that lost its subject is a greyed control the reader
// cannot act on.
...
// codecFacts is the three axes the codec table's rules match on. Nothing else is needed:
// a rule binds on the axes it names, and naming more here would be this test asserting
// facts the table never claimed.
...
// The offered range and the accepted value are one answer now. For every codec and
// engine, what the rules narrow a control to is the top the column declares, which is
// what makes the ceilings part of the same shape as the gaps rather than a second system
// beside them.
...
// A ceiling binds in the modes that read the knob and nowhere else. The columns could not
// say this at all, so the form narrowed the bitrate even in the two modes that send none.
func TestACeilingBindsOnlyInTheModesThatReadIt(t *testing.T) {
	const offered = 10000

	// libsvtav1 is the one row with a bitrate ceiling, and a constant-quality encode
	// sends no target for it to apply to.
...
// The statement that narrows the control carries the limit itself, since no axis holds it
// and a reader looking at a slider that stops early is owed the number it stops at.
...
// chromasDeclared is every pixel format any row lists, so the sweep above asks about
// formats a codec does not carry as well as the ones it does.
