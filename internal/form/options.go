...
// optionTileWatchTransports offers the legs a receive pipeline decodes from, which is
// the GStreamer roster. It reaches WHEP, which no player URL expresses, and not HLS,
...
// in the receive package's own order: the default first, then the device chains, then
// the unconverted one.
//
// A chain this machine cannot run keeps its entry and is greyed, like every other
// option here: the entry plus its reason names the element that is missing, where a
// list one item shorter would say nothing at all. Which of them that is is
// availability's answer and not this function's (availabilityOptionRules), for the
// reason every other builder here leaves its verdict alone.
//
// The default is marked recommended, which is a hint about this machine rather than a
// value: what a draft holds is Field.value, and the chain a stream renders through when
// nothing chose one is what the mark says.
