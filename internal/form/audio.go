...
//
// A kind is declared and what is inside it is enumerated, which is why the two are two controls:
// whether a machine serves desktop audio at all is a table's answer, and which microphone is
// plugged in or which application is running is not something a table can hold.
// The declaration is platform.AudioSources; this is the other half.
//
// The enumeration itself is the machine's answer and arrives in Deps, the way the monitors and the
// encoder probe do: a resolve is a pure function of the draft and what the machine last answered,
// so it may run on every keystroke and cannot pay for a probe.
// What is here is the reading of that answer, which is what an option list and a greying both ask.

// audioDevicesOf is what one kind offers on this machine, in the order the enumeration reported
// them, with the kind's own default leading.
//
// The default is an entry rather than an absence: it is what an entry naming no device takes,
// and offering it as the first entry is what lets a reader move back to it after picking something
// else.
// It is the one entry every served kind has, which is why a kind with nothing enumerated still
// offers something.
...
//
// The kind's own default always is, since it is what an entry naming no device takes and no
// enumeration has to report it.
...
// then every device the enumeration reported for it, then the selection itself where the
...
// and dropping the entry would lose a choice the user made without saying so.
// It carries the note that says which it is, so a reader sees the difference between a device that
// is here and one that was.
...
// The entry it is drawn for decides nothing about the list: which kinds exist is the platform's
// answer and the same for every entry, so this ignores the entry and takes the whole declared
// table.
