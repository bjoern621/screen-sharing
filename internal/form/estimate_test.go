...
// estimateTestTolerance is what two figures may differ by and still be the same prediction.
// The model is arithmetic over constants, so the only difference it allows for is the float
// representation of them.
const estimateTestTolerance = 1e-9

// The picture every prediction below is priced from: one 1080p output at index 0,
// running at the rate the streams target, so nothing here diags about the monitor while testing the
// bitrate.
...
// estimateTestStream sits exactly on the anchor: the software H.264 encoder,
// whose quantizer counts on the anchor's own 51-point scale, at the pixel format and the quantizer
// the anchor was measured on.
// Every other case below moves one term of it.
...
// 1920x1080 at 60 is 124,416,000 pixels a second.
// On the anchor - H.264 at 1.0, 4:2:0 at 1.0, the quantizer on the scale it was calibrated on -
// each of them costs the anchor's 0.07 bits, which is 8.70912 Mbit/s.
// Uncompressed, 4:2:0 spends 12 bits on the same pixel, which is 1492.992.
...
// Six quantizer points on the anchor scale halve the rate, and the coding efficiency and the chroma
// weight multiply it.
// HEVC at 0.6 and 4:4:4 at 1.5, six points softer than the anchor: 8.70912 * 0.5 * 0.6 * 1.5.
func TestTheCodecAndTheChromaPriceTheAnchorFigure(t *testing.T) {
	...
	// 4:4:4 carries the same 24 bits a pixel that RGB does, so the uncompressed rate is double the
	// anchor's regardless of what the encoder then does with it.
...
// libvpx counts to 63 and x265 to 51, and both codecs produce a format of the same coding
// efficiency, so the top of either scale is the same quality and has to predict the same rate.
...
// A mode that aims at a bitrate the user set is predicted at that target: the encoder holds it,
// and a quality-model figure beside the number in the field would be the form contradicting itself.
...
// A lossless encode has no quantizer to price, so it is priced from the raw rate:
// RGB at 24 bits a pixel is 2985.984 Mbit/s uncompressed, and the typical figure sits between the
// near-static end and the motion end of the lossless spread.
...
// The headroom is the whole point of the uplink field: a line under the prediction reads as a
// negative figure rather than as a clamped zero, since the size of the shortfall is what says
// whether a lower quality would fit.
...
// That is an environment condition and not a bug, so the prediction is withheld and the rest of the
// form still resolves.
...
// necessarily write.
// A value outside the tables is answered with no prediction rather than priced against a guessed
// weight, the publish path being the one that says why.
...
// Resolve runs on every keystroke, so a prediction that moved between two identical drafts would
// move the figure under a user who changed nothing.
