...
// Availability: which controls the current settings leave usable, and what the screen says about
// the ones they do not.
//
// This is the Go half of what the Wails frontend's util/deps.ts decided in TypeScript.
// Every verdict here reads a table - capabilities for the codec facts, transport for the carriage,
// gpupath for the frame-memory pairs, publish for the capture backends and their engines - and none
// of them restates a rule those tables already carry.
// The one thing this file adds is the statement, because a limit the user cannot read is a limit
// they cannot act on.
//
// A statement is a code and the identifiers it is about, never a sentence (statements.go).
// What that fact reads as on screen is the surface's, written where the column width and the tone
// are visible; what is true is decided here and nowhere else.
// The two halves meet on identifiers - a capture backend's name, an engine's,
// a pixel format's - which is what lets the domain gain a codec without a shell being edited.
//
// Two contracts govern everything below.
// A disabled control always says why, which form.go asserts on every field it renders,
// so every greying path here has to produce a statement.
// And where two facts block the same field, the reason names the one the user can act on
// (docs/field-availability.md, "Three facts decide a rate-control field"):
// B-frames under software x264 state that only some encoder families take a B-frame count,
// not the mode fact that would be a lie there.
//
// An unresolved fact withholds nothing.
// deps.ts wrote that as a null check per table; here the only facts that can be missing are the
// machine's - a capture backend this app has no publisher for names no engine,
// and a probe that never ran states no verdict - because the tables themselves are compiled in.
// Both cases leave the control live rather than greying it under a fact nobody stated.
...
//
// The key names a row of the availability table, and an unknown one is a caller that invented a
// control: returning a plain enabled state for it would put a widget on screen that no rule here
// governs and no repair below moves, which is exactly the silent fork the contract exists to
// remove.
// The key is the row's own, which for a repeated control is the template rather than one entry's
// key: the statement is about the control, and writing it per index would be writing it once per
// entry a reader happens to have made.
// Which entry it is being asked about travels beside it, for the rows that answer differently per
// entry.
...
//
// A field with no option-level rule leaves every entry enabled: the greying that applies to the
// whole control is fieldState's, and repeating it per entry would grey a dropdown twice over.
...
//
// It is a table rather than a switch for the reason every other fixed fact in this repository is
// (docs/development-principles.md): a control added to keys.go and left out here fails the lookup
// above instead of quietly rendering as a plain enabled widget, and the rows that decide nothing
// say so in one word rather than by falling through a default nobody reads.
var availabilityRules = map[string]func(availability) state{
	// The connection group.
	// The stream's name and the relay's address apply whatever else is selected;
	// each relay listener port is a knob of one protocol and is hidden while that protocol is not the
	// leg being configured, which is the hidden treatment's own case (docs/field-availability.md,
	...
	// HLS follows the watch leg rather than the publish one: the relay serves it and ingests nothing
	// over it, so nothing is ever published this way and the publish dropdown never names it.
	KeyHlsPort: func(av availability) state {
		return availabilityShownFor(av.s.Viewer.PlayerWatchTransport == availabilityHls)
	},

	// The dimensions whose greying is per entry rather than per control.
	// The dropdown keeps every value and greys the ones this combination rules out,
	// because the greyed entry plus its reason is what tells the user what to change.
	...
	// The pointer is greyed per entry and never as a control: every backend serves at least one mode,
	// so there is no combination where the question does not apply.
	KeyCursor: func(availability) state { return availabilityLive() },

	// The pixel format is the one control that carries a note about somebody else's machine:
	// what the choice costs a viewer to decode.
	// It is a note and never a greying, since every format has a software decoder,
	// so a format no GPU takes is a viewer spending cores rather than a limit here - a trade the
	// publisher is entitled to make once it is stated (docs/field-availability.md).
	//
	// It greys for one reason only: on the device path that converts nothing, the encoder picks the
	// layout itself, so the control would otherwise show a value the stream does not carry.
	...
	// The colour range is blocked by two facts, and the encoder-colour path outranks the format's own:
	// under it neither field reaches the stream, so naming RGB would send the user changing a control
	...
	// Three facts decide each one and the knob helper weighs them in the order
	// docs/field-availability.md states.
	...
	KeyMaxrateM: func(av availability) state {
		...
		// The va elements express a VBR target as a percentage of the ceiling and take 50% at the lowest,
		// so a ceiling more than twice the target has no form there and the GStreamer builder refuses it.
		// The knob is forwarded, so the field stays live and carries the bound instead of being greyed.
		if st.enabled {
	...
	// B-frames are blocked by two independent facts, so the reason names the one that applies instead
	// of always blaming the mode.
	// Which families take a count is the family table's, so the statement lists them from it rather
	// than naming one by hand.
	...
	// and a family that declares none offers nothing to spend.
	// Both the entries this control lists and the step a build spends come off that same row,
	// which is what keeps the greying and the encode in step.
	...
	// The tune is read the same way and from the same row, since it is the other half of one decision:
	// how hard the encoder works, and what it works towards.
	// A codec can declare either ladder without the other - the Vulkan rows tune and take no effort
	// step, the libvpx ones take a step and tune for nothing - so each control asks about its own
	// ladder rather than about the pair.
	...
	// It is greyed with the reason rather than hidden: the codec is a general concept,
	// and the greyed field plus its statement say why it does not apply here.
	...
	// is taken off, and greying it would leave a reader with a source they cannot remove.
	...
	// A kind with one device is a control with nothing to choose, so it greys and names the kind
	...
	// The level and the silence, which mean nothing until the entry names a source.
	// Both reach a pipeline that is already running, so neither is greyed for anything else.
	...
	// Under kmsgrab it stays rendered and greys where the run downloads nothing,
	// rather than vanishing a second time while the user changes codecs (docs/field-availability.md,
	...
	// geometry.
	// The backends that take no monitor index each say so, naming themselves, so the surface can state
	// what that one captures instead.
	...
	// The frame memory is never greyed as a whole: auto and the system copy are values every pair
	// satisfies, so no combination leaves a dead control.
	// What it carries is which import the direct path uses, and on the path that converts nothing,
	// what that costs.
	KeyCaptureMemory: func(av availability) state { return availabilityNoted(av.frameMemoryNote()) },

	// The per-protocol knobs, hidden with their protocol for the same reason the listener ports are.
	...
	// The watch-leg knobs follow the receivers that read them, and a knob two receivers read follows
	// either of them: the SRT window and the RTP lower transport are the link's rather than one
	// reader's, so a viewer configuring the tile leg is configuring the same link a player would open.
	...
	// time, so it follows the tile leg and not either.
	...
	// A chain this machine cannot run is greyed per entry with the element that is missing named
	// (availabilityOptionRules below).
	KeyRenderChain: func(availability) state { return availabilityLive() },

	// The output resolution is live.
	// What can refuse a value is one entry rather than the control: a frame path with nothing on it
	// that resizes greys the scaled entries and leaves the source size, so the reader is told which
	// pair to change instead of finding a dead field (availabilityOptionRules below).
	KeyOutputResolution: func(availability) state { return availabilityLive() },
}

// availabilityOptionRules is the option half of the same table: one row per field whose entries are
// greyed individually, each answering why this combination rules a value out.
...
var availabilityOptionRules = map[string]func(availability, string) *screensharev1.Text{
	...
	// The two watch legs read the same carriage table under different receivers:
	// an external player opens a URL through libavformat, and a tile decodes through a GStreamer
	// pipeline, so the two reach different protocol sets.
...
// The four ways a state can read, one constructor each, so a rule above states which of the four
// treatments it is applying rather than assembling a struct that could silently be a fifth.
...
// availabilityNoted is a control that stays editable and means something its label does not
// describe here.
// No note is a plain live control, which is what keeps a caller from branching on whether it has
// one.
func availabilityNoted(note *screensharev1.Text) state {
	return state{visible: true, enabled: true, note: note}
}

// availabilityDisabled is a control the combination blocks, greyed with the statement shown in its
// place.
...
// It stays enabled because nothing draws it: a hidden control carries no reason,
// and the disabled-says-why contract would otherwise demand a statement for a widget that is never
// on screen.
func availabilityHidden() state {
	return state{visible: false, enabled: true}
}

// availabilityShownFor is the hidden treatment stated as a condition, for the knobs that belong to
// one protocol and appear with it.
...
// availability is what one resolve reads beyond the fixed tables: the machine, the draft,
// and the three facts every rule above derives from.
//
// The three are derived once here rather than per rule because each is a lookup with a failure mode
// of its own, and a rule that repeated one would be a second place the same "not resolved" case is
// decided.
type availability struct {
	deps Deps
	s    settings.Settings

	// verdicts is what the rule system said about this draft, evaluated once per resolve and read
	// through by whichever control asks.
	// Every fact the domain tables state arrives here rather than being looked up per site,
	// so a control's greying and the publish's own refusal are two readings of one evaluation
	// (internal/rules).
	verdicts rules.Verdicts

	// engine is the publish engine the selected capture backend runs.
	// It is empty exactly when the settings name a backend this app has no publisher for,
	// which a hand-edited settings file can do; every rule keyed by engine then withholds nothing,
	// since a fact nobody stated is not a limit.
	engine string

	// codec is the capability row of the selected codec, and knownCodec whether the table has one.
	// A codec outside the table has no family, no format and no gaps, so the rules that read them
	// state nothing rather than guessing a half.
	codec      capabilities.Codec
	knownCodec bool

	// path is the frame-memory row of the selected capture backend and the codec's family on that
	// engine, and onDevicePath whether the pair has one.
	// It is a pair rather than a property of either end: the portal capture shares memory with a VAAPI
	// encoder and not with an x264 one.
	path         gpupath.Path
	onDevicePath bool

	// entry is which entry of the audio source list the question is about, and noEntry for a control
	// that belongs to no list.
	// It rides here rather than as a second argument to every rule, because a rule that does not
	// repeat has no use for it and a table with two signatures would be two tables.
	entry int
	// source is that entry, which for the row past the end of the list is the default entry: no kind,
	// unity gain, unmuted.
	// It is what the row a reader grows the list by holds, so a rule reading it needs no branch for
	// the row that is not stored yet.
...
// A codec outside the table has no family, and the zero row takes neither of the two fields,
// so both grey with the statement naming who does take them.
...
// The greying rules.
// Each returns the statement shown in place of the value it withholds, and nil where the value
// reaches the encoder.
...
// the catalog shows the same list and has to give the same answer, and a second copy of the gate
// here would be the drift docs/ipc-api.md exists to end.
//
...
// so the entry stays selectable, where the choice is the user's and the failure is theirs to read;
// what it needs granted rides on the option as a note.
...
// transportReason states why the selected capture backend's engine cannot carry a publish
// transport.
...
//
// Four facts withhold one and they are weighed in the order of what the user can act on.
// A probe that could not run the encoder outranks every table fact, since "no NVIDIA encoder on
// this machine" is the message that stops a search for the right transport.
// Below it sit the engine's own gap, the roadmap, and last the publish leg's carriage,
// which is the one a different transport fixes.
//
// A leg this engine cannot serialize at all states nothing about any codec.
// That is the transport's own refusal, already on the transport control, and greying every codec
// under it would name the wrong control.
func (av availability) codecReason(codec string) *screensharev1.Text {
	...
	// so its statement covers every codec rather than one at a time.
	...
	// The entry is refused where no encoder for it exists on this engine.
	// The rule binds on the engine alone and names the codec it takes, which is what lets one
	// evaluation answer for every entry of the dropdown rather than only for the selected one.
...
// either because its plugin is not installed or, for the hardware families,
// because the plugin found no device to register it for.
// Which of the two a verdict is follows the family and not the engine, so the statement carries
// both and the surface says what that combination means.
//
// An engine with no verdicts at all is an unprobed machine and not one with nothing usable,
// so it greys nothing.
// That is where this parts from the frontend it replaces: a null probe was a distinguishable state
// in TypeScript, and a zero Availability is the same state here.
func (av availability) probeReason(c capabilities.Codec) *screensharev1.Text {
	...
	if !ok {
		// Which half is missing is the family's fact and this codec's row names a family no table here
		// carries.
		// The probe's verdict still holds, so the codec is greyed under what is known rather than under a
		// guessed half.
...
// VP9 with it.
// A reason naming the protocol by itself would send a user hunting for another transport where
// another capture backend is the fix, so both ways out are named where the tables hold them - and
// each is an argument the surface may or may not receive, since the tables declare one for some
// rows and neither for others.
...
// naming that engine.
// The gap's code is passed through, since it already says which library or element is the limit.
...
// The gaps come from the capability table, which keys them by publish engine where only one builder
// reaches the mode: libvpx codes lossless VP9 and the vp9enc element has no such property.
...
// The capability table declares it per codec and engine: an encoder that signals no colour
...
//
// It reads the rules and nothing else, because the whole fact is theirs: what a backend does with
// the pointer is a per-backend table (internal/publish/cursor.go) written as rules rather than
// converted into them, and the one limit that is this app's rather than any backend's - that
// nothing carries a pointer position to a viewer yet - is a rule beside them.
...
//
// The verdict and its statement are the platform table's, read through rather than restated:
// which sources a machine offers is that table's whole question, and a second copy of the answer
// here is what would let the form grey a source the catalog offered or offer one the publish then
// refuses to open (docs/domain-model.md, "The second-track capture sources").
...
// The first is the audio table's own gap, the second the carriage table's,
// and the statement names whichever applies, since the fix differs - another capture backend
// against another codec on the same one.
func (av availability) audioCodecReason(name string) *screensharev1.Text {
	...
	if _, codes := a.EncoderOn(av.engine); !codes {
		...
		// Where it does not, the fact is still shown, without a cause invented for it.
	...
	// which is the list a refusal points at.
	// It is empty on a leg that carries none, and the statement then reads as the shorter fact rather
	// than trailing off.
...
// frameMemoryReason states why this capture backend and codec cannot publish through a frame
// memory.
//
// Two facts of the pair decide it, and both come off the pair table: whether the pair has a device
// path at all, and what that path does to the colour.
// With no row both device values are greyed under one statement, since neither has a way onto the
// device to demand.
// Where the row converts on the device, the value that pays for the path with the colour is greyed,
// having nothing to trade for it; where the row converts nothing, the value that demands the colour
// too is greyed and carries the cost it would have paid, leaving the value that accepts that cost
// as the live one.
//
// Auto and the system copy are never greyed.
// Auto answers with whichever path costs the user nothing and system memory is the path every pair
// has, so no combination leaves a dead control.
//
// Every statement names both ends, because neither decides on its own: the same portal capture
// shares memory with a VAAPI encoder and not with an x264 one, and switching either side is a way
// to reach the path.
...
//
// One case has it, and it is a fact about where the frames are rather than about the size.
// On a device path the frames never come back to system memory, so the only thing that can resize
// them is the filter already on that path - and a pair whose device path carries no conversion at
// all has none: the encoder reads the captured surfaces directly, which is exactly what makes it a
// colour-trading row.
// The ffmpeg builder refuses such a run, so the entry is greyed with the same fact rather than
// offered into a refusal.
//
// Having such a row is not enough: the run has to be taking it.
// The statement offers the system copy as the way across, so the same fact on a run that already
// downloads every frame would name a fix the user has already applied, and would grey a scale the
// CPU filter on that path can perform.
// staysOnDevice is what says which of the two a run is doing, and it is the same question
// gpupath.Resolve answers before either chain is built - auto included, which takes the round trip
// on exactly the rows this greys for.
//
// The source size is never greyed.
// It is what every pair does, and a control whose every entry is out is a control that teaches
// nothing.
...
//
// A format no implemented codec produces narrows nothing, matching the roster the transport table
// answers with: hiding a choice on absent information would hide one that would have worked.
...
// renderChainReason says why this machine cannot render through a chain, and nil for one it can.
//
// It is the one availability rule that asks the machine rather than a table:
...
// The receive package is what asks, and this turns its answer into the statement a surface shows -
// the chain and the first element it needs and does not have.
//
...
// resolve; refusing it here would grey an entry no list offers.
...
// which is set to one leg, or a player, which can be opened on any leg this machine has one for.
//
// It exists because two of the watch knobs belong to the link rather than to a reader:
// the SRT retransmit window and the RTP lower transport are what the connection is negotiated with,
// and a viewer that decodes in a tile negotiates the same connection a player would open.
// Hiding them behind one of the two legs would leave a viewer that only uses the other unable to
// reach a knob its own leg reads.
//
// The two receivers are asked different questions, and that is the difference between what each of
// their settings decides.
// A tile receives over the leg TileWatchTransport names and over no other,
// so that setting is the whole answer for it.
// A player is opened per press, on whichever leg the reader picked from the ones this machine can
// open, so PlayerWatchTransport decides nothing about which players run - it is the leg a surface
// offers first.
// Asking it here therefore hid knobs that were in force: a player opened over RTSP reads
// RtspWatchProtocol whatever that setting says, and with both legs on SRT the control it reads was
// not on the screen at all.
//
// A hidden control is the wrong treatment for a value that still reaches a pipeline.
// The choice this page states is between hiding a knob and greying it with a reason,
// and both are answers about a knob that does nothing here; one that does something is shown
...
// uses carries the first two, already weighed by the caller, with reason the statement for them;
// the engine's own rule is read here.
//
// An engine that forwards a knob the mode marks unused leaves a note instead of a greying,
// so the field states what the value does there rather than feeding the encoder a number the form
// never showed.
// An engine that drops the knob in every mode outranks the mode's own reason:
// no rate control brings the control back, so naming the mode would send the user hunting for a
...
// and false where the builder treats the knob exactly as the mode table says.
...
func (av availability) engineRule(key string) (availabilityEngineRule, bool) {
	for _, r := range availabilityEngineRules {
		...
		// one naming either covers the union of the two.
...
// It is a note rather than a greying because the knob is forwarded: the field stays live and
// carries the bound.
...
// Auto is the value that reads the colour verdict as well: it takes a device path only where that
// path costs nothing, so a pair whose only path leaves the conversion to the encoder downloads
// under auto.
...
// A path that converts nothing carries its cost beside the import, so the field that chose the
// trade states it too instead of leaving it to the two colour fields it overrides.
...
// signals what it chose, so both fields grey with the row's cost and Repair moves each onto what
// the encoder signals: a greyed field showing something other than what the run produces is the one
...
// decodeNote states what decoding this stream costs a viewer, nil while the codec is one the table
// does not carry.
//
// It is a note and never a block: every format has a software decoder, so the choice is between a
// viewer's GPU and a viewer's cores.
// Where some hardware decodes the pair the statement names those decode families,
// since which ones they are is the whole point of the choice.
// Where none does, it names the software element and one family whose limit stands for the rest:
// the other families are out for reasons of their own, and listing four of them states four times
// what the first already shows.
...
// The tables the rules read.
// Every one of them states a fact the Go domain packages do not carry: which capture backends take
// no monitor, what a family's encoders take, and how each engine's builder departs from the mode
// table.
...
// them.
// A name here the registry does not carry would gate a control on a protocol nothing publishes,
// which is what TestEveryGatedTransportIsRegistered holds.
const (
	availabilitySrt    = "srt"
...
// What each of them captures instead follows from the backend itself, so the statement names the
// backend and the surface says what that one grabs - which is the same sentence its own entry in
// the capture dropdown already has to write.
var availabilityMonitorless = []string{availabilityKmsgrab, "gdigrab", "portal"}

// availabilityFullRangeChromas are the pixel formats that carry no quantization range choice at
// all.
// RGB is full range by construction, so there is nothing for the colour range control to decide
// under it.
var availabilityFullRangeChromas = []string{"gbrp"}

// availabilityMode is what one rate-control concept needs from the encoder.
// It is the first of the three facts that decide a rate-control field, and it says which controls
...
type availabilityMode struct {
	...
	usesVbv bool
	// usesBframes: B-frames help the lossy bitrate and quality modes, and only where the family takes
	// a count.
	usesBframes bool
}

// availabilityModes is what each rate-control mode needs, keyed as capabilities names the modes.
// A mode outside the table needs nothing, which is the answer for a hand-edited settings file
// naming one no builder implements.
...
// It is the second of the three facts a rate-control field is decided by.
type availabilityFamily struct {
	// takesBframes: a field the family has no property for greys whatever the rate-control mode,
	// and the builders pin it off rather than forwarding a value the encoder would ignore.
	// The effort step is not such a field: it follows the codec's own ladder, since one family's
	// codecs can declare different ones.
	takesBframes bool
...
// Every family has a row, which TestEveryEncoderFamilyStatesWhatItsEncodersTake holds:
// a family missing here reaches no verdict of its own and would be greyed under the engine's name
// instead.
...
// availabilityFamiliesWith lists the families whose row sets a flag, for a statement that names who
// takes a field instead of stating one family by hand.
// A control greyed for every other family says which ones own it, and the list follows the table
// when a family gains the field.
//
// The order is capabilities.Families' own, so the statement is the same on every call rather than
// the map's iteration order of the moment.
...
// availabilityEngineRule is one place where a publish engine's encoder builder departs from the
// mode table: a knob the mode uses that the engine ignores, or one the engine forwards in a mode
// the table marks unused.
// It is the third of the three facts.
//
// The mode table says which knobs a rate-control concept needs.
// Whether the value reaches the encoder also depends on which engine builds the command,
// because the two express the same modes through different properties: the GStreamer nvcodec
// elements take no effort step, x264enc cannot raise a ceiling above its bitrate,
// and vpxenc has no unbounded constant-quality mode.
//
// A knob an encoder library has no form of at all is a rule with no engine,
// since both builders hit the same wall.
// The rules mirror the two builders, encoderArgs in ffmpeg/encoders.go and gstEncoder in
// publish/gstencoders.go.
type availabilityEngineRule struct {
	// engine is empty for a rule that describes the library rather than a builder,
	// and then holds on both.
	engine string
	// knob is the field key the rule governs, so a rule and the control it greys are the same
	// identifier.
	knob string
	...
	modes []string
	// forwards is true where the builder sends the value even though the mode table marks the knob
	// unused, and false where it ignores a value the mode would use.
	forwards bool
	// reason names why an ignored value never reaches the encoder, or what a forwarded one does there.
	// Both are shown to the user, as a code and not a sentence: what each row says is a fact about one
	// named element, so each carries its own.
...
// and fails at load.
//
// A row naming no knob governs nothing and would sit in the table doing nothing visible, and one
// carrying no reason would grey a control or annotate it with a sentence that has no code behind it,
// which is the hole in a refusal docs/field-availability.md exists to prevent.
func init() {
...
var availabilityEngineRules = []availabilityEngineRule{
	// No rule withholds the effort step on either engine.
	// Every element that codes a laddered codec takes its steps through a property of its own -
	// speed-preset, cpu-used, preset - and the nvcodec elements take the same p1-p7 steps ffmpeg does,
	// their own enum carrying them beside the presets it deprecates.
	//
	// An encoder that cannot bound the burst at all has no VBR mode, which the capability table
	// declares as a mode gap.
	// No rule here withholds the ceiling field for that case: the mode carrying it is gone,
	// so a rule would grey a field under a mode that cannot be selected.
	{
		knob:   KeyVbvMs,
...
// Planar RGB gets its own code: RGB reaches an encoder only through HEVC's Range Extensions or
// VP9's identity matrix, which is a coding-tool fact rather than a subsampling limit,
...
// availabilityExactColourReach states how an encoder family is reached on the device with the
// colour the form shows, nil where the pair table declares no such row.
//
// It is the way out the greyed direct value carries, and the one the dropdown cannot show by
// itself: the fix is another capture backend rather than another value of this field.
// The statement is assembled from the row, so it names whichever pair the table declares instead of
// a platform written in here.
...
//
// Nothing is refused today: what is inside a kind is what the machine answered,
// and a selection it no longer answers for is kept with a note rather than greyed - an application
// that is not running now is one that may be running when the stream starts (audio.go).
// The row exists because the control offers entries and every such control is answered for here,
