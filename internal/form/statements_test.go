...
//
// They ask the way a surface asks: which statement is this, and which identifier is it
// about. That is deliberate - a test that matched on wording would be asserting on the
// one thing this package no longer decides, and would start failing the first time a
// shell reworded its screen (api/proto/screenshare/v1/text.proto).
...
// idOf is one identifier argument, and the empty string where the statement carries
// none under that name. Absence is a real answer: several statements name a value some
// combinations have and others do not, and a caller with none is stating the shorter
