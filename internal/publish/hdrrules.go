...
// is recognised at all and how the encoder input is narrowed to the colour it actually has
// (gstrun/surface.go).
// The ffmpeg engine has no such report - a running ffmpeg tells its caller what it is encoding and
// never what it read - and it tags every encode BT.709 through a setparams filter,
// because a partial colour description is no description (ffmpeg/args.go, colourFilter).
//
// So an HDR desktop captured through an ffmpeg backend is published as a standard-range stream
// carrying HDR samples: every viewer trusts the tag, and the picture is wrong on all of them.
// Nothing can detect it there, which is exactly why it is stated rather than refused - a refusal
// needs a fact, and the fact is unavailable on that engine.
//
// The statement lands on the one control a reader reaches HDR through.
// Ten bits per component is the only format an HDR surface can ride in, so the note appears where
// somebody is reaching for it and nowhere else; a note on the whole engine would follow every
...
func engineColourRules() []rules.Rule {
	return []rules.Rule{{
		...
		// It is an argument rather than an axis reading because the axis carries the engine that is
		// selected, and what a reader needs is the other one.
