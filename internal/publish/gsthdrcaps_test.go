...
// What colours the encoder input accepts is the pixel format's answer, because an HDR surface has
// exactly one way through: the only format this app encodes that carries more than eight bits per
// component.
//
// The rows are what the child narrows to the surface's own colour (gstrun/surface.go).
// What is asserted here is the other half: that a publish states the rows a surface could be,
// and states no row an 8-bit encode could not honestly carry.
...
func TestTheEncoderInputOffersTheHdrColoursOnTheTenBitFormatAlone(t *testing.T) {
	...
	// Standard range leads, because it is what a capture stating no transfer at all is and what a run
	// nobody narrows has to end on.
	...
	// An 8-bit encode of an HDR surface is refused on the capture's own report,
	// so offering the rows here would be offering a negotiation that ends in a stream tagged PQ over
	// eight-bit samples.
...
// Every colour the encoder input accepts carries the configured range.
// The range is what decides how a viewer expands the picture, and a row that dropped it would leave
// one of the negotiable outcomes signalling something the settings never asked for.
...
// The rows reach the capsfilter as structures rather than as one value list,
// which is the difference between a choice and a conversion: videoconvert fixates a list to its
// first entry whatever the frames carry, so a list would convert every HDR surface into the
