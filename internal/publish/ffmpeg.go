...
// ffmpegEngine runs a screen grabber whose frames feed one ffmpeg process that captures, encodes,
// muxes and ships.
// It covers the ddagrab, gdigrab, x11grab and kmsgrab backends, which differ only in ffmpeg input
// arguments.
...
//
// The builder is pure over the settings: it names no operating system, which is what lets a Windows
...
// backend runs on, and that column is captureNeeds', in this package.
// So the check is made where both facts are reachable and the builder below stays a builder - the
// same split capabilities.Validate and transport.ValidatePublish already sit on.
//
...
// it carries a port the kernel handed out for this run, so a rendered command that showed it would
// be a different string every time it was rendered - and whether two settings build one pipeline is
// decided by comparing exactly that string (SamePipeline).
func buildArgs(s settings.Settings, preview PreviewLeg) ([]string, error) {
	...
	// The backend has already read the same table to decide whether to bring a receiver up at all,
	// so this branch is the one that survives the two disagreeing.
...
func (ffmpegEngine) Command(s settings.Settings) (string, error) {
	...
	// Name the binary the run will actually spawn: kmsgrab uses the capability wrapper (see
	// ffmpeg.FindCaptureExe), everything else plain ffmpeg.
	exe := "ffmpeg"
