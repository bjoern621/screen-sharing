...
// One publish engine drives the screen grabbers that feed a single ffmpeg process;
// the other drives the portal capture backend, where GStreamer captures, encodes and ships in one
// graph.
...
// It reuses the ffmpeg progress shape as the wire format; an engine without an equivalent progress
// stream measures the same figures its own way (see gstMeter), and a figure its pipeline exposes no
// measurement for is marked in Missing rather than sent as a zero, which is the reading that marks
// a stalled encoder.
...
// OnExit fires exactly once, with a non-nil error only on unexpected failure, the tail of stderr,
// and the path of the run log.
type Callbacks struct {
	...
	// It fires at the reader's own rate, which is faster than the frame rate on purpose,
	// and never at all on any other mode or any engine whose child cannot report one
	// (internal/pointer).
...
type Publisher interface {
	...
	// and its zero value is a run with none - which is what a rendered command always is,
	// since the port belongs to one launch (preview.go).
	...
	// The lifecycle code above the seam never reads it; the settings form does,
	// because which rate-control knobs reach the encoder differs per engine, and a control the engine
	// ignores is greyed with that reason instead of silently doing nothing.
	Engine() string
	// Carries reports whether this engine can drive the named transport, i.e. the transport implements
	// the publish capability the engine serializes through.
	// The ffmpeg engine needs an FFmpegPublisher, the GStreamer engine a GstPublisher,
	// so the portal capture backend cannot carry a transport with no GStreamer sink.
...
// so the spelling is the capability table's own rather than a second one that could drift from the
// engine a Gap names.
const (
...
// Capture backend and publish engine are two axes, not one: a screen source is a row here,
// and which engine the row names follows from which framework has an element or an input device for
// that source, not from a property of the engine.
//
// A screen both frameworks can read is two rows, one per engine, and each row is named as its own
// framework names the source: the macOS screen is avfoundation under ffmpeg and avfvideosrc under
// GStreamer, the Windows desktop ddagrab or gdigrab under ffmpeg and d3d11screencapturesrc under
// GStreamer.
//
...
// GStreamer has no capture element for DRM/KMS scanout buffers at all, so kmsgrab is ffmpeg's.
...
// The engine that owns the selected capture backend renders it, so the result is an ffmpeg command
...
// SamePipeline reports whether two settings publish through the same pipeline,
// which is what says whether moving from one to the other needs the child process relaunched.
//
// The rendered command is the whole of what an engine hands its child, and both engines render it
// from the settings alone.
// So a field no builder reads cannot change the string, and one a builder reads always does.
// Comparing the render is therefore the question itself, where a table of which fields matter would
// be a second statement of it, and would fall behind the builders the first time one of them reads
// a field the table does not name.
//
// Settings neither engine can render carry the reason instead of an answer:
// they name a pipeline that cannot run, so whether some other pipeline equals it is not a question
// with one.
...
// Captures lists the capture backends the app can run, sorted for a stable order across the wire.
...
// Each of them answers capabilities.Validate under its own name and is probed separately for which
// codecs it can run here, since the two wrap different encoder implementations.
func Engines() []string {
...
// in transport registry order.
// The result is the subset of transport.Names() the engine can serialize through,
// so a capture whose engine lacks a sink for a transport excludes it.
