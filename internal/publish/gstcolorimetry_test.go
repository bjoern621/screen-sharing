...
// roundTripFrames is how many frames one round trip encodes.
// The colour description travels in the first parameter set, so a single frame would carry it;
// five keep an encoder that emits nothing until its lookahead fills from writing an empty file.
const roundTripFrames = 5

// roundTripWidth and roundTripHeight are the picture one round trip codes,
// and roundTripPicture below the caps fields stating it, which the caps on both ends of the
...
// gstUnsignalledColorimetry is what a viewer reads off an HD stream that carries no colour
// description: limited-range BT.709, the default it picks off the picture size.
// A stream that signals nothing is watched as this, whatever it holds, so it is the value such a
...
// The file is what makes the round trip a measurement, since it severs caps negotiation:
// the colorimetry the decoder reports can then only have come from the bitstream.
//
// Framing is the whole of what the write side pins, never a colour description.
...
// The two VP formats travel in IVF: 32 bytes of fourcc, picture size, frame rate and frame count,
// with no colour field of any kind and byte-identical between a full-range and a limited-range
// encode.
//
// IVF rather than a parser for both, and for VP8 there is no choice at all:
// the format has no parser element to scan with.
// VP9 has vp9parse, and a stream it framed decodes only while the frames stay small - past roughly
// half a megabit at this picture size, vp9dec takes every buffer it hands over and reports no valid
// frame at end of stream, where the same encode read back through IVF decodes whole.
// Every rate a screen share publishes at is on the far side of that line, so the parser would
// measure the colour of exactly the streams nobody sends.
...
// The publish side pins four enum values and a decoder reports the same colour space by name,
// so limited-range BT.709 goes in as 2:3:5:1 and comes back as bt709, which is what the grid's
// stats card shows.
// A pinned colorimetry with no name here is expected back verbatim, which is what full-range BT.709
// does.
var gstColorimetryNames = map[string]string{
	gstRangeLimited + ":" + gstBt709: "bt709",
}

// gstSinkPadCaps matches the line gst-launch -v prints for the caps that reached the sink,
// and gstColorimetryField the one field of it this reads.
var (
...
// and where it cannot the table gaps it rather than encoding a stream that says one thing and holds
// another.
// This measures both, one hop further out than the failure a partially named colorimetry produces
// inside the pipeline (TestCaptureCapsNameEveryColorimetryComponent).
//
// So this encodes through the engine's own encoder-input caps and encoder,
// hands a decoder the bitstream and its framing alone (gstFraming), and reads the colorimetry off
// the decoded caps: the same pad the grid's stats card reads its colorimetry from,
...
// the system one would assert a claim about neither.
func TestPublishedColorimetryReachesTheDecoder(t *testing.T) {
	...
	for codec := range gstCodecs {
		...
		for _, chain := range gstRoundTripChains(t, cap.Family) {
			// The element is asked for per chain, because a family whose plugin ships one per memory kind is
			// encoded by a different one on each: asked for the other, the skip below would read as a
			// missing plugin where the run would have launched an element this install does carry,
			// or the reverse.
			...
			for _, colorRange := range []string{"pc", "tv"} {
				t.Run(codec+"/"+chain.memory+"/"+colorRange, func(t *testing.T) {
					...
					// pipeline is built, so the refusal is what this asserts for it.
					...
					if !signalled {
						// The stream states nothing, so what it is watched as is the viewer's own default.
						// That is a match only where the publish happens to be in it, which is the whole of what the
						// colour-range gaps are about.
...
// the two ends of the 8-bit range, and the studio ceiling 20 code values under it.
var whitePatchLuma = map[string]int{"pc": 255, "tv": 235}

// whitePatchTolerance is how far off a decoded patch may land.
// The two ranges are 20 code values apart and a lossy encode of a flat field can round one off it,
// so one value of slack cannot make either range read as the other.
const whitePatchTolerance = 1

// A device path claiming exact colour has to convert the frames and not only describe them.
// The signalled colorimetry says what a viewer will expand the picture as; this says what is in it,
// which is the half a label cannot earn on its own: an element that ignored the range and tagged it
// anyway signals bt709 full range over limited-range pixels, and every viewer then stretches a 235
// white to 255 and clips everything the picture had.
//
// So this codes a white patch through the pair's own conversion and encoder,
// decodes it and reads the luma plane.
// The patch enters as RGB, which is what every capture backend here hands the pipeline: fed YUV,
// videotestsrc writes the studio values itself and the conversion under test becomes a passthrough
		...
		for _, c := range capabilities.Codecs {
			...
			for _, colorRange := range []string{"pc", "tv"} {
				...
				t.Run(c.Name+"/"+chain.memory+"/"+colorRange, func(t *testing.T) {
					...
					// What this measures is the range, which is the same conversion at every depth.
...
// Both start with the full plane, so the pixel is at the same offset in either,
// and offering both keeps the read a memory download: a videoconvert asked for one layout converts
// the range along with it, which is exactly the value under test.
const (
	whitePatchChroma  = "yuv420p"
	whitePatchFormats = "video/x-raw,format={ NV12, I420 }"
	// whitePatchPattern is videotestsrc's flat white field: every channel at its maximum,
	// which is the one input whose stored luma is the range and nothing else.
	whitePatchPattern = "white"
)

// roundTripPattern is what the colorimetry round trip codes: videotestsrc's colour bars,
// its own default.
// What the frames hold does not decide what the stream says it holds, so the pattern is named once
// here rather than chosen per codec.
const roundTripPattern = "smpte"

// gstDecodedLuma decodes one stream and returns the luma of the centre pixel of its last frame.
// The centre because a patch is uniform and its border is where a scaler or a cropping decoder
// would show; the last frame because the first is the one an encoder codes alone,
// and a range applied to it and dropped afterwards is a failure this must not miss.
...
// roundTripChain is one chain a codec is published through: the capture backend the pair table
// names, the memory setting demanded of it, and what that setting resolves to.
...
//
// Only the exact-colour rows are taken.
// A row that lets the encoder convert states the colour it produces in Signalled and is measured
// against that instead of against the settings, which is the ffmpeg engine's round trip;
// this engine carries no such row.
...
// gstRoundTripEncode encodes generated frames of one videotestsrc pattern through the whole chain
// these settings publish through — the family's upload where its converter takes no system
// frames, the conversion, the encoder-input caps, the encoder and its link — and returns the file
// holding the elementary stream.
//
// The source is pinned to packed RGB (gstProbeSource), which is what every capture backend here
// produces and what leaves the conversion the one element deciding the layout and the colour.
func gstRoundTripEncode(t *testing.T, s settings.Settings, pattern string) string {
	t.Helper()
	...
	encoder, link, err := gstEncoder(s, s.Publish.Fps*2, mem.memory)
	if err != nil {
		t.Fatal(err)
	}
	c, ok := capabilities.Get(s.Publish.Codec)
	...
	// Forward slashes, because this path is interpolated into a gst-launch line and that parser reads
	// a backslash as an escape: a Windows temp path reaches filesink with its separators eaten,
	// and the round trip writes into the working directory instead of the temp one.
...
// narrowest chroma this engine reaches for it, in a rate-control mode its element implements,
// over one of the chains the pair table leaves it reachable through.
//
// The transport is RTSP because it carries every format the codec table holds,
// so the transport check gstInputCaps runs never decides which codecs this covers.
func roundTripSettings(t *testing.T, codec, colorRange string, chain roundTripChain) settings.Settings {
	t.Helper()
	...
	// and not the defaults' capture backend: the memory setting decides the caps feature,
	// the conversion and, on a family with one element per memory kind, the encoder.
	// The generated frames start in system memory either way, and the family's upload is what puts
	// them where a captured one already is.
	...
	// One encoder refuses a target above its ceiling outright, and the default settings carry one
	// above it.
	if limit := c.BitrateLimitOn(EngineGst); limit > 0 && s.Publish.BitrateM > limit {
		s.Publish.BitrateM = limit
	}
	return s
}

// gstChain joins pipeline fragments with the link operator and drops the empty ones.
// A codec whose encoder needs no parser and a format whose file needs no framing element each
// contribute nothing between their neighbours.
...
// runGst runs one pipeline to its end and returns everything it printed.
// A pipeline that produces nothing is a failure like any other, so the timeout reports as one
...
// decodedColorimetry reads the colorimetry off the caps that reached the sink,
// and reports false where the decoded frames carry no such field: a decoder that read no colour
// description states none rather than a default.
