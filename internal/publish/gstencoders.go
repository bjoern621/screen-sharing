...
// gstRates are the rate-control figures a stream's settings yield, rendered as the property values
// the elements take: kbit per second for the x26x elements, the constant-quality target,
// and the GOP length in frames.
...
// The elements that link an encoder to the sink.
// config-interval=-1 makes the parser insert SPS/PPS (H.264) or VPS/SPS/PPS (H.265) ahead of every
// IDR frame.
// Without it the parameter sets travel once at stream start, so a viewer that joins the relay
// mid-stream never receives them and its decoder cannot start.
// The ffmpeg publish engine repeats them by default, which is why only this engine needs the
// property.
// VP9 and AV1 carry no out-of-band parameter sets, so their parsers need none;
// rtspclientsink payloads the parsed video/x-vp9 and video/x-av1.
//
// VP8 is the one format with no parser element.
// vp8enc leaves the profile field on its output caps a list and then rejects its own unfixed caps
// with "Invalid vpx profile", so a capsfilter pins profile 0, which is the only profile an 8-bit
...
// gstCodec is one codec's GStreamer half: the element that encodes it, with the rate-control mode
// mapped onto that element's own knobs, and the elements linking it to the sink.
type gstCodec struct {
	...
	link   []string
	// limits refuses a settings combination this element cannot express, beyond what the capability
	// table and the family's own limits already cover.
	// It is per codec rather than per family where the bound is the element's property range,
	// since two elements of one plugin can declare the same property differently.
	limits func(settings.Settings) error
}

// gstCodecs is the GStreamer engine's half of the codec facts declared once in capabilities.Codecs,
// and the counterpart to encoderArgs in the ffmpeg builder.
// A rate-control mode or a pixel format an element has no form of is declared as a Gap on that
// codec's row, carrying Engine "gstreamer" where the ffmpeg encoder reaches it,
// and rejected before a pipeline is built, so no branch below approximates one.
//
// The VAAPI rows target the stateless "va" plugin (gst-plugins-bad), not the older "vaapi" one
// (gstreamer-vaapi: vaapih264enc, vaapih265enc).
// The va plugin is the maintained one, it is the only one with an AV1 encoder,
// and it negotiates the VAMemory caps that make the portal backend's GPU path possible:
// vapostproc imports the compositor's dmabuf and converts into surfaces these elements read
// (gpupath.Paths).
// Its elements register per detected device, so an element name below exists only where the driver
// exposes that encode entrypoint, which is the same condition the codec's probe tests
// (encoders.Detect).
//
// The QSV rows target the "qsv" plugin, which drives Intel's oneVPL runtime over VA on Linux and
// D3D11 on Windows.
// Its elements register per detected device as the va ones do, so an element name below exists only
// where an Intel GPU with that encode capability is present, which is the same condition the
// codec's probe tests.
//
// Two kinds of family have no entry, both rejected before a pipeline is built:
// the ones still declared Implemented:false in capabilities.Codecs (v4l2, rkmpp),
// and the ones that row gaps off this engine (amf, vulkan).
var gstCodecs = map[string]gstCodec{
	...
	// No link element: VP8 has no parser, and unlike vp8enc the va element leaves nothing unfixed for
	// a capsfilter to pin, so rtspclientsink payloads its video/x-vp8 directly.
	...
	// The AV1 and VP9 elements state their bitrate in an unsigned 16-bit property where the H.26x ones
	// take any rate (qsvShortBitrate).
...
// GstEncoderElement returns the GStreamer element that encodes codec from frames in system memory,
// and false when this engine has no mapping for it.
//
// The element is what the encoder probe asks the plugin registry about: unlike the ffmpeg encoders,
// each of these lives in a plugin an install may not carry, and the hardware ones register per
// detected device.
//
// System memory is the path every pair has, which is why it is this form's answer and why a caller
// asking "can this machine encode that codec at all" needs no memory to ask it with.
// A caller holding a resolved memory asks GstEncoderElementOn instead: on a family whose plugin
// ships one element per memory kind the two differ, and a registry query for the wrong one reports
// the family present while the element a run would launch is missing.
func GstEncoderElement(codec string) (string, bool) {
	return GstEncoderElementOn(codec, gpupath.MemorySystem)
}

// GstEncoderElementOn returns the element that encodes codec from frames reaching it in the
// resolved memory, and false when this engine has no mapping for the codec.
//
// The system-memory name is read off a built encoder rather than stored beside the mapping,
// so it cannot drift from the element a pipeline runs; the device name comes from the family's
// device row, which is the same lookup gstEncoder builds with.
func GstEncoderElementOn(codec, memory string) (string, bool) {
	...
	s := settings.Defaults()
	s.Publish.Mode = mode
	// The steps this codec's row declares for that mode, which is what a draft naming this codec would
	// hold.
	// The defaults carry another codec's, and a mapping handed a step off its own ladder would spell a
	// property value no element takes - harmless for the element name read back below,
	// and a trap for the next caller that reads further.
...
// firstGstMode names a rate-control mode this codec's element has on this engine,
// and false when the table gaps every one of them off.
//
// Building an encoder is what reads the element name off the mapping, and a mapping takes the mode
// apart to decide which properties to set, so the mode it is handed has to be one the element
// implements: the settings' own default is lossless, which the hardware families have no form of,
// and asking for it would reach the exhaustive dispatch every mapping ends in.
...
// gstFamilyLimits are the settings bounds an encoder family's elements impose beyond what
// capabilities.Codecs declares, keyed as capabilities.Codecs names the family.
// A family absent here takes whatever the capability table already approved.
//
// The bound is per family rather than per codec because it comes from the properties a plugin's
// elements share, and it lives here rather than in the capability table because it binds two
// settings against each other (a ceiling against its target) instead of taking one value out of a
// set.
var gstFamilyLimits = map[string]func(settings.Settings) error{
	capabilities.FamilyVaapi: vaRateLimits,
}

// gstEncoder returns the encoder element (with its properties) and the elements that link it to the
// sink for the selected codec, reading frames in the resolved memory.
// A rate outside the element's property range is refused rather than moved into it.
func gstEncoder(s settings.Settings, gop int, memory string) (encoder []string, link []string, err error) {
	...
	// Both bounds apply where both exist: a family's limit comes from what its elements share and a
	// row's from one element's own property range, so neither stands in for the other.
	if gst.limits != nil {
	...
	// The element name is the first word every mapping writes, which is also how GstEncoderElement
	// reads it back out.
	// A family whose plugin ships one element per memory kind changes that word and nothing after it:
	// the two elements derive from one base class and take the same rate-control properties,
	// the memory deciding only which of them can link to the conversion ahead of it.
	if device, named := gstDeviceEncoderElement(c.Family, s.Publish.Codec, memory); named {
		encoder[0] = device
	}
	// The name goes on every encoder, whichever element it turned out to be, because that is how a
	// value reaches this element while the pipeline plays: the control socket addresses "enc" and the
	// codec decides what wears it (gstlive.go).
...
// speed-preset carries x264's named ladder, and the tune travels in whichever of the element's two
// tune properties declares it (x264TuneProperty).
...
//
// The element splits x264's own tune list across two properties: tune is a flagset of the three
// tunes that change what the encoder codes, and psy-tune an enum of the five that weigh what the
// eye sees.
// ffmpeg spells all eight through one -tune, so a step's property is this engine's alone while the
// step itself stays the encoder's own identifier.
...
// Both properties default to no tuning, so an untuned encode is what the element does with neither
// of them set.
...
func x265Encoder(s settings.Settings, r gstRates, l capabilities.Steps) []string {
	...
	case "cbr":
		// vbv-bufsize is in kbit: the bitrate held over the VBV window, one second when unset,
		// matching ffmpeg's bufsizeArg.
		bufKbit := r.kbps
		if s.Publish.VbvMs > 0 {
...
// x265TuneProperty is the tune step as x265enc's own property, and nothing where the ladder leaves
// the knob to the encoder.
//
// The untuned step is the one that has to be spelled.
// This element's tune enum starts at ssim rather than at no tuning, so leaving the property off is
// a tuning choice here where it is none on ffmpeg, and the enum's zero entry is named with a space
// that a gst-launch line cannot carry.
// It travels as its number instead.
...
// vpxEncoder maps the rate-control mode onto vp8enc or vp9enc, the libvpx elements and the
// counterpart to the ffmpeg libvpx branches.
// One mapping serves both: the two expose the same libvpx properties, and what differs between them
// (VP9's row-mt, the cpu-used point each is worth running at) is bound per codec in gstCodecs.
//
// deadline=1 is libvpx's realtime deadline, and static-threshold is the motion threshold the
// elements' own documentation recommends at 100 for screen and window sharing.
// end-usage selects the rate-control family.
// Constant quality is cq at cq-level, where the bitrate is a burst cap and not a target:
// libvpx has no unbounded constant-quality mode here, unlike the -b:v 0 the ffmpeg path uses.
// end-usage=vbr is libvpx's average-bitrate family and takes no ceiling, so vbr has no branch and
// is a gap on this engine (gstNoRateCeiling), as lossless is for want of a property.
// vpxenc counts target-bitrate in bits/sec and buffer-size in milliseconds,
// unlike x264enc/x265enc's kbit.
...
// aomEncoder maps the rate-control mode onto av1enc (libaom AV1), the counterpart to the ffmpeg
// libaom-av1 branch.
// usage-profile=realtime switches libaom off its two-pass defaults, and cpu-used,
// row-mt and lag-in-frames=0 are the same realtime trade as there.
//
// end-usage=q is libaom's unbounded constant-quality mode, the one place this element goes further
// than vpxenc, which has only the bitrate-capped cq.
// av1enc exposes no cq-level, so the quantizer target is pinned by setting both quantizer bounds to
// it.
// Its end-usage=vbr takes no ceiling either, so vbr is a gap on this engine (gstNoRateCeiling).
...
// branch.
// Two of the library's constraints leave it with two branches where the other elements have four,
// and capabilities.Codecs declares both as gaps rather than letting a mode run as another:
// max-bitrate is refused outside constant-quality mode, so vbr has no form on either engine,
// and cbr's low-delay prediction structure stalls this element, so it has none on this one.
...
// so vbr has no form on either engine and cbr and abr differ only in whether frame reordering is
// dropped for delay.
// bitrate is in bits/sec and the quantizer counts to 255. The element exposes no keyframe interval
// at all, so the configured GOP does not reach it and rav1e's own default stands.
...
//
// bitrate and cpb-size are in kbit; a zero cpb-size leaves the element its own calculation,
// so the VBV window only appears when the settings carry one.
// No effort step or B-frame count, matching the ffmpeg path: this family's row declares no ladder
...
// The bounds the va elements' rate-control properties impose: the highest value bitrate accepts,
// and the floor of target-percentage, which bounds how far under its ceiling a VBR target can sit.
const (
	vaMaxBitrateKbps      = 2_048_000
	vaMinTargetPercentage = 50
)

// vaAbrPeak is the factor the abr mapping places its ceiling at above the target,
...
// and nil where they can.
// A rate outside a property's range is refused rather than moved into it: the ffmpeg engine drives
// the same hardware at the rate the settings name, so a substitution here would make the bitrate a
...
// vaTargetPercentage renders the va elements' way of expressing a VBR target under a ceiling:
// bitrate is the maximum and this percentage places the target below it.
...
// qsvAbrPeak is the factor the abr mapping places its ceiling at above the target,
// the same derivation the ffmpeg builder's QSV mapping uses, so an unbounded average means the same
// thing on both publish engines.
const qsvAbrPeak = 2

// qsvShortBitrateKbps is the highest rate the AV1 and VP9 qsv elements accept:
// their bitrate and max-bitrate properties are unsigned 16-bit where the H.264 and H.265 elements'
// are unbounded.
const qsvShortBitrateKbps = 65535

// qsvShortRateLimits returns the reason an element with the 16-bit bitrate property cannot express
// the settings' rates, and nil where it can.
// As on the va elements the rate is refused rather than moved into range: the ffmpeg engine drives
// the same hardware at the rate the settings name, so a substitution here would make the bitrate a
// function of the capture backend.
...
//
// One mapping serves all four codecs: the qsv elements derive from one base class,
// and the quantizer properties are spelled the same on each.
// The H.264 and H.265 elements count their quantizer to 51 and the AV1 and VP9 ones to 255,
// which is the CqMax of each capability row.
//
// bitrate and max-bitrate are in kbit/s.
// The VBV window does not reach these elements: they expose no rate-buffer property,
// so the settings' figure binds on the ffmpeg engine alone.
// The effort step is the settings' own and reaches the element as its target-usage,
// which is the property oneVPL's scale is spelled on here.
// No B-frame count: the elements code no B-pictures unless asked, which matches the ffmpeg mapping
// pinning them off.
...
// nvencTuneProperties is how each step of the NVENC tune ladder is spelled on this engine,
// keyed as the capability row declares the step.
//
// The row spells the steps as the SDK and ffmpeg do; the elements spell the same four in full
// words, which is the property's own GstNvEncoderTune enum.
// A step this map has no entry for is one the elements do not implement, and the builder passes no
// tune rather than a word the element would refuse.
//
// The deprecated half of the elements' preset enum is what this replaces.
// Those entries name presets (default, hp, low-latency-hq) where the p1-p7 half names ladder steps,
// and the element's own help says to use the ladder with a tune instead.
...
//
// Both ladder steps reach these elements: preset takes the row's p1-p7 step and tune the row's step
// in this engine's spelling, so an NVENC stream encodes at the step the form shows whichever engine
// builds it.
// B-frames apply only to the lossy bursting modes.
//
// The element name is bound per codec in gstCodecs, so H.264, HEVC and AV1 share one mapping:
	...
	return func(s settings.Settings, r gstRates, l capabilities.Steps) []string {
		...
		withBframes := func(enc []string) []string {
			if s.Publish.Bframes > 0 {
				// bframes, not b-frames: the nvcodec elements spell it without the separator,
		...
		switch s.Publish.Mode {
		case "lossless":
			// No B-frames: bit-exact coding gains nothing from them, which is why the UI greys the field
			// here.
			//
			// The tune carries the mode rather than the preset: the enum's lossless preset no longer reaches
			// the H.264 encoder at 4:2:0 at all, failing session init with NV_ENC_ERR_INVALID_PARAM,
			// where the tune reaches every element and chroma the mode is offered on.
			// The row declares that tune for this mode, so it arrives through base above.
			return append(base, "gop-size="+r.gop)
		case "crf":
			// The quantizer goes on the three frame types rather than through qp-const,
			// which the AV1 element does not carry: nvav1enc exposes qp-const-i, -p and -b and no combined
			// one, where the H.26x elements expose both.
			// One quantizer on all three frame types is what the combined property sets anyway,
