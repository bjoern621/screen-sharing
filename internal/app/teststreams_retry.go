...
// and its last entry is the wait the slot keeps: the set is always-on, so there is no attempt
// budget for it to spend.
//
// A publish gives up because settings this machine cannot run fail the same way every time
// (publish_retry.go).
// The synthetic set is the other case: it runs one fixed pipeline that either works here or never
// did, and the failure it waits out is usually the relay, which this process starts before and
// outlives.
...
// testStreamHealthy is how long a publisher has to have run for its slot to count as working on
// this machine.
...
// testStreamWait reports how long slot i waits before the relaunch that follows attempts relaunches
// already spent.
...
// arms the relaunch while the set still wants it, and reports the failure once per outage.
//
// slot is the entry the dead child was launched into, and it is held against what the map carries
// now for the reason publishEnded holds its run: a stop and a restart both replace the entry,
// so an exit arriving after either belongs to a stream the app has already moved off.
func (a *App) testStreamEnded(i int, slot *testStream, err error, stderrTail string, logPath string) {
	...
	// The ladder above never gives up, so a relay that stays down would otherwise write a line into
	// the session log every thirty seconds for the rest of the run.
	// spent being zero is what marks the exit as the start of a streak: a slot that had already been
	// relaunched is still inside the outage its first exit reported.
	if err == nil || spent == 0 {
		// The exit says why this one stopped; the count beside it says how many are left.
		// A publisher that died on its own moves the count with nothing having been called,
		// which is the case the state event exists for.
...
//
// A relaunch that cannot start re-arms rather than ending the slot's life,
// for the same reason the ladder holds: a launcher that is missing now is the machine's state and
// not the set's, and the set is wanted until something says otherwise.
