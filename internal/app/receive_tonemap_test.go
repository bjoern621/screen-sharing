...
//
// The trap it is written against is a machine that cannot tone-map at all.
// A decode there is built without the rung whatever it was asked for, so a caller comparing its own
// request against what ran finds them different on every call and tears down a pipeline that was
// already everything it can be.
// What the two are compared through is WillToneMap, which answers what a request builds rather than
// what it asked for.

// aDecodeOf is one real decode of a synthetic picture, on the chain that needs no display.
//
// A real pipeline rather than a stand-in, because what is being asked is what a run was built with:
// a double would answer with whatever it was handed, which is the reading this is here to rule out.
...
// TestAskingAgainForTheToneMappingThatRanRebuildsNothing is the case a viewer produces every time
// it repeats a call, and the one a machine with no rung would otherwise loop on.
...
// TestTheOtherAnswerIsADifferentDecode is the half that only means anything where the machine has a
// rung: there the two answers are two pipelines, and the running one is handed back to be stopped
// rather than left beside its replacement.
func TestTheOtherAnswerIsADifferentDecode(t *testing.T) {
	...
	// Taken out of the set by the same call that handed it back, so nothing is left keyed by a pair
