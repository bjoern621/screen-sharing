...
// so a large count saturates the CPU without testing anything new.
const maxTestStreams = 9

// testStreamsAtBoot is the set this process brings up with itself and keeps alive,
// so the viewer roster carries streams on a machine that is publishing nothing.
// The tile grid is being built against them, and a relay with nothing on it shows the roster's
// empty state rather than the screen under construction.
const testStreamsAtBoot = 3

// EnvTestStreams overrides that count for one run, and 0 turns the boot set off entirely.
// It exists because the default is not free: three x264 encoders run for as long as the backend
// does, whether anybody is watching them or not.
const EnvTestStreams = "SCREENSHARE_TEST_STREAMS"

// testStream is one slot of the synthetic set: the child publishing it, or the relaunch that
// child's death armed.
...
// test-1) and picks the pattern, so a relaunch comes back as the same row on the roster instead of
// as a fourth stream.
...
// it wants to watch before anything has decoded: "test-2" says nothing about what is in it.
...
// They exercise the viewing paths (the viewer roster, the per-stream viewers,
// the receive pipelines) without a screen capture.
...
func (a *App) StartTestStreams(count int) error {
	...
	// Announced after the lock is released, for the reason StartWatch states: the count is read back
	// through a method that takes the same mutex.
...
// and the contract is up at its own speed rather than at theirs.
func (a *App) startTestStreamsAtBoot() {
	...
	if err := a.StartTestStreams(count); err != nil {
		...
		// validate.
		// The backend runs without the set, and the roster shows what the relay actually carries rather
		// than a promise.
		logger.Warnf("test streams not started: %v", err)
	}
}

// testStreamsAtBootWanted is how many the boot set holds, the default unless the environment names
// another count.
//
...
//
// It is read out here because settingsMu is taken before procMu everywhere (app.go),
// so a launch under the lock could not snapshot the settings itself.
...
//
// It counts through TestStreamsRunning rather than being handed a number, so what is announced is
// what a read would answer with: a publisher that died between the call and this is already out of
// the count.
...
// TestStreamsRunning reports how many synthetic publishers are alive.
// It is what the state event announces and what the contract answers with,
// so a publisher that died on its own drops out of the count without an extra event round-trip,
// and one waiting out its relaunch is not counted as alive.
