...
// convenient one: it is the only one that reports the applications playing as nodes of their
// own, which is what the per-application kind is. Its sinks and sources carry the same names
// its Pulse server serves them under, so one enumeration describes what either engine can
// open - a pulsesrc takes a sink's monitor by name, and only a pipewiresrc can take one
// application's output.
//
// A machine with no PipeWire enumerates nothing and every kind keeps its own default, which is
// the entry that needs no enumeration and what every build before this recorded through.
package audiodev
...
// pwDumpExe is the tool the enumeration reads, and pwDumpArgs what it is asked for: every
// object the daemon holds, as JSON, which is the one form that carries a node's properties
// rather than a line written for a reader.
const pwDumpExe = "pw-dump"

var pwDumpArgs = []string{"--no-colors"}

// The media classes a node carries, which is how one kind is told from another. A sink is
// something the machine plays into, a source something it hears, and an output stream one
...
// monitorSuffix is what a sink's own recordable side is called. Recording what the machine
// plays is recording the monitor of the sink it plays into, and the name is the sink's plus
// this - which is what both the Pulse server and the engines that open it use.
...
// devices changed since the first call is answered with what was there then; following
// PipeWire's own add and remove events is what replaces that, and it is worth its own
// mechanism rather than a shorter cache - the application just launched is the one worth
...
//
// A daemon that is not running, a tool that is not installed and output that does not parse
...
func enumerate(ctx context.Context) []platform.AudioDevice {
	...
	for _, o := range objects {
		...
		case classSink:
			// The sink's monitor and not the sink: what is recordable is what the machine
			// plays, and a sink is where it plays it.
		...
		case classSource:
			// A source that is itself a monitor is the desktop kind arriving by another
			// route, and listing it under both would offer one device twice.
		...
		case classStream:
			// One application's own output. It is named by its binary first and its own
			// name second, because two windows of one program are one binary and a name a
			// program writes for itself is a name it can change while it runs.
			devices = append(devices, platform.AudioDevice{
	...
	// Sorted so the same machine answers the same order on every run: the enumeration is
	// what an option list is built from, and a list that reshuffled between resolves would
	// move a reader's entry under their cursor.
...
// describe is what a device is called on screen: the description the daemon wrote for a
// reader, and the handle itself where there is none.
...
//
// The binary is the identity and the name is the description, which is the order the design
// asks for: two windows of one program are one binary, and a name a program writes for itself
// is one it can change halfway through a stream.
