...
// The enumeration is the one part of the audio list that touches the machine,
// so what it is held to is that it answers a real one: a list built from nothing would offer every
// kind its default and nothing else, which is what the app did before this existed.

// Every device carries the kind it is inside and the handle a publish opens it by.
// One without either is an entry a control can offer and a pipeline cannot open.
func TestEveryEnumeratedDeviceIsOpenable(t *testing.T) {
	...
	// A machine playing sound has a sink, and the desktop kind is that sink's monitor.
	// It is the one kind every machine with a daemon has something in.
	if kinds[platform.AudioSourceDesktop] == 0 {
		t.Errorf("the daemon reports no sink to record, and it enumerated %v", kinds)
	}
}

// The desktop kind is a sink's monitor and the microphone kind is not, so a monitor listed under
// both would offer one device twice under two names.
