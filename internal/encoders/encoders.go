// Package encoders reports which video encoders this machine can actually run,
// so the UI can grey out codecs that would only fail at launch.
//
// Two kinds of encoder cannot be assumed present.
// A hardware codec needs the GPU, its driver and the matching build: NVENC an NVIDIA card,
// VAAPI a render node whose driver exposes that encode entrypoint, QSV an Intel GPU whose oneVPL
// runtime the dispatcher finds, Vulkan Video a driver implementing the encode extension for that
// format.
// That is where the families diverge per generation, an AMD card carrying no VP8 or VP9 encoder and
...
// build may well lack.
//
// The answer is per publish engine, because the two wrap different encoder implementations.
...
// Each engine is probed the way its own failure shows up, and a codec that runs on one engine only
// is reported usable there and unusable on the other, so the settings form can grey it for the
// capture backends that cannot run it instead of for all of them.
//
// Every verdict here is a reading of the machine, so nothing in this package asserts on one.
...
// gstInspectExe is the GStreamer registry query.
// It ships with the same package as the pipeline launcher, so an install that can publish can
// answer this.
...
// publish.Engines is the list this has to cover, which TestEveryEngineIsProbed checks.
//
// The two probes differ because the two engines fail differently.
// An ffmpeg encoder appearing in "ffmpeg -encoders" only proves the build lists it,
// not that it initializes, so the reliable test is a one-frame encode.
// A GStreamer element is absent from the registry altogether when its plugin is not installed,
// and the hardware plugins register their elements per detected device, so asking the registry
...
// Without the executable every test encode fails, and reporting that as a per-codec verdict would
// answer a question about ffmpeg with a sentence about the machine's encoder hardware.
...
// gst-inspect ships with the same package as the pipeline launcher, so its absence means this
...
// ffmpegAssumed are the encoders no probe is spent on: both are in every ffmpeg build worth
// shipping and neither fails to initialize, so a test encode would spend a process on a foregone
// answer.
...
// It is derived from the capability table rather than listed, so a codec added there is probed and
// greyed where it cannot run instead of reaching a launch that fails.
...
// gstProbed lists every implemented codec this engine has an encoder for, unlike the ffmpeg half.
// Each element comes from its own plugin package (x264enc from gst-plugins-ugly,
// rav1enc from gst-plugins-rs, the va and nvcodec elements from device-conditional plugins),
// so none of them is safe to assume.
//
...
// A working encoder returns in well under a second; the timeout only guards against a hung ffmpeg.
const probeTimeout = 10 * time.Second

// Availability maps each publish engine to the codecs probed on it and whether each one ran.
// A codec absent from an engine's map was not probed there and the UI treats it as available;
// an engine absent from the outer map imposes no restriction at all.
//
// Unprobed names the engines whose own tooling is missing, with the reason.
// No codec on such an engine was tested and none of them can run there, the ones no probe is spent
// on included, so the reason covers the whole engine rather than a codec at a time.
// Without it a missing ffmpeg reads as a machine with no encoder hardware,
// and the two encoders in ffmpegAssumed stay selectable while being the only ones certain to fail
// at launch.
...
// Detect probes every codec on every publish engine, concurrently, and returns what this machine
// can run.
...
func Detect(ctx context.Context) Availability {
	...
	// An engine is asked or it is excused, never both and never neither.
	// A name in both maps would let a surface read a verdict off an engine it had just been told
	// carries none.
...
// It clears each probed encoder's minimum: SVT-AV1 refuses anything under 64 square,
// and a fixed-function encoder refuses more than that, with a floor that varies by driver and codec
// (an AMD VCN wants 130 pixels of width for HEVC and 128 for H.264).
const probeSize = "256x256"

// ffmpegUsable encodes a single frame with codec and reports whether ffmpeg exited cleanly.
// Success confirms the whole chain: the build has the encoder and, for a hardware codec,
// the driver is loaded and a GPU accepted the session.
// Detect has already located the executable (ffmpegAvailable), so a failure here is the encoder's
// and not the engine's.
//
// The frame is 8-bit 4:2:0, so the verdict is per codec and not per chroma:
// an encoder that opens here but implements no 10-bit profile fails at launch on p010le instead.
// capabilities.Codecs declares only the bit depths a family's drivers implement broadly,
// which is what keeps that gap narrow.
func ffmpegUsable(ctx context.Context, codec string) bool {
	...
	if err != nil {
		// The family has no device on this machine: no render node for VAAPI, which is the verdict the UI
		// wants anyway.
		return false
	}
	...
	// A VAAPI, QSV or Vulkan encoder reads GPU surfaces, so the probe uploads the frame exactly as the
...
// Registration is the test because it already carries both conditions: a plugin that is not
// installed contributes no element, and the va and nvcodec plugins enumerate devices at load and
// register an element per encode entrypoint the hardware exposes, so a missing card leaves the name
// unresolved exactly as a missing plugin does.
