...
//
// A scanout framebuffer usually carries a GPU tiling or compression modifier,
// so hwdownload cannot map it directly and fails with EINVAL.
// Mapping the frame through a hwdevice that understands the modifier first solves this,
// but which device works depends on the GPU.
// The strategy is therefore selectable, and the table is the single source of truth for what the
// ffmpeg vf chain becomes.
type DrmMap struct {
	// Name is the settings value and UI key.
	Name string `json:"name"`
	// Device is the hwmap derive_device that understands the modifier.
	// An empty Device downloads the frame directly, correct only for a linear framebuffer.
	Device string `json:"device"`
	// Auto resolves Device from the capture device's kernel driver at runtime instead of using the
	// fixed Device above.
	Auto bool `json:"auto"`
}

// DrmMaps is the DRM-download strategy table.
...
// The whole point of the setting is overriding the driver guess, so resolving a misspelled "vaapi"
// to auto would run the strategy the user chose against, and the frame either arrives mapped
...
//
// The lookup is separate from resolving a device so that it holds against the table alone:
// a machine with no DRM node would otherwise answer a misspelled strategy with its own missing
// hardware.
...
// drmMapNames lists the strategy names for the sentence a refusal ends in,
// so the message cannot name a set the table no longer holds.
...
// including NVIDIA and unknown drivers, goes through Vulkan, the cross-vendor DRM interop path.
...
//
// kmsgrab reads scanout buffers below the compositor and needs CAP_SYS_ADMIN (run through the
// capability wrapper, see FindCaptureExe, or via sudo).
//
// The GPU path takes no strategy.
// A strategy names the device a tiled scanout buffer is mapped through so that hwdownload can read
// it, and the GPU path downloads nothing: the map that follows targets the encoder's own device and
// is decided by the encoder family, not by the user (GpuFilters).
// The setting is therefore unread rather than overridden, which is what the form greys it for.
func kmsgrabCaptureArgs(s settings.Settings, fps, memory string) (captureSource, error) {
	...
	// The strategy is checked before the machine is looked at, so a name no row carries is answered
	// with the setting rather than with whatever the sysfs probe finds.
...
// the first card whose kernel driver is neither absent nor the boot framebuffer.
//
// card0 is not a safe default, which is why no default is taken.
// On machines with a discrete GPU the boot framebuffer often holds card0 under the
// simple-framebuffer driver while the real display controller lands on card1 or higher,
// so naming card0 when the probe finds nothing hands kmsgrab a node with no scanout to read and the
// failure reads as a broken capture rather than as a machine with no display controller the probe
// could see.
func drmCaptureDevice() (string, error) {
	// filepath.Glob returns matches in lexical order, so the lowest-numbered real GPU is selected.
...
// captureDriver returns the base name of the kernel driver bound to a /dev/dri card node (e.g.
// i915, amdgpu, nvidia, simple-framebuffer), or "" when no driver is readable.
// The driver symlink under sysfs resolves to the bound module.
