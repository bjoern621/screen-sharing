...
//
// It does three things and holds almost nothing.
// A group is created by drawing a key; a member trades that key for a short relay token;
// and the index answers which streams a key can see.
// All three are derivations rather than lookups, which is why there is no database behind any of
// them: possession of the key is membership, the prefix is the key's own digest (internal/group),
// and which streams exist is the relay's answer rather than this service's.
//
// It lives in this repository rather than in one of its own because the path-prefix derivation has
// to be identical on both sides, and two repositories means two copies of it (docs/plan.md,
// "Groups, auth and encryption").
//
// None of it is end to end.
// The relay terminates every protocol and re-muxes for every listener, so it sees plaintext by
// construction, and this service holds the key that signs tokens for every group.
// Both can watch a private stream, and the interface says so rather than implying otherwise.
//
// Every refusal here answers a caller rather than crashing: a malformed key,
// a missing body and a caller over the creation bound are all Umgebungsfehler,
// since the input arrives over HTTP from somebody this process does not control.
package groupsvc
...
//
// Short, because possession as membership means a member who leaves still holds the group key:
// what stops them is a rotation, and the window is how long the old key keeps working after one.
...
// The relay checks at the handshake and not again, so this is a bound on opening connections rather
// than on holding them, which is the assumption the whole lifetime rests on (docs/plan.md,
// "Assumptions to verify").
const TokenWindow = 5 * time.Minute

// CreationsPerHour is how many groups one address may create in an hour.
//
// Creation is open, since there are no accounts and so nobody to charge it to,
// which makes it something to bound rather than something to gate.
// The figure is high enough that a person creating groups all afternoon never meets it and low
// enough that a script filling the relay with prefixes does.
const CreationsPerHour = 60

// PublicPrefix is where a stream anybody may watch lives.
//
// Public means watchable and discoverable, and both follow from the prefix:
// the relay grants reading on it to everyone, and the index answers it to a caller with no key.
// It is a group id's shape without being one, since no key derives it, so a group's own listing
...
// It is safe for concurrent use, which is what an HTTP server needs of a handler.
type Service struct {
	signer  *token.Signer
	streams Streams
	// now is read rather than called directly, so a test can issue a token at a moment it chooses and
	// read the window back off it.
	now func() time.Time

	mu sync.Mutex
	// created is when each address last created groups, oldest first, for the bound above.
	// A map of slices rather than a counter per address, because what has to age out is each creation
	// on its own hour.
...
//
// Every route is a POST or a GET and nothing else, and the two that take a key take it in the body
// rather than in the path: a key in a URL is a key in every proxy log between here and the client.
// The index is the exception and takes its key in the query, because a GET has no body a cache or a
// proxy will honour.
...
// Nothing is stored, because there is nothing to store: the group exists by virtue of somebody
// holding the key, and the prefix is that key's own digest.
func (s *Service) createGroup(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// The randomness failed, which is the one failure here that is this machine's rather than the
		// caller's.
		// A key drawn from something weaker would look exactly like a real one, so nothing is handed back
		// at all.
...
//
// The key is verified by deriving from it and nothing else.
// There is no membership list to check it against, which is the model: a caller holding a
// well-formed key is a member, and a caller holding a key nobody else has is a group of one.
...
// listStreams answers which streams the caller may see: their group's where they present a key,
// and the public ones where they do not.
//
...
// that what a member asked for is their group.
...
// It is what the relay fetches once and checks every connection with locally,
// which is why nothing here is called per stream.
...
// A key is 44 characters of base64, so anything past this is not a request this service has a
// reading for.
const bodyLimit = 4096

// allowCreation reports whether this caller may create another group, and records it where it may.
...
// so bounding by one is bounding by a number the caller chooses.
// A deployment behind the reverse proxy sees the proxy's address, which makes the bound the proxy's
// to enforce there and this one a backstop.
...
//
// The sentence is the service's rather than a code, because this is not the app's own contract:
// the caller is a client of an HTTP API, which has no vocabulary to look a code up in.
// docs/ipc-api.md draws that line for the control plane, and this is the other side of it.
