...
//
// It is the publish package's engine seam from the other side of the wire: each viewer engine
// serializes a stream into its own command line, asking the transport registry for the form it
// consumes.
//
// ffplay and mpv drive any transport with a URL watch form (transport.Watcher).
// WebRTC has none, its playback protocol being WHEP, and would take an engine keyed on a transport
// capability of its own: another engine in this package, and no app-layer change.
package watch
...
// EnvViewer selects the viewer program: "mpv" switches away from the default ffplay, for comparing
// the two on a machine without a rebuild.
const EnvViewer = "SCREENSHARE_VIEWER"

// Select returns the viewer engine for the named watch transport.
// The leg is per viewer rather than the publish settings', so a stream can be received over any
// transport the relay serves it on.
//
// A transport no registry row names is an Umgebungsfehler, the value being stored in a file the
// user owns, so it leaves as an error rather than an assert.
...
// WindowTitle carries the transport beside the stream name, so a user watching one stream over
// several legs at once can tell the windows apart.
