...
**A subscription names one of two kinds of decode, and the second one has no key.** `FrameSubscribe` carries either a `WatchKey` - a stream and the leg it crossed the relay over - or `PublishPreview`, the local decode of the stream this machine is sending. The preview message is empty and needs to be: `PublishState.live` is singular, so "the running publish's preview" is already a complete identity, and a name or a port on that message would be the consumer restating what it read off the state. It is a separate arm rather than a leg in the transport table because its frames crossed no protocol at all - the publish child copies the encoded video to a loopback port and the backend decodes it there (`viewer-architecture.md`, "What the broadcast preview draws"). A synthetic transport entry would state that some protocol carries this stream, to every consumer of the table that decides what a leg may offer.

**Which effect opens each is the rule that keeps the frame channel from deciding anything.** A relay decode is opened by `StartReceive`, and the preview by the publish itself: the loopback port has to be in the child's argv, so bringing it up belongs to the launch, and there is no `StartPreview` on this contract for a shell to call. Both are the same division seen twice - the frame channel finds a decode or is refused with `FAILED_PRECONDITION`, and never starts one. What tells a shell whether there is a preview to ask for is `PublishState.Live.preview`, present exactly while one is running and carrying the port and what the pipeline turned out to be. Reading it rather than asking and being refused is the same discipline every other screen follows.
