...
They live in `form/presets.go`, beside the capability table the search walks and the repair it holds every candidate to, with the shells that used to hold them (`ipc-api.md`).
...
That is where they belong rather than on a method of their own, because all three answers change with the draft exactly as a greying does, and a surface that asked for them separately would hold a preset verdict older than the settings beside it.
...
The claim is what decides whether a preset is still the selected one.

The retransmit window on it is the publish leg's alone.
A viewer pays that window and the watch hop's together, but the watch hop belongs to the machine doing the watching and a preset carries no viewer settings, so the claim speaks for the half a preset can move.

Colour range is one preset's axis and not a shared one.
Coding the desktop into the narrower studio swing throws away code values before the encoder sees them, so `lossless` claims full range and would not be itself without it; the other two promise nothing about the range and leave the settings' own where it is.
A base that wrote a value the claim does not mention would strand the preset on every machine that refuses that value - the GStreamer VA elements signal no colour description, which is exactly such a machine - for a promise the preset never made.
...
It writes those fields and leaves the others standing, so a field the preset promises nothing about keeps what the settings hold - a bitrate target means nothing to a lossless encode, and zeroing it would spend a figure the user chose on a field the preset never reads.
...
The settings a search runs against are the repaired ones, which is what makes rejecting a repaired candidate sound rather than merely strict.
...
That is the same question asked of the same function, one field at a time instead of a fixed point over the whole table: it decides nothing new, and it is what keeps a machine that reaches no preset from paying for a full repair per candidate on every keystroke.
...
- **Capture backend**, from `publish.AutoCaptures`, with the selected one first.
...
A preset no candidate satisfies carries the reason instead of settings, and a surface draws it ruled out with that reason beside it - the same treatment `field-availability.md` gives an option the tables rule out.
...
The reason names the publish transport, that being the one dimension the search worked within rather than varied, so what it hands the reader is something they can change.
...
The two kinds are two lists rather than one, so a draft that equals a snapshot and delivers a promise marks a row in each, and neither has to win: they are two true statements about one draft, made about different things.

## Adding a preset

Add the entry.
Its claim must be disjoint from every existing one, which the load-time check enforces, and the line a surface writes for it must be true of every settings object the claim accepts, which is what makes the claim a safe test for the selection.
...
The base writes only what the claim speaks for: a field written past the claim can strand the preset on a machine that refuses that value, for a promise the preset never made.

A surface then owes the new key a name and that line, as it owes every identifier one (`ipc-api.md`).
