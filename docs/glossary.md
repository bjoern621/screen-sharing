...
| Encoder | One concrete encoder within a family, named as the ffmpeg encoder: `libx264`, `hevc_nvenc`. What the "Encoder" dropdown offers, one entry per row of the capability table (`publish.codec`). | codec (a codec is the format), encoder engine |
| Video codec | Coding format an encoder produces: H.264, HEVC, AV1, VP9, VP8. A column of the capability table (`capabilities.Codec.Format`), so it follows from the encoder rather than being chosen beside it. | codec family, video format (in the UI) |
