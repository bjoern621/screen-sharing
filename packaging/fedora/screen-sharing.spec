# ffmpeg and GStreamer are declared dependencies rather than bundled copies, which is
# what docs/packaging.md picks for every channel with a package manager behind it, and
# what rpm expects.
#
# This spec builds with network access, because both toolchains fetch their dependencies
# during the build: `go build` resolves modules and `dotnet publish` restores NuGet
# packages. That rules it out of Fedora's own build system, which builds offline against
# bundled sources, and suits a local `rpmbuild` or a CI job, which is where the released
# package comes from.
#
...
#   gst-plugins-rs is not packaged at all, so whipclientsink and whepsrc are absent and
#   the WebRTC legs (WHIP publish, WHEP watch) fail at pipeline start on a stock install.
...
# No debuginfo package. Half of what this installs is a .NET publish output, which
# carries no DWARF to extract, and rpm's extraction over the rest finds no sources to go
# with it. The Arch PKGBUILD disables its split package for the same reason.
%global debug_package %{nil}

# Fedora's default takes the build timestamp from the newest changelog entry, and this
# spec has no changelog to take one from (the note at the end of the file states why).
...
# The project's own code. What this package installs beyond it is compiled-in Go and
# .NET libraries under MIT, BSD-3-Clause and Apache-2.0; ffmpeg and GStreamer are
# Requires rather than bundled copies, so their terms are theirs to declare.
License:        Apache-2.0
...
# Capture, encode, publish and the single-stream viewer. The paths rather than a package
# name because both ffmpeg-free and RPM Fusion's ffmpeg provide them, and either serves.
...
# The GStreamer publish engine spawns gst-launch-1.0, and the tile grid decodes through
# the libraries the backend links. Both read the same plugin set.
Requires:       gstreamer1
...
# and which the frame channel imports decoded frames through. rpm's dependency generator
...
# the backend from beside itself.
...
# A launcher rather than a symlink into %{_libdir}: the shell resolves the backend
# against its own directory, and a symlinked entry point would answer %{_bindir}.
...
# No %%changelog section. The one rpm would carry is the git history, and rpmautospec,
# which generates it from that history, is part of Fedora's build system rather than
# something a local rpmbuild has.
