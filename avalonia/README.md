...
Quit is one sequence whichever control runs it: the window's decodes close and the stream ends, side by side and bounded, then the process shuts down and the exit hooks take a backend this shell started with it (`Backend/BackendProcess.cs`).
The stop goes out whichever backend serves the stream and whatever is on the air, a stop naming the state it wants and one with nothing publishing succeeding.
