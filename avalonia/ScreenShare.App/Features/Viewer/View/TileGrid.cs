...
///
/// <b>It computes nothing.</b> Every rectangle comes from <see cref="TileLayout"/>, which is pure and tested
/// without a window; what is here is the two Avalonia passes and the reading of each child's shape.
/// That split is the reason the arrangement can be asserted at all (<c>avalonia/README.md</c>).
///
/// Each child declares its own aspect ratio through <see cref="AspectProperty"/> and whether it is the
/// focused one through <see cref="IsStagedProperty"/>, both bound in the item template.
/// The panel reads attached values rather than reaching into anybody's data context, so it knows nothing
/// about view models and a second kind of tile would need no change here.
/// </summary>
public sealed class TileGrid : Panel
{
    /// <summary>
    /// The space between two tiles, and between two rows.
    /// Fixed rather than styled: it is an argument to the arrangement, and a layout whose spacing came from a
    /// theme would be a layout whose test and whose screen disagreed.
    /// </summary>
    private const double Gap = 8;

    /// <summary>
    /// How much of the height the rail under a focused tile takes, and the bounds it is held between.
    ///
    /// A fraction so that the rail scales with the window, bounded so it neither disappears on a short one
    /// nor takes half of a tall one.
    /// The focused tile gets everything the rail does not.
    /// </summary>
    ...
    /// <summary>
    /// The height the arrangement is fitted into when the panel is measured with an unbounded one, which is
    /// what a scroll viewer does.
    ///
    /// It is bound to the viewport rather than guessed, because "fits the box" is the whole of what the
    /// arrangement decides and a panel measured against infinity has no box.
    /// Zero means nothing has measured the viewport yet, and the arrangement then uses whatever finite height
    /// it was given.
    /// </summary>
    ...
    ///
    /// The reported height is the arrangement's own rather than the box's, so a grid that had to scroll is
    /// taller than its viewport and the scroll viewer around it has something to scroll.
    ...
    ///
    /// <b>The viewport wins over the size this pass was handed, and that is not a preference.</b> Inside a
    /// scroll viewer the two passes are handed different heights: measure gets an unbounded one, and arrange
    /// gets back the height measure returned.
    /// Solving against whatever each pass was given therefore solves two different boxes, picks two different
    /// arrangements, and arranges the tiles by one while having measured them by the other - which is a grid
    /// whose rows are the wrong heights and whose tiles are laid out for a window that does not exist.
    ///
    /// One box for both passes is the fix, and the viewport is the only honest candidate for it: it is the
    /// space a reader actually sees, which is the thing "fits the box" is about.
    /// </summary>
    ...
    ///
    /// A box with no room in it arranges nothing at all, tiles or not (<see cref="TileLayout.Solve"/>), and
    /// that box is the first measure pass inside a scroll viewer: the viewport it is fitted into has not been
    /// measured yet, so there is no height to solve against until the pass after.
    /// Every child is placed at nothing there rather than left out, because a child this list skips is a
    ...
    ///
    /// With nothing focused this is a grid, which is what makes the mode safe to be in while the focused
    /// stream is being chosen or after it has gone: the arrangement degrades to the other mode rather than to
    /// an empty screen.
    /// </summary>
    private List<(Control Child, Rect Rect)> Focused(Size box)
    {
        ...
        // The rail is one row at a fixed height, tiles at their own widths, centred while they fit and
        // left-aligned once they do not.
        // It is not the grid arrangement: the rail's height is decided by the stage above it rather than by
        // the tiles in it.
    ...
    ///
    /// The focused tile keeps its stream's shape like every other tile does; what changes is that it is
    /// alone, so the leftover space becomes a margin instead of another tile.
    /// </summary>
