...
/// only thing that persisted them was starting to share: the wizard's draft reaches the backend through
...
///
/// <b>Nothing here reaches a decode before the button does.</b> Every knob a receive pipeline
/// reads is read by the backend out of its own settings as the pipeline is built, and the one
/// value the shell names in the call - the tile's leg - is read out of those same stored settings
/// (<c>Features/Viewer/Tile/Model/TileLeg.cs</c>). Which is why the panel says when what it shows
/// is not yet what is stored: a staged group that looked identical whether or not it had been
/// kept would give the button nothing to mean.
///
...
public sealed class WatchSettingsViewModel : Observable
{
    ...
    /// <summary>
    /// Shuts the panel. It is the holding screen's, because whether a panel is open is part of
    /// that screen's arrangement and this component draws one column of it
        ...
        FormSession form, Session session, Action<Action> dispatch, Action close)
    {
        ...
        // Dismissing takes no round trip and cannot be refused, so it is the plain command. It
        // keeps nothing: a reader who moved a slider and shut the panel decided against it, and
        // storing on the way out would be this screen keeping what nobody asked it to.
    ...
    ///
    /// It is the whole difference a staged group makes, and there is no other way to see it: the
    /// controls draw the draft either way. Two things raise it - a field the reader moved and has
    /// not kept, and a value the resolve repaired that nothing has written back - and both mean
    /// the same thing for a decode, which is that it will open on the held value and not on the
    /// one on screen.
    /// </summary>
    ...
    public void Apply()
    {
        ...
        // The failure the whole window shares, read through rather than kept: this panel and the
        // wizard write the same settings message down the same queue, so what could not be
        // stored is one sentence and not one per screen (FormSession.Unsaved).
    ...
    ///
    /// The keys are the form's own rather than a list written here, so a knob the backend adds to
    /// the group is compared with nothing to edit - the same property that makes the panel draw
    /// it in the first place.
    ///
    /// Neither side arriving is not a difference. Before the first answer there is nothing on
    /// screen to be unkept, and saying so would put a warning under a panel with no controls in
    /// it.
    /// </summary>
    ...
    ///
    /// It does not reach the backend itself, and that is the point. The settings travel whole,
    /// so this commit and an applied field's keystroke are two writes of one message; sent from
    /// two places they are two unary calls with no ordering between them, and the older snapshot
    /// landing last is a stored setting the reader had already changed. One queue removes that
    /// (<see cref="FormSession.SaveAsync"/>).
    ///
    ...
    private async Task SaveAsync()
    {
        ...
        // Back to the UI loop, where the answer to the write was recorded. It is queued behind
        // that record rather than racing it: both go through this dispatcher, and the write's own
        // was handed over before the task this awaited completed.
        _dispatch(Kept);
    }
    ...
    ///
    /// <b>A write that landed shuts the panel and one that did not leaves it open.</b> The reader
    /// asked for these settings to be kept: once they are, there is nothing left on this column
    /// to look at, and a panel that stayed would be the reader closing it a second time by hand.
    /// A refusal is the opposite - the sentence explaining it is on this panel, and dismissing
    /// the panel would take it off the screen with the fields it is about.
    /// </summary>
    private void Kept()
    {
        // Read through rather than passed in: the write reports its own answer, and a copy
        // carried back through the commit would be the same sentence held twice.
