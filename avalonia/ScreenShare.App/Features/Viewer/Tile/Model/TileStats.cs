...
///
/// The name and the meaning are fixed and the reading moves, which is the whole reason this is
/// a live object rather than a record replaced per sample. A row replaced once a second is a
/// row whose tooltip closes once a second, and the tooltip is what the panel is for.
/// </summary>
public sealed class StatLine : Observable
{
    ...
    /// <summary>
    /// What it reads now, or <see cref="Figure.NoValue"/> where nothing has measured it. Never
...
///
/// The rows are a collection that is converged rather than replaced, for the reason a row's
/// reading is written rather than rebuilt. Which rows a block holds changes only when the
/// pipeline negotiates something new.
/// </summary>
...
/// <summary>
/// The stats panel, composed from one sample of a decode and one report from the window
/// drawing it.
///
/// <b>It is a function and holds nothing.</b> A panel is the whole of what two readings say,
/// rebuilt on every pass, so there is no accumulated figure here to disagree with the backend
/// and nothing to reset when a decode is rebuilt under the same tile.
///
/// <b>The blocks follow the frames.</b> Arriving, picture, decode, render, timing, then what
/// this window did with the result - which is the order a reader walks when a tile looks wrong,
/// because the first stage that reads badly is the one to act on and everything after it is
/// downstream of that.
///
/// <b>Nothing is computed here that the backend measured.</b> The rates are the sample's, taken
/// against the backend's own interval, so two windows on one decode read the same figure
/// (<c>api/proto/screenshare/v1/events.proto</c>, ReceiveStreamStats). What this adds is the
/// window's own block, which is the one thing a backend cannot see.
/// </summary>
public static class TileStats
{
    ...
    ///
    /// <paramref name="sample"/> is null on a tile whose decode has not been sampled yet and on
    /// one drawing something that is not a relay decode at all. Both leave the panel with the
    /// window's own block alone, which is honest: this window's counters are true whatever is
    ...
    public static IReadOnlyList<StatSection> Of(ReceiveStreamStats? sample, TileReport report)
    {
        ...
        // Last, because they are the deepest thing on the panel and the only part of it that
        // depends on which leg the decode was opened on.
    ...
    ///
    /// <b>It writes readings into the rows that are there rather than replacing them.</b> A row
    /// is a name, a meaning and a figure, and only the figure moves between two samples of one
    /// pipeline; a panel rebuilt every second would take the row out from under a pointer that
    /// is resting on it, which closes the tooltip that row exists to show.
    ///
    /// The shape is what decides between converging and replacing, and the shape is the
    /// headings and the labels. It changes when a pipeline negotiates something it had not - an
    /// audio branch coming up, a transport element starting to keep counters - and then the
    /// block or the panel is rebuilt once and holds still again.
    /// </summary>
    ...
    /// A heading over an empty block says a stage exists and refuses to describe it, which is
    /// worse than the block being absent: the audio block is missing because there is no track.
    /// </summary>
    ...
    [
        ...
        // Stated in both directions rather than only when it is on. A reader comparing two
        // tiles of one HDR stream is comparing exactly this, and a row that disappeared when
        // the answer was "no" would leave them unable to tell which tile they are looking at.
    ...
    ///
    /// Emptiness is the test rather than a flag, because it is the same fact: the branch is
    /// what fills every field here, so a stream published without a track and one whose branch
    /// has not come up yet both read as no block. Which of the two it is belongs to the tile,
    /// which draws a meter for the first and none for the second.
    /// </summary>
    ...
    /// <summary>
    /// What this window got and drew. It is always drawn, on every kind of tile, because it is
    /// true of every kind: these are the frame channel's counters and they do not depend on
    ...
    private static StatSection Transport(ReceiveStatGroup group)
    {
        ...
        // The element's own name, beside what it is. It is what tells two jitterbuffers of one
        // muxed stream apart, and it is the name to look for in a pipeline dump.
    ...
    /// <summary>
    /// One row. An empty value is what a figure nothing has measured looks like on the way in,
    /// and it prints as absent rather than as a zero: the pads negotiate after the pipeline is
    /// built, so every field here is empty for a moment on every decode.
    /// </summary>
    ...
    /// <summary>
    /// One of an element's counters. The unit follows from the key, because that is where the
    /// unit is stated in the first place: an element that reports a figure in milliseconds names
    /// the field for it.
    /// </summary>
    ...
    /// <summary>
    /// The frame rate the caps declare. A denominator of zero is a stream that declares a
    /// variable rate, which is a different fact from declaring none and is worth saying.
    /// </summary>
    ...
    /// <summary>
    /// The latency window as one reading, since the two ends of it are only meaningful
    /// together. A window with no upper end is a pipeline that will hold a frame for as long as
    /// it takes, which is worth its own word rather than a very large number.
    /// </summary>
    ...
    /// <summary>
    /// A byte total at the scale it is worth reading at. Decimal rather than binary multiples,
    /// because the figure beside it is a bitrate and the two are compared.
    /// </summary>
