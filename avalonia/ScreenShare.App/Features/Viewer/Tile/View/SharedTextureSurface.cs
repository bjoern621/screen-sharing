...
/// <summary>
/// Import path for a handle the compositor takes itself: a shared texture named to Avalonia's device,
/// drawn as a composition visual.
///
/// No pixel read: the decoder writes the texture, the frame channel carries its name,
/// and the name goes to the compositor (<c>docs/viewer-architecture.md</c>, "The frame channel").
///
/// Not a <see cref="NativeControlHost"/>.
/// A native child window paints over every Avalonia control,
/// burying the name, the badge and the stats panel that belong above the video.
/// A composition visual sits among them (<c>avalonia/README.md</c>).
///
/// A frame is a slot on loan, returned once the compositor has taken it,
/// which <see cref="CompositionDrawingSurface.UpdateWithKeyedMutexAsync"/> resolves on.
...
internal sealed class SharedTextureSurface : Control, ITileSurface
{
    ...
    /// Each is imported once and drawn from repeatedly, an import per frame being a driver crossing per frame.
    /// </summary>
    private readonly List<ICompositionImportedGpuImage> _slots = [];

    /// <summary>
    /// Keyed-mutex keys, taken crossed: acquire on the pool's consumer key, release on its producer key,
    /// so neither end hardcodes a number the other picked.
    ...
    public async Task<string?> ImportAsync(FramePool pool, CancellationToken cancellation)
    {
        ...
        // Queried of the renderer rather than derived from the platform, and refused outright:
        // copying through system memory is the gigabyte-a-second fallback the frame channel exists to avoid.
        var handleType = HandleTypeOf(pool.HandleType);
        if (handleType is null || !interop.SupportedImageHandleTypes.Contains(handleType))
        {
            return "This window's renderer cannot open the kind of shared frame this computer decodes into.";
        }
    ...
    /// Flow control lives in that await: it marks the slot free,
    /// so the release behind it reports rather than assumes,
    /// and a tile the compositor serves slowly stops asking for frames instead of overwriting one being drawn.
    /// </summary>
    ...
    /// <summary>
    /// Discards the imports of a replaced pool.
    /// Awaited, not fired off.
    /// The release runs on the render thread while the backend frees the texture as the call ends,
    ...
    /// Two graphics backends on one operating system import different sets.
    /// </summary>
