...
///
/// The value is an entry of the catalog's player roster, so a protocol added to the transport registry arrives
/// here with nothing in this module to edit; the word on the row is this side's.
///
/// <b>Every row is pressable.</b> The roster names the legs a player on this machine opens, so no entry is one it
/// cannot, and whether this stream's format travels on the leg is answered against the stream as the viewer
/// opens: the relay's snapshot can be older than the stream, so greying from a stale format would refuse a viewer
