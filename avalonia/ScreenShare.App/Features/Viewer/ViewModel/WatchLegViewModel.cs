...
///
/// <b>That is not the same fact as the option's own verdict, and this row draws both.</b> The
/// availability pass answers whether a player on this machine can be opened on the protocol at
/// all, which is a property of the receiver rather than of a stream, and nothing about it goes
/// stale between a snapshot and a press. An entry it ruled out is greyed and carries its reason,
/// as every other option in the product is (<c>docs/field-availability.md</c>). This row used to
/// throw both away and offer every leg live, so the one rule the design states about an
/// unavailable option was the rule this menu did not keep.
///
/// The reason is drawn in the row rather than hung off it as a tip. A disabled control in
/// Avalonia takes no pointer events, so a tooltip on a greyed row is a sentence nobody can open -
/// and a greyed row whose reason cannot be read is the blank the treatment exists to prevent.
///
