...
/// Which groups exist, in what order and with what under each is the backend's (<c>docs/ipc-api.md</c>, "The
/// rule").
///
/// The watch group holds the legs a stream comes back on, the jitter buffers a receiver holds and the chain a
/// tile converts frames with.
/// None of it changes a byte this machine sends, so the viewer draws it beside the tiles it governs and the
/// wizard draws every other group.
///
/// Defaulting to the wizard is what makes a group the backend adds a step that appears with nothing here to
/// edit (<see cref="Setup.Model.SetupSteps"/>).
/// </summary>
