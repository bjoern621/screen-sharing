...
///
/// <b>Whether the press is offered is the command's answer, and why it is not is
/// <see cref="Notice"/>.</b> A screen that refuses the effect - a measurement that would compete
/// with a live stream is the case that exists - greys the button through the command's own gate
/// and states the reason beside it, in the place the same screen states why a control is inert
/// (<c>docs/field-availability.md</c>, "A live stream blocks no field").
///
/// A record, so a pass over an unchanged state produces an action that compares equal to the
/// last one and the bound properties are left alone. It is made per pass, because the notice is
/// read from state that moves; <see cref="Command"/> is held by whoever offers the action, which
/// is what makes two passes equal rather than merely alike, and it is the one place the
...
public sealed record FieldAction
{
    ...
    /// <param name="notice">
    /// What stands about the effect now: why the press is refused where it is, and what the last
    /// attempt answered otherwise. Empty where there is nothing to say.
    /// </param>
    ...
    /// <summary>
    /// Why the press is refused, or what the last attempt answered. Empty where the effect is
    /// offered and nothing has been said about it.
    /// </summary>
