...
namespace ScreenShare.App.Features.Insights.Plots.Model;
...
public static class PlotSeries
{
    ...
    public static LatencySeries Latency(IReadOnlyList<RelayReading> samples, string stream)
    {
        ...
        {
            var path = InsightsSnapshot.PathOf(sample.Status, stream);
            if (InsightsSnapshot.WorstRttMs(path) is not { } rtt
                || InsightsSnapshot.WorstLossPercent(path) is not { } loss)
            {
                continue;
            }
