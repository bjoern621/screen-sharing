...
namespace ScreenShare.App.Features.Insights.ViewModel;

/// <summary>
/// Insights screen: the live overview, and read-only about everything else.
///
...
/// The one way out is <see cref="InsightsAction.EditInSetup"/>, which navigates rather than editing here.
///
...
public sealed class InsightsViewModel : Observable
{
    ...
    public event Action<InsightsAction>? ActionRequested;
    ...
    /// </param>
    public InsightsViewModel(IBackend backend, FormSession form, Session session, Action<Action> dispatch)
    {
        Assert.NotNull(backend, "a insights screen asks the backend to act");
        Assert.NotNull(form, "a insights screen describes the settings a stream is running on");
        Assert.NotNull(session, "a insights screen renders the session's running state");
        Assert.NotNull(dispatch, "a insights screen needs a UI loop to marshal an answer back to");
        ...
        Config.EditRequested += () => Request(InsightsAction.EditInSetup);
    ...
    /// </summary>
    public Dock ActionsDock => SideColumns.InsightsActions.FitsBeside(_window) ? Dock.Right : Dock.Bottom;
    ...
    public void Apply()
    {
        var reading = _held.Fill(InsightsSnapshot.Of(_session.Publish, _session.Stats, _session.Relay));
        Snapshot = reading;
    ...
    {
        var path = InsightsSnapshot.PathOf(relay, stream);
        if (path is null)
        {
            return [];
        }
    ...
    {
        Request(InsightsAction.OpenFullLog);
    ...
    private void Request(InsightsAction action)
    {
