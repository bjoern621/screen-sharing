...
/// <summary>
/// The answer where the system cannot keep a window out of a capture, which is everything but Windows.
///
/// X11 serves the root window to any client that asks, so a capture is a read of the whole screen and there is
/// no per-window fact to set.
...
/// from a window rule in their own configuration, which is the user's to write rather than this app's to ask
/// for, and the Avalonia Wayland backend sends no <c>xdg_toplevel.set_app_id</c>, so such a rule matches this
/// window in an X11 session alone (avalonia/README.md).
///
