...
/// menu reach the platform through the markup rather than through a press handler here.
///
/// <see cref="DropFocus"/> is the one thing that is not markup, and it is the window's because focus is: a
/// text box holds the caret and the selection until something takes them, and most of the surface on every
/// screen here is a panel that takes nothing.
/// </summary>
public sealed partial class ShellWindow : Window
{
    public ShellWindow()
    {
        ...
        //
        // Whether the band standing in for the caption is drawn is the shell's to write, because it is also
        // off while a stream fills the window (ShellViewModel.HasCaption).
    ...
    ///
    /// The whole window rather than the tiles in it: a tile draws a decode of the screen the window stands
    /// on, so a window left in the capture nests one more copy of the screen per round trip.
    ...
    /// A press inside one is left alone, since the box moves focus itself.
    /// </summary>
