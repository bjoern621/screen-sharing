...
///
/// What the answer decides is a subscription and never a decode: the preview pipeline goes up with the
/// publish child and down with it, so a card nobody is looking at stops drawing and the stream is untouched
/// (<c>docs/viewer-architecture.md</c>, "What the broadcast preview draws").
///
/// The data context is watched as well as the tree, because the two arrive in no fixed order and a template
/// can hand this view a different card.
/// Whichever view model is being left is told it is no longer showing, which is what keeps frames from being
/// lent to a card nothing is drawing.
/// </summary>
public sealed partial class PreviewView : UserControl
{
    /// <summary>
    /// Whether the card is on screen in a window that is in front.
    /// Both halves are facts only a control and the platform can answer, and both are answered in one place
    /// because the wizard's screen picker asks the same question (<see cref="ShowingWatch"/>).
    /// </summary>
    ...
    /// <summary>
    /// Tells the card how large it is drawing the picture, which is the other fact the markup cannot state.
    ///
    ...
    /// <summary>
    /// Tells the card whether it is being looked at.
