...
public sealed class ConfigCardViewModel : Observable
{
    ...
    /// <summary>
    /// What the pipeline reports it is running: one row per group of the form resolved against
    /// the settings the running pipeline was built from. An input rather than an output, since
    /// nobody writes it from this screen.
    /// </summary>
    ...
    /// <summary>
    /// The one render function. Rows are records, so a second pass over the same reported
    /// configuration leaves the bound list untouched.
    /// </summary>
    public void Apply()
    {
