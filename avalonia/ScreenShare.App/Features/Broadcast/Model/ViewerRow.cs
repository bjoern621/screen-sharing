...
///
/// <b>Filled from the relay's reader roster, cell for cell.</b> The relay names each reader by the protocol
/// it watches over and joins it to that protocol's connection list, so a row is an address, a join time and
/// whatever the leg is instrumented for.
/// SRT is the one leg the relay times a round trip and states a loss rate on; the rest report what was sent
/// to them and what the relay's own queue threw away.
/// A cell with no measurement behind it prints <see cref="Figure.NoValue"/> and never a zero, so a viewer on
/// an untimed leg never reads as a perfect one.
///
/// <b>Two of the design's columns name figures the relay does not have, and carry what it does measure
/// instead.</b> Buffer fill and the decoder in use are the viewer's own facts and reach no publisher, so a
/// column for either prints an ellipsis in every row forever.
...
///
/// <see cref="IsLast"/> is nobody's measurement: the render function derives it, because the last row sits
/// flush against the card's rounded edge and carries no separator.
/// </summary>
    ...
    bool IsStruggling)
{
    ...
    /// SRT states resent data against sent data, so the figure is already a rate, and a link losing a
    ...
    /// Distance alone is not trouble, but SRT's retransmit window is a small multiple of the round trip, and
    /// past this the relay resends into a gap that has already played.
    /// </summary>
    private const double StrainingRttMs = 200;

    /// <summary>
    /// One discard, which is the whole threshold.
    /// A packet the sender gave up on and a frame the relay's queue threw away are both data this viewer was
    /// never sent, and either is a picture that did not arrive.
    /// </summary>
    ...
    /// follows: the escalation lands in four places in the markup, fill, weight, the hot roles and the one
    /// red, and a rule restated at each is four rules that can disagree
    /// (<c>docs/development-principles.md</c>, "Stateless").
    /// The view asks this row whether it is struggling and asks nothing else.
    ///
    /// A figure the leg does not report takes part in nothing, which is why each entry reads presence rather
    /// than value: a viewer over RTMP is untimed and not calm, and an absent round trip read as a zero
    /// certifies every one of them.
    /// </summary>
    ...
    /// Nothing here is decided: a cell is the relay's figure for that reader, formatted, or the ellipsis that
    /// says the leg states none.
    /// </summary>
    /// <remarks>
    /// The two identifying cells fall back and then give up rather than asserting, and do it in
    /// <see cref="Readers"/>, since the session log names the same readers
    ...
    /// discarded.
    /// Summed rather than shown apart, since the column asks one question, did anything fail to go out, and a
    /// leg reports at most one of the two.
    ...
    ///
    /// An age reads better and is not used: this runs on every render pass, and an age would come from a
    /// clock nobody handed it, so one roster would render differently on two consecutive passes and repaint
