...
///
/// <see cref="Extent"/> is that space, 480 by 104: X runs across <see cref="WindowSeconds"/>, Y runs 0 at the
/// top to 104 at the bottom.
/// Design coordinates are what let a wider card stretch the same window instead of covering a longer one.
///
/// Three series, three clocks.
/// Egress is placed by the encoder's own running time.
/// The two relay curves are placed by when this shell received the snapshot, since the relay stamps none and
/// its poll interval is not on the contract.
///
/// Each latency point is the worst reader on the path at that snapshot, and worst by round trip and worst by
/// loss can be two different viewers.
///
...
public static class PlotSeries
{
    ...
    /// Newest reading at the right edge, one this old at the left, so a younger run fills the right of the
    /// card and leaves the rest empty rather than stretching over it.
    ...
    /// A single sample draws nothing: one point is a reading, and a line through it would read as a flat
    /// stream.
    /// </summary>
    ...
    /// nobody measured.
    /// SRT is the leg that reports either figure, so one filter serves both curves.
    /// </summary>
    ...
    ///
    /// Placed against the same peak the curve is scaled to, since a rule at a fixed height would mark the
    /// ceiling only by coincidence.
    ...
    /// <summary>
    /// The window's readings in drawing space: placed horizontally by when they were taken, scaled vertically
    /// to their own peak.
    ///
    ...
    private static IReadOnlyList<Point> Curve(IReadOnlyList<Reading> readings, double? now)
    {
        ...
        {
            // Right edge is the anchor, left edge one window before it, so spacing is elapsed time and not a
            // share of however many readings there are.
            ...
            // A window whose peak is zero draws along the floor instead of dividing, the right shape for a
            // loss curve that stayed at nothing.
    ...
    /// carries.
    /// A relaunched pipeline leaves the stream live, so its samples are appended to the previous pipeline's
    ...
    /// <summary>
    /// End of the egress axis: running time of the newest sample that states one, null where none does.
    ...
    /// A sample carrying no time contributes nothing either, since a rate with no moment cannot be placed on
    /// the axis.
    /// </summary>
