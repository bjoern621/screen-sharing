...
///
/// Boundaries accumulate along the row and each lands on a whole device pixel, so arranging a chip between two of
/// them rounds nothing outward and the last chip ends inside the panel.
/// One share rounded and multiplied instead ends the row up to a pixel per chip past that edge, and a panel
/// clipping to its bounds takes the overhang out of the last chip's border rather than out of the gap behind it.
