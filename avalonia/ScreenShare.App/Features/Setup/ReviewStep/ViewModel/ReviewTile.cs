...
/// Not key/value rows, the fields naming themselves (<c>latency 120 ms</c>, <c>cq 21</c>):
/// a key column would print the word twice and halve the width left for the value.
///
/// A record whose <see cref="Edit"/> is the owner's own command instance,
/// so two passes over an unchanged review compare equal.
/// </summary>
...
/// The shorthand is <c>FieldGroup.summary</c>, the same sentence the strip's chip repeats, so the review
/// and the strip cannot disagree about what a step settled on.
/// </summary>
