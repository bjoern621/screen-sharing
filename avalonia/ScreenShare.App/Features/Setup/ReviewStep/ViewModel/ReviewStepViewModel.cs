...
///
/// <b>It composes the preset card</b> (<see cref="Presets"/>), which is the one thing on this screen that is
/// neither a reading of the settings nor the commit.
/// It sits here because a preset is the whole way of publishing, and this is the screen where the whole way
/// of publishing is read back: naming one and picking one belong beside that summary rather than on a step
/// that owns a fraction of it.
///
/// <b>Outputs</b> are written by <see cref="Apply"/> alone, and every one of them comes off a state some
/// other side stated.
/// The tiles are the groups' own shorthands, the list is the form's diagnostics, the name is the draft's, and
/// whether the button lights is the <see cref="PublishGate"/> - which reads <c>Form.publishable</c>, what is
/// publishing and what the relay said, so the button and the refusal that would follow it cannot disagree.
///
/// <b>What the button says is the same gate's answer, and that is the point of it being one.</b> A stream
/// already on the air no longer locks the commit; it decides which effect the press is
/// (<see cref="PublishGate.Commit"/>), and the label and the sentence under it are that answer read out of
/// one table.
/// The alternative was a ternary at the binding site and a second one beside the call, which is one fact
...
public sealed class ReviewStepViewModel : Observable
{
    ...
    /// <param name="edit">Hands a command that moves the flow to one step.
    /// The review edits nothing itself.</param> <param name="back">Moves to the step before this one, which
    /// is the flow's own answer rather than a key held here.</param>
    /// <param name="goLive">
    /// What committing means, and it answers when the backend has answered.
    /// Owned above this view model - there is no publisher here to call, and starting one is an effect on the
        ...
        Action<Action> dispatch)
    {
        ...
        // button waits rather than going inert: the round trip is long enough for a reader to press again,
        // and the command is what refuses that.
    ...
    /// step only decides where on the screen it sits and renders it on every pass.
    /// </summary>
    public PresetsViewModel Presets { get; }

    /// <summary>
    /// The commit.
    /// Its own in-flight field is what the button draws its wait from, so the spinner on screen and the press
    /// the command would refuse are one fact.
    /// </summary>
    ...
    /// The gate's own answer, so the one red button and the refusal that would follow it are one decision.
    /// </summary>
    public bool CanStartSharing { get => _canStartSharing; private set => Set(ref _canStartSharing, value); }

    /// <summary>
    /// What the button says it will do: start a stream, or restart the one already on the air on these
    /// settings.
    /// It comes off the gate's <see cref="PublishGate.Commit"/> through one table (<see cref="CommitCopy"/>),
    /// so the word on the control and the call the press makes are the same answer read twice rather than two
    /// answers that happen to agree.
    /// </summary>
    public string CommitLabel { get => _commitLabel; private set => Set(ref _commitLabel, value); }

    /// <summary>
    /// The promise under the button, up to the stream name.
    /// It is two halves because the name sits inside the sentence at full strength, and the sentence itself
    /// is what tells a reader that applying restarts the stream rather than changing it under the viewers.
    /// </summary>
    ...
    /// It is never this step's own prose about a setting: the settings' own blockers are in
    /// <see cref="Checks"/>, in the words the backend wrote them in.
    /// </summary>
    ...
    /// <summary>
    /// The backend's own sentence when it refused the last start, empty otherwise.
    /// A refusal is about an attempt rather than about a precondition, which is why it is a second line and
    ...
    /// <summary>
    /// The stream's path on the relay, as the draft carries it.
    /// It used to be a name out of the mockups printed under every configuration, which said the same thing
    ...
    /// Idempotent: the rows are rebuilt from the arguments and reconciled, and two passes over one form
    /// produce rows that compare equal.
    /// </summary>
        ...
        IReadOnlyList<PreflightCheckRow> checks)
    {
        ...
        // a stream that ended has to take the word "restart" off the button with it, and a property written
        // only in the apply branch is one that sticks.
        ...
        // The card draws from the draft and from the store, neither of which this step holds, so it is
        // rendered rather than fed: what it reads has moved by the time this pass runs.
        Presets.Apply();
