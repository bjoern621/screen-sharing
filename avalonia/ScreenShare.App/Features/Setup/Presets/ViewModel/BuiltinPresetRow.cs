...
/// <summary>
/// One built-in preset as the card draws it: its name, whether the draft delivers its promise,
/// and whether this machine reaches it.
///
...
/// A built-in one is a promise about the picture, and which encoder, pixel format
/// and capture backend deliver it here is the backend's search rather than anything stored (<c>docs/presets.md</c>).
///
/// A record whose command is the owner's own instance, so two passes over one form produce rows that compare equal
/// and the list is left alone (<c>Mvvm/Reconcile.cs</c>).
/// </summary>
public sealed record BuiltinPresetRow
{
    ...
    /// A false row keeps its place and its reason rather than disappearing,
    /// the treatment every other ruled-out choice gets (<c>docs/field-availability.md</c>).
    /// </summary>
