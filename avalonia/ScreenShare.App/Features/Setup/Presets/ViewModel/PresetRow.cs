...
///
/// A record whose commands are the owner's own instances,
/// so two passes over an unchanged store produce rows that compare equal and the list is left alone
/// (<c>Mvvm/Reconcile.cs</c>).
/// </summary>
public sealed record PresetRow
{
    ...
    /// A snapshot claims nothing about a region of the settings, so selected can only mean equal:
    /// an edit moving one field unmarks the row, and no stored selection is left to disagree with the settings
