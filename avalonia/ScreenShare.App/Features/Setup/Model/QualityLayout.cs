...
/// A field in neither is a setting the backend offered that nobody can reach, and a field in both is one
/// setting edited from two places.
///
/// The split is by control kind rather than by key, so a field the form adds to the group lands in whichever
/// half suits its widget with no line changing here.
/// The two exceptions are named because the step draws them as cards and a banded track, which a generic
/// renderer has no shape for, and choosing where a field sits is placement (docs/ipc-api.md, "The rule").
/// </summary>
public static class QualityLayout
{
    /// <summary>
    /// The one group drawn by a layout of its own rather than the generic renderer.
    /// Naming it is placement: what the group holds, what its controls are called and which are greyed stays
    /// the form's answer, and a group the backend renames falls back to the generic renderer.
    /// </summary>
    ...
    /// <summary>
    /// Whether the step places this control: the two it lays out by name, and every field whose whole control
    /// is a choice, which the read-back row draws as a dropdown.
    ...
    /// <summary>
    /// Complement of <see cref="OnStep"/>, so between them the group's fields are drawn exactly once.
    /// </summary>
    public static bool InDrawer(FieldViewModel field) => !OnStep(field);

    /// <summary>
    /// Whether the read-back row draws it: an options field the step does not lay out by name.
    /// </summary>
    ...
    ///
    /// Squared off so an option's paragraph gets a third of the column rather than a fifth: five modes across
    /// one row set each explanation thirty characters wide, which reads as a column of fragments.
    ///
    /// Stated here rather than left to the panel because a UniformGrid asked for neither dimension squares off
    /// <b>both</b>: five options become a three by three, and the row under the cards is a card's worth of
    /// empty column.
