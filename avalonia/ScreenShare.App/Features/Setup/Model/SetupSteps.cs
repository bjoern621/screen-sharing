...
/// <summary>
/// One step, as a row of facts: which group of the resolved form it draws, its place in the strip, and what
/// the chip calls it.
/// </summary>
...
///
/// <b>The list is not written down here, and that is the fix rather than an omission.</b> It used to be a
/// static table of seven rows, each naming the group key it drew - and three of those keys named groups the
/// backend does not answer with, so three steps of the wizard drew an empty column for as long as nobody
/// clicked them.
/// The four groups the backend does answer with and the table did not name were unreachable at the same time.
/// That is the fork docs/ipc-api.md exists to prevent, in the one place the shell was still allowed to hold a
/// list: which steps exist is which groups the form carries, so a group added to the contract is a step that
/// appears and works with nothing here to edit.
///
/// What the shell still owns is placement, which is what the contract leaves it: the order is the form's, one
/// group is drawn by a layout of its own (<see cref="QualityLayout"/>), one is drawn by another destination
/// entirely (<see cref="Fields.Model.GroupPlacement"/>, and the caller is what applies that filter), and the
/// terminal step is appended because committing is not a group of settings.
/// </summary>
public static class SetupSteps
{
    /// <summary>
    /// The terminal step's key.
    /// It is not a group key and must never collide with one, which <see cref="For"/> asserts rather than
    /// assumes - a backend that grew a group called this would otherwise produce two steps that claim the
    /// same identity.
    /// </summary>
    ...
    /// <summary>
    /// The steps for one resolved form: a step per group in the form's own order, then the terminal one.
    ///
    /// Empty for a form that has not arrived.
    /// A strip of chips naming steps nothing has described yet would be the shell asserting a shape it has
    /// not been told - and the sentence saying why there is no form is already above the column.
    /// </summary>
    public static IReadOnlyList<SetupStepRow> For(IReadOnlyList<FieldGroup> groups)
    {
        ...
        foreach (var group in groups)
        {
            ...
            rows.Add(new SetupStepRow
            {
                ...
                // cannot see, so what fits in it is decided here.
    ...
    /// <summary>
    /// The step this key names, or null where the current form has none.
