...
/// <summary>
/// Quality group of the resolved form, laid out the way this screen is designed instead of by the generic
/// renderer.
///
/// No state of its own.
/// Which rate-control modes exist, what each is called, which end of the quantizer scale keeps more detail, which
/// resolutions this source scales to, which frame rates the panel produces and what a keyframe interval works out
/// to all arrive through <see cref="FieldGroupViewModel"/> already decided (docs/ipc-api.md, "The rule").
/// Every write leaves through the field the reader moved.
///
/// This class's is placement, which the contract leaves to the shell: cards for the rate control, each option
/// carrying a paragraph; the banded track for the quantizer, its scale having named zones; every other select in
/// the read-back row at the foot.
...
public sealed class QualityStepViewModel : Observable
{
    ...
    public QualityStepViewModel(FieldGroupViewModel group)
    {
        ...
        // The group is rendered by the flow owning the draft, so a change arrives as a notification and never as a
        // copy held here (docs/development-principles.md, "State is written explicitly and read continuously").
    ...
    /// drawn rather than drawn empty: an unreachable backend is reported once, above the column.
    /// </summary>
    ...
    /// The scale arrives with the field and differs per codec and engine, so the numbers are read off the range
    /// the slider is bound to rather than written into the markup
    ...
    /// parting company as soon as the availability pass refuses an entry: a count including the hidden ones opens
    /// a column the last row has no card for, the empty row <c>CardColumns</c> asserts against.
    /// </summary>
    public int ModeColumns { get => _modeColumns; private set => Set(ref _modeColumns, value); }

    /// <summary>
    /// The one render function.
    /// Safe to run twice: the group hands back the same field view models by key, so an unchanged pass assigns the
    /// same references, reconciles onto an equal list and fires no binding.
    /// </summary>
    public void Apply()
    {
    ...
    /// The two subscriptions the constructor takes do not cover this one path: pressing the control's own reveal
    /// writes <see cref="FieldViewModel.RefusedShown"/>, which rebuilds <see cref="FieldViewModel.Shown"/> without
    /// the group's properties or its field list moving, so nothing would call the render function and the column
    /// count would be left over from the list before the press.
    /// The collection is listened to rather than the flag beside it, the flag being announced before the list it
    /// rebuilds.
    /// </summary>
    ...
    /// A group carrying no quantizer clears them, so nothing left over from the codec before is drawn beside the
    /// next one.
    /// </summary>
    ...
    /// <summary>
    /// Read-back row's controls: every options field the group offers, in the form's order, minus the two this
