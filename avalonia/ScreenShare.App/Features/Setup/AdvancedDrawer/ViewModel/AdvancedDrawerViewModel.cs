...
/// <summary>
/// The rest of the quality group, behind a drawer because that is where an expert expects the raw knobs to
/// be: the step above it is the answer for everyone else, and this is the same group's remaining settings
/// stated as a table.
///
/// <b>It holds no state of its own but the drawer's own open flag.</b> Which rows exist, what each is called,
/// what it means, which are greyed and why are the backend's answers, arriving through
...
///
/// It once carried a table of its own - an element name, five property rows, their defaults and a changed
/// count - seeded from a mockup while there was no backend behind it.
/// All of it is gone rather than kept alongside the real fields: a screen that prints a value the backend
/// never reported is a screen that lies, and the reader cannot tell which half is which.
/// What the form does not carry, this drawer no longer shows.
///
/// <b>Inputs</b>: <see cref="IsOpen"/>, a named write that <see cref="Apply"/> reads and never assigns.
/// </summary>
public sealed class AdvancedDrawerViewModel : Observable
{
    ...
    public AdvancedDrawerViewModel(FieldGroupViewModel group)
    {
        ...
        // Rendered off the notification rather than off a copy, the same way the step above reads the same
        // group (docs/development-principles.md, "State is written explicitly and read continuously").
    ...
    /// Held here rather than in the view because the drawer is a piece of state the reader set, not a
    /// widget's mood.
    /// </summary>
    public bool IsOpen
    {
    ...
    /// <summary>
    /// How many settings are in here, so a closed drawer says what it is hiding.
    /// Derived from the rows rather than stored, so the header cannot claim a count the table below it does
    /// not show.
    /// </summary>
    ...
    /// False is the honest state before the first resolve and for a group whose every field the step places
    /// itself, and the drawer is not drawn at all rather than drawn empty.
    /// </summary>
    ...
    /// reconciles onto an equal list and no binding fires.
    /// </summary>
    public void Apply()
    {
    ...
    /// <summary>
    /// The rows: every field of the group the step above places nowhere, in the order the form gave them.
