...
public static class Fields
{
    ...
    private static readonly Dictionary<string, Entry> Entries = new()
    {
        ...
        ["relay.group_key"] = new(
            "Group key",
            "The secret that decides who can watch. Everyone holding it sees your streams, and nobody else does. Share it like a meeting link. Change it to cut someone off. Left empty, anyone who knows the relay address can watch."),

        ["relay.display_name"] = new(
            "Name in the group",
            "What this computer goes by in the group: its row in the member list, and the name beside each stream it publishes. The first claim on a name holds it, so a name another member uses cannot be taken. Joining a group needs one."),
