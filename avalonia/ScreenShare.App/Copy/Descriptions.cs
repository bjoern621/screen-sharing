...
/// <summary>
/// Paragraph behind one choice: what it is, what it costs, and when to pick it.
...
/// Say the trade and not the definition: "4:2:0 keeps a quarter of the colour" is the definition, "coloured text
/// and edges smear" is what the reader will see, and both belong in that order.
/// Name what a choice needs before it is picked, the backend greying only what it can prove.
...
public static class Descriptions
{
    private static readonly Dictionary<string, string> Captures = new()
    {
        ...
        ["portal"] = "Asks the desktop to show its own picker, so the monitor or window is chosen there and the desktop consents. The one that works on Wayland, and it works on X11 too. Fewer pixel formats and rate-control knobs than the ffmpeg backends. The fields say so where it matters.",
    ...
    private static readonly Dictionary<string, string> Memories = new()
    {
        ...
        ["gpu"] = "Hand the encoder the frames the capture already produced: no copy out, no CPU conversion, no copy back. It also demands the colour chosen here, so it is refused where the pair has no shared path and where their shared path cannot convert.",
        ["gpu-encoder-color"] = "Keep the frames on the GPU even where nothing on the way can convert them: the encoder reads the captured surface as it is and picks the colour itself. The pixel format and colour range chosen here stop applying: both fields grey and say what the encoder will send instead.",
    ...
    {
        ["software"] = "Encodes on the CPU. Needs no particular graphics card and works everywhere. At a full desktop resolution and a high frame rate it will use a large part of the machine, more so for the newer formats.",
    ...
    {
        ["cbr"] = "The encoder holds the target every second, so the bandwidth is fixed and the quality moves with the scene. The right choice when the connection has a hard cap that cannot be exceeded.",
        ...
        ["crf"] = "The encoder spends whatever it takes to hold the quality set here. The bitrate rises when the screen moves and falls to almost nothing when it is still. Set a ceiling to keep the busy moments inside a connection's budget: the picture softens there instead of the stream falling behind.",
    ...
    {
        ["none"] = "",
        ["desktop"] = "Everything the machine plays, mixed into the stream as a second track. Viewers hear it without doing anything. Pick the output on the row beside it where this machine plays into more than one.",
        ["application"] = "One running program's own sound and nothing else, which is what a stream carrying a game and not the call about it needs. Pick the program on the row beside it.",
    };
    ...
    /// <summary>
    /// What each tune aims at, and never whether it is good: which one is right follows from the content and the
    /// delay budget, both the reader's.
    /// </summary>
    private static readonly Dictionary<string, string> Tunes = new()
    {
        ...
        ["vq"] = "Weighs what a person watching would notice, rather than a score. The one to leave in place for a picture somebody is looking at.",
        ["iq"] = "Aims at how a still frame reads, and takes over the sharpening, the quantisation tables and the screen-content tools to get there.",
        ...
        ["displayremoting"] = "Tells the encoder it is coding a desktop watched somewhere else, which is what this app does. The fitting answer here.",
        ["videoconference"] = "For faces over a static background, where the picture holds still and one region moves.",
        ["archive"] = "For an encode nobody is waiting on, which trades delay for the smallest file.",
        ["livestreaming"] = "For a broadcast to many viewers, which holds a steady rate over a long run.",
        ...
        ["remotegaming"] = "For rendered motion somebody is playing through, which puts delay above everything else.",
    };

    private static readonly Dictionary<string, string> Transports = new()
    {
        ["srt"] = "UDP that asks for lost packets again, within a delay window set here. One connection carries everything in both directions, so it asks no more of a home router than any other outgoing connection.",
    ...
    /// Where the frames are converted decides the cost; whether the route states its colour decides whether dark
    /// content survives it.
    /// </summary>
