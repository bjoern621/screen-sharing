...
/// <summary>
/// The quality step, driven end to end: a backend describing a group, the flow adopting the
/// draft it answers with, and the controls the step draws from it.
///
/// What these lock out is the shell deciding anything. Every assertion below is made through
/// the properties the markup binds, and none of them names a value this module wrote: the
/// entries, their labels, their trailing notes and their greying all come out of the resolved
/// form, so a shell that started inventing a list would fail here rather than on someone's
/// screen (docs/ipc-api.md, "The rule").
///
/// Every test awaits, because the seam is a round trip: a draft change asks the backend for a
/// form and the answer lands on a later pass. The stand-in answers from memory and the
/// dispatcher below runs inline, so the wait is over before it starts - but waiting is what
/// the assertion is entitled to do, and a test that read the controls without it would be
/// asserting against a timing the real client does not have.
/// </summary>
public sealed class QualityStepTests
{
    /// <summary>
    /// A flow whose first form has landed. The dispatcher runs the answer straight through,
    /// so what a test reads afterwards is what the render pass wrote.
    /// </summary>
    ...
    /// <summary>
    /// The case the screenshot showed: a control that printed a value and opened nothing. It
    /// opens now, and what it opens onto is the backend's list.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The trailing note is what makes an entry honest: it names what the value was derived
    /// from, so the reader sees the cost without opening the step that owns the source.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A select over a number is the case a string-only write gets silently wrong: it would
    /// mark one entry and store zero. The frame rate is that case.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A frame rate above the screen's own refresh rate stays offered. The capture has no
    /// new picture for the frames in between and the encoder codes repeats of the last one,
    /// which costs bandwidth and buys no motion - but it is a legal thing to ask for, so the
    /// form says so as a diagnostic rather than taking the entry away.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The greying the shell renders without knowing that a mode and a quantizer have anything
    /// to do with each other: the backend says so, and the same control draws both answers.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Rendering twice over an unchanged draft produces the same rows, which is what lets an
    /// open menu survive a pass (docs/development-principles.md, "Idempotency").
    /// </summary>
    [Fact]
