...
// The colour description travels in the first parameter set, so one frame would carry it.
...
// The publish side pins four enum values and a decoder reports the same colour space by name, so
// limited-range BT.709 goes in as 2:3:5:1 and comes back as bt709, which is what the grid's stats
// card shows.
// A pinned colorimetry absent here is expected back verbatim, which is what full-range BT.709 does.
var gstColorimetryNames = map[string]string{
	gstRangeLimited + ":" + gstBt709: "bt709",
}

// gstSinkPadCaps matches the line gst-launch -v prints for the caps that reached the sink, and
// gstColorimetryField the single field of it read here.
var (
...
func TestPublishedColorimetryReachesTheDecoder(t *testing.T) {
	...
	for codec := range gstCodecs {
		...
		for _, chain := range gstRoundTripChains(t, cap.Family) {
			// The element is asked for per chain, a family whose plugin ships one per memory kind encoding
			// through a different one on each.
			// Asked for the other, the skip below would read as a missing plugin where the run would have
			// launched an element this install carries, or the reverse.
			...
			for _, colorRange := range []string{"pc", "tv"} {
				t.Run(codec+"/"+chain.memory+"/"+colorRange, func(t *testing.T) {
					...
					if !signalled {
						// The stream states nothing, so it is watched in the viewer's own default.
						// That matches only where the publish happens to be in it, which is what the colour-range
						// gaps are about.
...
const (
	...
	// whitePatchPattern is videotestsrc's flat white field: every channel at its maximum, the one
	// input whose stored luma is the range and nothing else.
	whitePatchPattern = "white"
)

// roundTripPattern is what the colorimetry round trip codes: videotestsrc's colour bars, its own
// default.
// What the frames hold does not decide what the stream says it holds, so the pattern is named once
// rather than chosen per codec.
...
// would show.
// The last frame, because the first is the one an encoder codes alone, and a range applied to it
...
func gstDecodedLuma(t *testing.T, s settings.Settings, stream string) int {
	...
	// 4:2:0 stores the luma plane whole and each chroma plane at a quarter of it.
...
// names, the memory demanded of it, and what that demand resolves to.
...
// gstRoundTripChains returns every chain this engine reaches a family's encoders through: system
// memory, which every pair has, plus one per device row the pair table declares.
//
// The exact-colour rows alone are taken.
// A row that lets the encoder convert states the colour it produces in Signalled and is measured
// against that rather than against the settings, which is the ffmpeg engine's round trip; this
// engine carries no such row.
func gstRoundTripChains(t *testing.T, family string) []roundTripChain {
	t.Helper()
	// No capture backend on the system chain: the memory setting alone decides that path, and naming
	// one would tie what this covers to whichever backend the defaults carry.
...
// these settings publish through, and returns the file holding the elementary stream.
// That chain is the family's upload where its converter takes no system frames, the conversion,
// the encoder-input caps, the encoder and its link.
//
...
func roundTripSettings(t *testing.T, codec, colorRange string, chain roundTripChain) settings.Settings {
	...
	// The quantizer target rides each encoder's own scale, and the default settings carry one off
	// another codec's.
	s.Publish.Cq = c.CqMaxOn(EngineGst) / 2
	// An encoder with a bitrate ceiling refuses a target above it outright, and the default settings
	// carry one above it.
...
// A codec whose encoder needs no parser and a format whose file needs no framing element each leave
// nothing between their neighbours.
...
// runGst plays one pipeline to its end and returns everything it printed.
...
// decodedColorimetry reads the colorimetry off the caps that reached the sink, and reports false
// where the decoded frames carry no such field: a decoder that read no colour description states
