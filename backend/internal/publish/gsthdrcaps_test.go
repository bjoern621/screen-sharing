...
// Which colours the encoder input accepts follows from the pixel format: an HDR surface has one way
// through, the only format this app encodes at more than eight bits per component.
//
// The child narrows those rows to the surface's own colour (gstrun/surface.go).
// Held here is the other half: a publish states the rows a surface could be,
// and no row an 8-bit encode could not carry.

// hdrCapsSettings is a draft this engine builds a pipeline for, at the given pixel format.
...
func TestTheEncoderInputOffersTheHdrColoursOnTheTenBitFormatAlone(t *testing.T) {
	...
	// Standard range leads: it is what a capture stating no transfer is,
	// and what a run nobody narrows ends on.
	...
	// so offering the rows here would offer a negotiation ending in a stream tagged PQ over eight-bit
	// samples.
...
// The range decides how a viewer expands the picture, so a row dropping it would leave one
// negotiable outcome signalling what the settings never asked for.
...
// Structures rather than one value list, which is the difference between a choice and a conversion:
// videoconvert fixates a list to its first entry whatever the frames carry, so a list would convert
// every HDR surface into the standard-range row and call it negotiation.
