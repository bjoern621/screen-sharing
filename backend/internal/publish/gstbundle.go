...
// played and a count read off it, which is all gst-launch does.
...
//
// Spawning the binary that is already running keeps the pipeline in a process of its own without a
// second artifact to build, sign, ship and find at runtime.
...
//
// gstbundle answers where the plugins are, since internal/receive asks that of the same
// installation.
// Nothing is set on this process: a publish pipeline links no GStreamer, so the variable belongs to
// the child that does.
