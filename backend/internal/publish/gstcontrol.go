...
type gstHandle struct {
	...
	// mu guards conn, which an apply dials on first use and a failed write drops.
	mu     sync.Mutex
	socket string
	conn   net.Conn
}
...
// Stop closes the control connection, then stops the child behind it.
...
// so it belongs with the sockets that die with theirs.
// The name carries the supervisor's tag for the run and this process's id, so two backends on one
// machine take separate paths.
...
// gstLiveArgs give a run its control socket, and nothing to a run that takes no values: the encode
// probe and the test streams measure a pipeline rather than carry one, so nothing talks to them.
...
// so at the call site rather than a table saying it elsewhere.
type LiveApplier interface {
	ApplyLive(s settings.Settings) error
}

// Live returns the applier where this handle's pipeline takes values while it runs.
