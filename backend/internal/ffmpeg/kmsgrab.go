...
// Mapping the frame through a hwdevice that understands the modifier first makes it readable,
// and which device does depends on the GPU.
// The strategy is therefore selectable,
// and this table is the one source for what the ffmpeg vf chain becomes.
type DrmMap struct {
	...
	// Auto reads Device off the capture device's kernel driver
	// at runtime rather than taking the fixed one above.
	Auto bool `json:"auto"`
}
...
// The setting exists to override the driver guess, so resolving a misspelled "vaapi" to auto
// would run the strategy the user chose against, and the frame then either arrives mapped
...
// a machine with no DRM node would otherwise answer
// a misspelled strategy with its own missing hardware.
...
// drmMapNames lists the strategy names a refusal ends in,
// so no message can name a set the table does not hold.
...
// Intel and AMD expose the scanout modifier through VAAPI; everything else, NVIDIA
// and unknown drivers included, goes through Vulkan, the cross-vendor DRM interop path.
...
// through the capability wrapper (FindCaptureExe) or under sudo.
//
...
// and the GPU path downloads nothing: the map that follows targets the encoder's own device
// and comes off the encoder family rather than from the user (GpuFilters).
// The setting is unread rather than overridden, so the form greys it.
func kmsgrabCaptureArgs(s settings.Settings, fps, memory string) (captureSource, error) {
	...
	// The strategy is held against the table before the machine is looked at,
	// so a name no row carries is answered with the setting
	// rather than with whatever the sysfs probe finds.
...
// drmCaptureDevice returns the /dev/dri card node kmsgrab captures from:
// the first card whose kernel driver is neither absent nor the boot framebuffer.
//
// No default is taken, card0 not being a safe one.
// On a machine with a discrete GPU the boot framebuffer often holds card0
// under the simple-framebuffer driver while the real display controller lands on card1 or higher,
// so naming card0 when the probe finds nothing hands kmsgrab a node with no scanout to read,
// and the failure then reads as a broken capture rather than as a machine with no display
