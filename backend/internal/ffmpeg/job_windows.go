...
// The one Job Object every media child is put into, created on the first assignment
...
//
// JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE is the whole point of it:
// the job terminates every process still in it as soon as the last handle to the job closes.
// The only handle is this process's, so the children go when this process goes,
// by whatever means it went.
//
// A job that cannot be created or configured is an Umgebungsfehler the app runs past:
// every child starts, and a normal quit stops each one through App.shutdown.
// What is lost is the guarantee for the exits that never reach it,
// which the warning names rather than the API that refused.
func ensureJob() {
	jobOnce.Do(func() {
		...
		if err != nil {
			// Without the limit the job is a grouping that kills nothing,
			// so the handle goes rather than being kept and relied on.
			windows.CloseHandle(handle)
			logger.Warnf("job object took no kill-on-close limit, so an app killed outright can leave its children running: %v", err)
			return
		}

		// Never closed: closing this handle is what kills the children,
...
//
// App.shutdown stays the path that stops children on a normal quit,
// and is the faster and quieter one.
...
// from Task Manager, and a publish ffmpeg left behind by that keeps capturing the screen
// into the relay with no window left to stop it from.
//
// Children are assigned one at a time rather than by putting this process in the job
// and letting them inherit it, which would be less code
// and would also cover the short-lived capability probes.
// Inheritance would take in every process the app starts, and system.go's openInShell starts
// processes meant to outlive it: the file browser or editor opened on a run log would be killed
// the moment the user quits.
//
// Called after cmd.Start and before the goroutine that waits on the child.
// The assignment goes by pid, and the pid names only this child for as long as os/exec holds
// its process handle open, so Windows hands the number to nothing else.
// Putting a stranger into a job that kills its members
// on this process's exit is the failure that ordering rules out.
//
// A failed assignment is reported and survivable on ensureJob's reasoning: the child runs,
// only its crash guarantee is lost.
...
func KillOnAppExit(cmd *exec.Cmd) {
	...
	// AssignProcessToJobObject needs both: the job accounts for the child's quotas
	// and has to be able to terminate it.
	...
	if err != nil {
		// A child that exited between Start and here is refused like one the job cannot take,
