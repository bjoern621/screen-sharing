// Package ffmpeg builds the publish command line, locates the media executables
...
// ffmpeg builds one encoder per output,
// so two outputs written the ordinary way put two encoders on one capture.
// The tee muxer is the one shape that writes one encoder's packets to several muxers.
//
// Options are the slave's own, already in the tee muxer's key=value spelling,
// and the caller's business: what to select, which muxer, and whatever that muxer needs.
...
// Any tap makes the command a tee: automatic stream selection does not apply to a tee,
// so the streams are mapped by hand,
// and the transport's own output arguments render as a slave of it beside every tap.
func BuildPublishArgs(s settings.Settings, taps []Tap) ([]string, error) {
	...
	// Every grabber takes the rate as an input option and gopFor derives the keyframe interval
	// from it, so a non-positive rate reaches the command as "-framerate 0" and "-g 0".
	...
	// Frames never become software ones on the GPU path, so the map
	// and the device-side conversion replace the colour tag, the upload and the device option at once:
	// the conversion states the colour it wrote,
	// and the encoder takes its device from the frames handed to it (gpu.go).
	var device []string
	onDevice := gpupath.OnDevice(memory)
	surface := onDevice
	if surface {
		// The device-side conversion carries the scale, since swscale cannot read frames already
		// on the device and no software stage is left to put one in.
		...
		// and the upload pins a surface layout,
		// so a later scale would resize a picture both had already been told the shape of.
		if scaled {
			src.filters = append(src.filters, scaleFilter(size))
		}

		// The colour description rides on the frames, so it is tagged while they are software ones,
		...
		// A VAAPI, QSV or Vulkan encoder reads GPU surfaces, so its device opens ahead of the input
		// and the grabber's chain ends in a conversion and an upload (hwsurface.go).
		var uploads bool
	...
	// The audio inputs follow the capture's, so each index counts the inputs before it:
	// a filter source opens no -i at all,
	// and its audio would otherwise map off an input that is not there.
	inputs := inputCount(src.args)
	audioFilters, audioOut := audioMixFilters(s, inputs)

	// The chain and the maps are one decision wherever a tee or a mixed track exists:
	// neither a filter source nor a mix has an input to map, so each chain labels its output
	// and the map names that label.
	...
	// The upload filter has pinned a surface encode's layout and the encoder's own pixel format
	// is the opaque hardware one, so -pix_fmt would ask ffmpeg to convert GPU surfaces it cannot read.
	if !surface {
	...
	// It is dropped on a device path whose family converts nothing as well:
	// no swscale stage is left for it to steer, the encoder converts the captured RGB at a range
	// of its own and signals that one, and a displayed command carries no option the run ignores
	// (gpu.go, gpupath.ColourEncoder).
	...
	// The relay leg is rendered from the arguments it would otherwise be the whole output of,
	// rather than declared a second time per transport.
	// A transport states one output, option pairs and then the destination,
	// and a tee slave is that statement in the tee muxer's spelling.
...
// filterOutLabel names the output of a filter source's chain,
// so a map has something to name where there is no input.
...
// Counts -i rather than reading a field: a field is a second copy of the fact,
...
// "-f mpegts srt://..." becomes "[f=mpegts]srt://...",
...
// so neither may appear in an option.
...
// Lanczos rather than the bicubic default: what is scaled is a desktop of text and hairlines,
// and a softer kernel costs exactly the edges a reader is reading.
...
// The aspect ratio is not preserved: the sizes come from a list derived from the capture's
// own dimensions, so one off the source's ratio is one that list asked for.
func scaleFilter(size settings.Size) string {
	return fmt.Sprintf("scale=%d:%d:flags=%s", size.Width, size.Height, scaleAlgorithm)
}

// colourDescription is the colour space this engine encodes against,
// spelled as setparams and ffprobe name it.
// BT.709 covers every HD and larger picture, which is every screen this app captures,
// and the GStreamer engine pins the same one as gstBt709, so a stream's colour does not follow
// from the engine that published it.
const colourDescription = "bt709"

// colourFilter tags captured frames with the colour space they are encoded against,
// empty for a chroma with none to state.
//
// The tag is what puts the description in the bitstream, and the bitstream is the only place
...
// and it picks limited-range BT.709: an unsignalled full-range stream is then expanded as limited,
// crushing the blacks and clipping the whites.
//
...
// and a partial description is no description,
// a GStreamer viewer reporting no colorimetry at all for one.
//
// The range is absent from the tag.
// Left unspecified on the frames, the conversion to the encoder's
// pixel format takes its target range from -color_range.
...
//
// Planar RGB is skipped for the reason it carries no -color_range: no matrix,
// and full range by construction.
//
// The chroma is the parameter rather than the settings because the GPU path needs the same tag
// from a caller holding no settings struct, and on a pair whose device path leaves the conversion
// to the encoder this tag is the whole of what the command still states about the colour (gpu.go).
...
// The encoder builder reads the same figure, since one encoder aligns its parameter-set repeat
// to the GOP and both halves of the command have to name one interval.
...
// The chain is held apart from the arguments because the encoder may extend it,
// a VAAPI encode appending a conversion and an upload, and a chain travels in one option.
type captureSource struct {
	...
	filters []string
	// filterFlag carries the chain: -vf, or -filter_complex for ddagrab,
	// a source filter with no input to attach a per-stream chain to.
	filterFlag string
}

// captureBackends is the input side of the command, one entry per screen grabber.
// Which operating system a grabber runs on is publish.captureNeeds' column,
// so nothing here reads a platform.
// fps arrives rendered, since every grabber takes it as a string.
//
// A backend errors where the settings name something it cannot capture:
// a monitor this machine does not have, a DRM download strategy no row carries.
// The alternative is a command that captures something else,
// which is the one outcome no form can show.
...
// captureArgs returns the input arguments and filter chain for the configured backend,
// in the memory this run's frames reach the encoder in.
// A backend with no GPU path takes the resolved value all the same and ignores it,
// which keeps the pair table the one place the two shapes are chosen between.
...
// frameMemory resolves where this run's frames reach the encoder,
// against the pair table both publish engines read (gpupath.Paths).
//
// No device check follows.
// Every GPU path here maps the captured frames onto a device derived from the frames themselves,
// so the encoder runs on the GPU the capture came off whatever else the machine carries.
...
// On the system-memory path hwdownload hands the frames back for any encoder to read,
...
// or "0" every ffmpeg capture device spells it with.
// The cursor modes collapse onto those two here: an ffmpeg capture either draws the pointer
// into the frames or leaves it out,
// and nothing on this path reports a position to send beside the picture.
// A mode a backend does not serve is refused before a command is built
// (internal/publish/cursor.go), so only a mode the device can run reaches here.
...
// A monitor index no enumerated output carries is refused: it names a screen this machine
// does not have, and grabbing the whole desktop instead would publish something
// other than what the form shows selected.
// An entry with no geometry is display.List's placeholder for enumeration being unavailable,
// and the whole X screen is what it means.
//
// An unset DISPLAY is refused rather than guessed at: x11grab reads an X screen,
// and an environment naming none is no session to capture.
...
// avfoundationScreenDevice is what AVFoundation lists a screen under,
// the monitor index appended: "Capture screen 0".
// ffmpeg takes the video and the audio device in one -i separated by a colon,
// so the audio half rides in this string rather than in an input of its own.
...
// The monitor index goes into the device name unchecked, the way ddagrab's output_idx does:
// it indexes AVFoundation's own screen list, and display.List has no macOS enumerator to hold
// it against, only a placeholder that would refuse every index but zero.
//
// The device naming follows ffmpeg's documented
// avfoundation input rather than a reading off a machine.
...
//
// Each opens by the handle its kind or its own device names: desktop audio as the monitor
// of the default sink, an entry naming a device as that device (platform.AudioSourceDevice).
// The names are stated once for both engines, and "-f pulse" against GStreamer's "pulsesrc device="
// is the whole of the difference.
//
// Whether the backend's platform serves a kind is settled above this builder,
// in publish.AudioAvailable: arguments are built from the settings alone,
// and which operating system a capture backend runs on is publish's column.
// A source no platform serves reaching here is a caller that skipped that check,
// the way an unknown codec is (audioEncodeArgs).
...
// audioDevice is the handle one entry opens by: its own device where it names one,
// the kind's default where it does not.
// A kind with neither is refused rather than opened as something else:
// an entry naming an application the enumeration never answered
// for would record the desktop, which is the wrong room.
...
// audioMixFilters is the graph mixing every recorded source into one track,
// and the label its output carries.
// Empty for a stream with no second track.
//
// One track rather than several is carriage: RTMP carries one audio track
// and the relay re-serves every ingest on all of its listeners,
// so a second track would be unplayable on the narrowest
// leg while the form said it published (domain-model.md).
//
// amix normalizes by default, dividing every input by their number,
// so adding a second source would halve the first.
...
//
// first is the input index the audio inputs start at, a count of what the capture contributed
// rather than a constant, since a filter source opens no -i.
...
// Element, sample rate and bitrate all come off capabilities.AudioCodecs,
// so both engines code one track from one declaration rather than from an element list each.
//
// The stream reaching here is validated, so an unknown codec or one this engine has no encoder
// for is a caller that skipped the validator rather than a user's value.
