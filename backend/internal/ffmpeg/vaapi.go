...
// so this side spells a path where the QSV and Vulkan ones spell two options.

// VaapiDevice returns the global option opening the VAAPI
...
// the condition every VAAPI codec's probe fails on, and the UI has greyed them
...
// As in drmCaptureDevice, the lowest-numbered node is no safe default:
// the boot framebuffer can hold an early one, so a node with no kernel driver bound,
// or the simple framebuffer's, is skipped.
