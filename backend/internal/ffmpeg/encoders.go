...
// l holds the two ladder steps resolved against the codec's own row,
// so a mapping names the option a step travels in and never the step itself.
type encoderArgsFunc func(s settings.Settings, r rates, l capabilities.Steps) []string

// tuneArgs is -tune for a resolved step, and nothing for a mode whose row declares no tune
// or whose step is the untuned one.
...
// encoderMappings is the codec-specific half of the command,
// one entry per encoder whose knobs differ.
// The rate-control modes are the methods: cbr, vbr and abr target a bitrate
// and differ in the ceiling, crf targets a quality, lossless is bit-exact.
// gstCodecs expresses the same five on the GStreamer engine.
// A mode an encoder has no form of is a Gap in capabilities.Codecs
// and is rejected before a command is built, so no branch here approximates one.
//
// A family whose codecs share one mapping outright belongs in familyMappings,
// so a codec added to such a family needs no row here.
var encoderMappings = map[string]encoderArgsFunc{
	...
	// the H.26x encoders own a -qp, where the VP8, VP9
	...
	// One QSV mapping serves all four codecs: the rate control is oneVPL's,
	// so the codec decides only which of its modes the silicon generation implements.
	"h264_qsv": qsvArgs,
	"hevc_qsv": qsvArgs,
	"av1_qsv":  qsvArgs,
	"vp9_qsv":  qsvArgs,
	// One AMF mapping serves all three codecs, with what differs bound here:
	// the profile a chroma selects, and the options only some of the three own.
	"h264_amf": amfArgs(nil, amfH264Options),
	"hevc_amf": amfArgs(amfHevcProfiles, nil),
	"av1_amf":  amfArgs(nil, amfNoBPictures),
	// One Vulkan mapping serves all three codecs: the options are the encode extension's,
	// so the codec decides only which of them the driver honours.
...
// The NVENC codecs differ only in the codec name the mapping reads off the settings,
...
// encoderArgs returns the encoder half of the command
// for the configured codec and rate-control mode.
...
// The families declared Implemented:false (v4l2, rkmpp) each need a device
// and a load path of their own, as VAAPI, QSV and Vulkan do (hwsurface.go),
// and none encodes from a command carrying only a bitrate.
...
// Only the encoder name and the lossless knob differ,
// and both are bound per codec in encoderMappings.
func softwareArgs(codec string, lossless []string) encoderArgsFunc {
	return func(s settings.Settings, r rates, l capabilities.Steps) []string {
		...
		case "crf":
			// A quality target held inside a ceiling where the settings state one:
			// the rate factor drives the rate until the VBV would overflow,
			// and the picture softens from there rather than the stream outgrowing the link.
			// Unbounded without one, as -crf alone is.
...
// qualityCeilingArgs is the ceiling a constant-quality encode is held to,
// and nothing where the encode is unbounded: no ceiling stated,
// or a codec whose constant-quality mode carries none on this engine (capabilities.QualityCeiling).
// One helper for every mapping that takes one, so the flag pair
// and the field it reads are one decision rather than one per codec.
...
// All three read the same generic ffmpeg knobs, so the base command
// and the lossless branch stay with the caller.
func aomRates(base []string, s settings.Settings, r rates) []string {
	switch s.Publish.Mode {
	case "crf":
		// These libraries bound a quality target with -b:v rather than with a VBV pair:
		// zero leaves the rate factor alone to drive the rate, and a figure there is libvpx's constrained
		// quality, the rate it codes toward and stays under.
...
// the selected profile cannot carry, so one codec row offers all four chromas.
...
// realtime, row-mt, cpu-used and tune-content=screen
// keep a live screen-content encode within a few cores.
// libvpx is the one software encoder here whose lossless mode is a dedicated flag rather
// than a quantizer of zero.
...
// VP8 has one profile and no lossless mode, and libvpx's VP9-only threading
// and content tuning do not apply to it.
// screen-content-mode is VP8's own equivalent,
// turning on the coding tools for text and sharp edges.
// Its ladder runs further than VP9's and its row starts higher on it,
// VP8 having less to trade away.
...
// The realtime usage profile switches libaom off its two-pass defaults;
// without it a live encode falls minutes behind.
// row-mt and the tile split spread the frame over cores, and lag-in-frames 0 drops the lookahead
// that would hold frames back.
//
// tune-content=screen turns on the coding tools for text and sharp edges,
// as -screen-content-mode does on VP8 and -tune-content on VP9.
...
// svtav1Args maps the rate-control modes onto SVT-AV1,
// whose rate control does not take the aom shape:
...
//     vbr has no form on either engine and is declared as a gap:
//     SVT-AV1 accepts a rate ceiling in constant-quality mode alone
//     and rejects the whole encode when given one in VBR.
//   - cbr is rate-control mode 2, reachable only through -svtav1-params,
//     and it needs the low-delay prediction structure.
//     buf-sz sizes its rate buffer in ms, which the element clamps to 20..10000.
//
// Everything the library takes rather than the wrapper travels in that one parameter string,
// so the rate-control half and the picture half are assembled together (svtav1Params).
...
// svtav1Params assembles one -svtav1-params string out of the caller's rate-control keys
// and the picture keys every SVT-AV1 encode carries,
// and nothing at all where there is none to state.
//
...
//
// The library refuses a key or a value it does not know rather than dropping it,
// so a step off the row's ladder ends the encode at launch
// instead of coding at a setting nobody asked for.
...
// Its rate control is one bitrate target with no ceiling and no rate buffer,
// so vbr is a gap on both engines, and cbr
// and abr differ only in whether frame reordering is dropped for delay.
...
// The rav1enc element takes the same two tunes on a property instead (publish/gstencoders.go),
// the step being rav1e's own either way.
...
// videoToolboxArgs maps the one rate-control mode Apple's framework reaches onto its encoders,
// the counterpart to vtEncoder on the GStreamer path.
// It serves both codecs, the codec name being the only difference between them.
//
// abr is all there is, as the rows' gaps state: -b:v is the average the framework codes towards,
// and the three modes that would bound or replace it have no form here.
//
// realtime is the framework's own answer to an effort ladder, set rather than offered:
// it stops the encoder buffering frames to spend more time on them,
// a delay a live stream cannot pay whatever else it asks for.
...
// vaapiArgs maps the rate-control modes onto the VAAPI encoders' shared -rc_mode knob,
// the counterpart to vaEncoder on the GStreamer path.
// quantizer is the option the constant-quality target travels in,
// bound per codec in encoderMappings.
...
//   - abr: VBR with no ceiling given,
//     which ffmpeg turns into a ceiling of twice the target.
//     VAAPI rate control always codes against a maximum,
//     so that is the closest it comes to an unbounded average.
//
// CQP, CBR and VBR are the three modes both vendors' drivers implement.
// ffmpeg also offers ICQ, QVBR and AVBR, which are Intel-only or absent,
// and a driver handed a mode it lacks refuses to open the encoder rather than falling back.
// lossless has no VAAPI form at all (vaapiGaps).
//
// No effort step and no B-frame count: the family's rows declare no ladder,
// and VAAPI B-frame support varies per driver and hardware generation,
// so the form greys both fields.
...
// The two ends of oneVPL's target-usage scale the ladder's defaults sit at,
// spelled as ffmpeg names them: the balanced point mid-scale,
// and the speed end a live rate is held at.
const (
...
//
// The ladder is the scale's own numbers, which oneVPL defines
// and the GStreamer elements take on their target-usage property.
// ffmpeg names the same points, and this map is where one becomes the other.
// A step with no name here is one this engine cannot ask for,
// and it passes no preset at all rather than a nearby one: the encoder's own default is an honest
// answer where a substituted step is a silent one.
...
// qsvArgs maps the rate-control modes onto the QSV encoders' options,
// the Intel counterpart to vaapiArgs and amfArgs.
// It serves every QSV codec, the codec name being the only difference between them.
//
// This family has no -rc_mode to set: oneVPL derives the method from which rate options carry
// a value, so each branch below is the shape that names one method.
//   - crf: -q:v carries the quantizer and sets ffmpeg's qscale flag with it, which selects CQP.
//     The value is the quantizer itself rather than a quality level,
//     and ffmpeg states it on the H.26x 0..51 scale for every codec but AV1,
//     which capabilities.Codecs carries as each row's CqMax.
//   - cbr: a ceiling equal to the target selects CBR, the VBV window sizing its rate buffer.
//     async_depth drops the encoder's pipeline from four frames in flight to one,
//     which the qsv elements' low-latency property does on the other engine,
//     so live delay does not follow from the capture backend.
//   - vbr: a ceiling above the target selects VBR, bursting to the maxrate.
//   - abr: the same VBR with the ceiling at twice the target.
//     A VBR encode given no ceiling leaves oneVPL its own,
//     which is not a rate the settings ever stated.
//
// ICQ and QVBR stay out, though a quantizer with a bitrate beside it would select them:
// both take a quality level on a scale of their own rather than a quantizer,
// and ICQ is absent from the AV1 and VP9 encoders.
...
//
// B-pictures are pinned off rather than taken from the settings, as on AMF:
// the B-frame count is NVENC's alone,
// and a live screen stream pays their reorder delay for a gain it cannot spend.
//
// The tune step is oneVPL's scenario hint, spelled on -scenario rather than on -tune.
// It reaches the H.264 and HEVC encoders, the two rows declaring the ladder, so the AV1
// and VP9 ones resolve an empty step and state nothing.
...
// as on VAAPI and AMF: a fixed-function encoder codes against a maximum either way,
// so an unbounded average means the same thing on all three hardware families.
const vulkanAbrPeak = 2

// vulkanArgs maps the rate-control modes onto the Vulkan encoders' -rc_mode knob,
// the cross-vendor counterpart to vaapiArgs and amfArgs.
...
//   - vbr: vbr targeting the bitrate and bursting to the maxrate ceiling.
//   - abr: the same vbr with the ceiling at twice the target,
//     the encoders taking no unbounded average.
//
// usage and content are the encode extension's declarations about the stream:
// a live one whose pictures are screen content, which this app captures either way.
// The tune comes off the row's ladder.
// No effort step and no B-frame count, matching the other hardware families:
// the rows declare no effort ladder, and the settings' B-frame count is NVENC's,
// so the reorder delay a live screen stream cannot spend stays off.
...
// AMF leaves the profile indication at Main whatever the input surface carries,
// so a p010le encode comes out as a 10-bit bitstream announcing an 8-bit profile,
// which a decoder gating on the profile is entitled to refuse.
// AMF's H.264 encoder needs no such map, its row being 8-bit, and neither does its AV1 one,
...
// A usage preset is a bundle of AMD defaults applied ahead of the rate-control properties,
// so it sets the encoder's character while -rc decides the rate.
//
// The same preset in all four modes, cbr included, because AMF's low-latency presets are unusable
// for a stream a viewer joins late: under lowlatency and ultralowlatency its H.264 encoder drops
// the IDR period and codes one IDR for the whole stream, leaving a later subscriber no recovery
// point and a decoder that never starts.
// The keyframe interval the settings ask for is the stronger claim, so the preset gives way,
// and cbr states its low-delay character through the speed end of the quality scale
// and the B-pictures the mappings pin off.
// transcoding is also the one preset every VCN generation implements;
// high_quality is refused by the older ones.
...
// header_spacing repeats SPS and PPS every GOP, so they arrive with each IDR;
// written once at stream start otherwise, they leave a viewer who subscribes later
// with no parameter sets to configure its decoder from.
// AMD's HEVC encoder repeats VPS, SPS and PPS per IDR on its own,
// and AV1 carries no out-of-band parameter sets at all, so this is the one row that needs telling.
func amfH264Options(r rates) []string {
	return append(amfNoBPictures(r), "-header_spacing", r.gop)
}

// amfNoBPictures switches off the B-picture pattern of the two AMF encoders that have one,
// rather than leaving it to the usage preset: the settings' B-frame count is NVENC's alone,
// and a live screen stream pays their reorder delay for a gain it cannot spend.
...
// amfArgs maps the rate-control modes onto the AMF encoders' -rc knob,
// the AMD counterpart to vaapiArgs.
...
//   - abr: the same peak-constrained VBR with the ceiling at twice the target,
//     the figure ffmpeg derives for a VAAPI encode given no ceiling,
//     so an unbounded average means the same thing on both hardware families.
//     AMF codes against a peak either way,
//     and left unset it keeps the usage preset's own, which is not a rate the settings ever stated.
//
...
func amfArgs(profiles map[string]string, options func(rates) []string) encoderArgsFunc {
	return func(s settings.Settings, r rates, l capabilities.Steps) []string {
		...
		// The step reaches the encoder verbatim: all three AMF encoders spell the scale alike,
		// so there is nothing to map here the way oneVPL's numbers need mapping (qsvPresets).
		if l.Effort != "" {
...
// nvencArgs maps the rate-control modes onto the NVENC SDK's knobs:
// the preset ladder, the tune and the rc mode.
...
// cbr runs the low-latency preset the row pins rather than the settings' step,
// so the encoder holds a constant rate.
// The pin also greys the control and names the step in force,
// so a settings file carrying p7 cannot run p7 while the form reads what the row declares.
func nvencArgs(s settings.Settings, r rates, l capabilities.Steps) []string {
	preset := []string{"-preset", l.Effort}
	tune := tuneArgs(l.Tune)
	switch s.Publish.Mode {
	case "lossless":
		// True nvenc lossless: no rate control, the frame costs what exactness costs
		// and can burst well past 1 Gbps.
		// B-frames pinned off rather than taken from the settings, bit-exact coding gaining nothing
		// from them, so the UI greys the field.
