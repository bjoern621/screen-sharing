...
// the two answering to different rules:
// every ControlService method is a read, a named effect or the event stream,
// and this one is a loan of memory with a lifetime.
// One type would leave that division a comment rather than a shape.
//
// Nothing here decides what is drawn.
// A subscription names a picture something else opened,
// and the size it carries is a count of pixels rather than a layout.

// FrameServer is the FrameService implementation.
//
// It holds no state of its own, as Server holds none:
// a subscription's whole state lives in the backend that lent it,
// and this is the pipe between that and one gRPC call.
...
//
// Bidirectional because the loan and the release travel on one call:
// a release arriving on a second one could outlive its subscription
// and free a slot of a pool that is gone.
//
...
func (s *FrameServer) Frames(stream screensharev1.FrameService_FramesServer) error {
	...
	if err != nil {
		// The world is not ready rather than the request malformed:
		// a relay decode is opened by StartReceive and the publish preview by the publish itself,
		// so a shell that lost the race asks again instead of being told it named something impossible.
	...
	// The close frees the pool, so a consumer that died gives its GPU memory back
	// rather than holding it for the life of the stream.
	defer frames.Close()
...
//
// The kinds are dispatched here rather than by the backend, being different questions:
// which decode of several, the one preview a publish can have, and which screen.
...
// describe names what a subscription draws from,
// for a log line and for the refusal a consumer reads.
...
// a consumer built against a later minor may send a message this build does not know,
// and ending its stream over that would turn a compatible addition into a broken tile.
...
//
// A pipeline that ended is not a failed request:
// the subscription did what it was for and the stream it drew from was then over.
