...
// where Start answers an empty token, as xdg-desktop-portal-hyprland does, every SelectSources asks
// the user to point at a screen again.
// A publish that relaunches over a refused relay would ask once per attempt.
//
// The session outlives the child instead, so the picker is answered once per stream however many
// children carry it, and each child takes a remote of its own on the held session.
//
// Nothing here expires on its own: the holder decides when the stream is over and calls Release.
// A session held past the last child keeps the compositor sharing a screen nobody receives, and
// keeps whatever indicator it shows for that lit.
type Hold struct {
	mu sync.Mutex
...
// RestoreToken is no part of that comparison, since it is what opening a session starts from and a
// held one is a consent already granted.
...
// A default spelled out and one left off are one request, which is what the normalized precondition
// carries: comparing an unfilled set would reopen a session over a value Open would have supplied,
