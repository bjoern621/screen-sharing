...
//
// The portal is the compositor-agnostic Wayland capture backend:
// nothing here touches a raw framebuffer, the compositor draws its own picker,
// and a PipeWire node comes back on a dedicated remote fd.
// GNOME, KDE and wlroots compositors implement it.
//
// Every ScreenCast method is asynchronous:
// the call returns a Request object path, and the result arrives on that object's Response signal.
// The handle_token in each options map makes the Request path predictable,
// so the Response match is installed before the method is called and no signal races.
//
...
//
// Every failure belongs to the compositor or the bus rather than to this code,
// so all of them are Umgebungsfehler and leave as errors:
// a portal that is not running, a dismissed picker, a revoked consent.
package portal
...
// hidden, embedded in the frame, or delivered as metadata beside it.
...
// Two option sets asking for one source then compare equal whether or not either spelled them,
...
// Session is one granted ScreenCast consent:
// the node the compositor shares and the bus connection that keeps it alive.
...
// Restore is what the compositor issued for this consent, empty where it persisted none,
// stored as it stands for the next SelectSources (settings.SavePortalToken):
// a revoked consent and an empty token replacing a spent one both send the next Open to the picker.
type Session struct {
	client  *client
...
// Remote opens a PipeWire connection on the session and answers the descriptor it lives on,
// which a child inherits and reads NodeID from.
// The caller owns the file and closes it when the child that inherited it is gone.
//
// A connection of its own per child rather than one shared between them:
// the descriptor carries the PipeWire protocol, an exited child already spoke it there,
// and a second one writing a hello into that conversation is not a client the daemon can serve.
