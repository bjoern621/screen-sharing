...
// which is what lets somebody join, and neither figure is something a member is owed: one is an
// operator's handle on the relay and the other is where another member is sitting.
...
type Connection struct {
	...
	// Member is the display name of the live member whose id this connection's subject matches, and
	// empty where no live member of this group derives it.
...
type Result struct {
	...
	// Enforced is false for a group with no live member, where a run is a no-op rather than a removal
	// of everybody.
...
//
// Idempotent by construction: it acts on what the relay reports now against the leases as they stand
...
// sweep is Reconcile beside the connections it left open, which is where an answer reads who is
// publishing from.
func (r *Registry) sweep(prefix string) (Result, []relay.Session) {
	...
	// against. No key derives that prefix, which is what makes a lease there unreachable rather than
	// merely unstated, and this states the same thing where a path arrives instead of a key.
	...
	for _, session := range sessions {
		...
		// The leases are read again per connection rather than off the snapshot above: reading the relay
		// and closing a connection are both round trips, and a member that states presence across one
		// holds a lease by the moment its kick would land.
