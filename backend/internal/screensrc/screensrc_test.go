...
// An index no machine running this test has an output for, which is what makes the heads below the
// same everywhere.
// What a head does with an enumerated index differs per machine; what it does with one the
// enumeration does not carry is this package's own answer, and is the shape asserted here.
const unenumerated = 4096

// init asserts the same invariant at load.
// A build failing at load fails every test with one message, and this one names the row that is
// wrong.
...
// Stated here as well as in the table because the rows are the whole of the feature's reach: a row
// added or dropped changes what a wizard on that platform offers.
func TestWhichSessionsReadOneScreen(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		// The portal is Wayland's only way to a screen and answers with whatever the picker was told, so
		// there is no picture of one named output to take.
		...
		// avfvideosrc chooses its own display, which is also why the monitor setting never reaches the
		...
		t.Run(name, func(t *testing.T) {
			...
			// The sentence is the shell's, written per platform, so the statement carries which platform it
			// is about.
...
// The invariant the package exists for: one head, so the picture a screen is offered by carries the
// element and the rectangle the stream would.
...
// Both bounds are what keep a preview at the cost of a wizard tile: the default render chain on
// Linux writes no size bound, so a preview that did not reduce its own frames would upload whole
// desktops.
...
// Refused rather than served a fragment that would not build, as an Umgebungsfehler the control
// service turns into a status.
...
// The Windows head names the index whatever the enumeration knows, the index being that
// enumeration's own.
