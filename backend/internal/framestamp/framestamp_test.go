...
// A stamp read back off the unit that carries it is what was written, to the nanosecond and to the
// millisecond.
...
// The publishing side's reading rides with the picture it describes, which is what puts it in front
// of a viewer of somebody else's stream.
// It is carried as a sum and a count, so the viewer divides over its own interval rather than being
// handed a rate somebody else averaged.
...
// A publish measuring none of its own stages stamps the clock and nothing else, which reads back as
// nothing measured rather than as a stage that cost nothing.
...
// A stamp is found where it sits in front of the rest of an access unit, which is where an inserted
// one is.
...
func TestReadOfATruncatedStamp(t *testing.T) {
	...
	// Every length that cuts into the reading itself, which ends one byte short of the unit: what
	// closes the unit is a trailing bit no reader needs.
...
// The unit holds no zero byte past its start code, which is what keeps a decoder's
// emulation-prevention rule from rewriting the bytes a reader matches on.
...
// A codec with no user-data unit carries no stamp, and says so rather than producing bytes its
// bitstream cannot hold.
...
// A stream whose buffers are not whole access units carries no stamp: a unit prepended to a
// fragment lands wherever that fragment sits, which is not necessarily ahead of a picture.
...
// itself, which is where the codecs put a prefix message and where a parser looking for the start
// of a stream keeps it.
...
// A frame nothing can be read out of takes the unit in front, a guessed offset landing inside a
// unit and breaking the frame.
