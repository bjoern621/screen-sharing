...
type Path struct {
	// Name is the stream's own name inside the prefix this machine reaches under,
	// and the whole path where nothing came off.
	// Which prefix comes off the settings, so the poll shortens it (internal/app, groups.go).
