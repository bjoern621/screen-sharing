...
// The two staying apart is what lets a decode outlive every window drawing it: a shell that closed
// stops no stream, and one that opened again finds it running.
//
// Several consumers may subscribe to one decode, and each gets a pool of its own.
// Two tiles on one stream are two copies on the GPU rather than one buffer with two owners, which
// keeps a slow consumer from holding a slot the other one needs.
