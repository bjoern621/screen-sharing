...
//
// The render chains between the two are a table (chains.go), the only place a launch line
// is written and the only place stating where a chain holds its frames and what it claims about
// their colour.
// What a run negotiated is read back off the pads (memory.go), so the receive state reports what
// happened rather than what was asked for.
//
// The receive-side counterpart of the publish pipeline in internal/publish/gstreamer.go,
// differing in where the process boundary falls: a publish pipeline is a gst-launch-1.0 child,
// and this side links GStreamer, a decoded frame having to stay in this process's address space
// to be exported as a GPU handle.
package receive
...
//
// Must not leak.
// The sink holds each buffer until its presentation time, so a leaky queue sits at its bound
// for most of every frame period and each arrival drops a frame that was going to be shown:
// on a 60 fps 1080p stream, a three-buffer leaky queue rendered 22 fps where the same chain without
// the leak rendered 46, the rate the source delivered.
// Non-leaky, the queue backpressures and what is really late is dropped once, by the sink,
// the element that knows what late means.
...
//
// sync is stated rather than inherited, renderQueue's bound being written against a sink that holds
// each buffer until its presentation time.
//
// qos keeps a chain short of the rate it is sent from falling behind, and appsink inherits it off.
...
// A cutoff at the sink is no second line of defence, discarding there draining nothing:
...
// With qos on it only takes frames the proportion loop had already decided were worth drawing,
// a 90 fps stream into a chain holding a third of it drawing 430 frames against 541 over the same
