...
// A frame is decoded into GPU memory, copied on the GPU into a slot of a pool this side owns, and
// named to the consumer; the bytes stay on the device from the decoder to the window.
// That copy is the price of the pool, and it buys what a bare decoder buffer cannot give: a
// lifetime this side controls, so a consumer that died cannot pin the decoder's pool, and a fixed
// set of handles the consumer imports once instead of per frame.
//
...
// slotCount is how many buffers a pool lends one consumer: one the consumer is drawing, one it
// has been handed and not drawn yet, and one this side is writing into.
//
// Two would serialize the producer against the compositor's frame time, every write waiting for
// the draw of the frame before it.
// Four buys nothing but latency, a consumer two frames behind being one that will be three
// behind.
const slotCount = 3

// HandleKind names what a slot's handle is, in the contract's own identifiers.
// The consumer matches it against the handle types its compositor imports, a list that differs
// between two graphics backends on one operating system.
type HandleKind string

const (
	// HandleD3D11GlobalShared is a DXGI global shared handle, valid in every process on
	// the machine without being duplicated into one.
...
// Plane places one plane inside a slot's allocation, for a handle kind that does not describe its
// own layout.
type Plane struct {
	Offset uint64
	Stride uint32
...
//
// Every figure is read off what the pipeline negotiated rather than what was asked for: the size
// is the scaler's output and not the size the tile asked to be drawn at, and the format is what
// the chain's converter produced.
type Pool struct {
	...
	// FDSocket is where a descriptor kind's descriptors are read from, one per slot in
	// index order, and Modifier the tiling the frames carry.
...
//
// It opens from a frame rather than from caps: a chain can negotiate device caps and still hand
...
type sharer interface {
	...
	// It fails where the frames are in a memory this platform cannot export, naming the
	// memory, because the fix is another render chain rather than a retry.
	open(sample *gst.Sample, slots int) (Pool, error)
	// write copies one frame into one slot, taking and releasing whatever the pool's
	// synchronization is.
	// The slot is free by contract: the caller lends one out again only after the
	// consumer handed it back.
	write(slot int, sample *gst.Sample) error
	// close frees the pool.
	// Every handle the consumer holds names nothing afterwards, so it runs when the
	// subscription ends and never while one lives.
	close()
}
...
//
// It yields an address and not a reference: the collector cannot see it, so the Go wrapper is
// dead from the moment the call's arguments are evaluated.
