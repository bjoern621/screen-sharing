...
// The audio half of the codec facts:
// which codec the second track is coded in, and how each publish engine codes it.
//
// A table for the reason the video one is.
// An audio codec is carried by some transports and not others,
// is coded by a different element on each engine,
// and pins the sample rate the capture branch resamples to.
...
// Not a row: nothing to encode and nothing for a transport to carry,
...
type AudioEncoder struct {
	...
	// Parser frames the coded stream for the muxer,
	// a GStreamer muxer pad negotiating framed caps an encoder's output does not carry.
...
type AudioCodec struct {
	...
	// Separate from Name for the reason a video codec's Format is:
	// a protocol carries a bitstream, not the encoder that produced it.
	...
	// Desktop audio is speech and system sounds over a stereo pair,
	// which both codecs carry transparently at this rate.
...
var AudioCodecs = []AudioCodec{
	...
	{
		// What the RTMP leg's FLV container carries,
		// and what the players behind an RTMP or HLS URL expect.
		// WebRTC negotiates no AAC, which is carriage,
		// stated by the transport rather than gapped here.
		Name:     "aac",
		Format:   "aac",
		Rate:     48000,
		BitrateK: 128,
...
// EncoderOn is how this engine codes the codec,
// and false where it has no encoder for it, which the row then states as a Gap.
...
// EngineGap is the gap that takes this audio codec off the named engine,
// and false where that engine codes it.
//
// It matches on the engine alone, as the codec table's EngineGap does.
// An audio codec has no per-option values,
// so a gap naming an option is a row this lookup cannot honour:
// skipping it would leave it reading as the engine-wide refusal it never declared,
// so it asserts instead.
...
// in table order.
// What a refusal points at, so it narrows by both:
// a codec the engine cannot code and one the leg does not carry are equally useless as advice.
...
// The codec is the user's and leaves as an error.
// The engine is the caller's own and asserts.
func ValidateAudio(engine, audioCodec string) error {
	...
	// A row an engine cannot code states why,
	// so a surface greying it names the missing element rather than reporting a short table.
