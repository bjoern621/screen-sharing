...
// Each difference is a Gap naming the engine, the option, the value and the reason, rather than a
// fact narrowed to what both engines do: an option value one engine reaches stays offered there,
...
// They are not what anything reads: rules.go turns each into a rule in the one evaluator
// (internal/rules), and the greying, the offered range and the refusal in Validate are three
// readings of one evaluation.
// The split exists because a fact about a codec and a capture backend together, or a codec and a
// platform, has no row to sit on, and a fact with no row grows a table of its own with a consumer
// written against it.
//
...
//
// These facts are consumed on both sides of the wire: the argument builders branch on them and
// reject an impossible combination, and a shell reads them (App.Capabilities) to grey out what
// cannot be picked.
// One definition is what keeps a codec's constraints from saying one thing to the encoder and
// another to a form.
//
// The words are not here.
// Labels, tooltips and refusal sentences are written where the layout is, keyed by the identifiers
// these rows use (docs/ipc-api.md).
//
// Every row is a compiled-in fact, so a malformed one is an Entwicklungsfehler and the lookups
// assert on it.
...
// This package depends on the rule vocabulary and on nothing else in the domain, which is what lets
// every consumer read it, so the names are declared here and the publish package's own constants
// take them from here rather than the other way round.
const (
	EngineFfmpeg = "ffmpeg"
	EngineGst    = "gstreamer"
)
...
// Some aim at a bitrate and differ in the ceiling they hold it under, one aims at a quality, and
// one codes bit-exact; which aim at a bitrate is targetsBitrate.
const (
	ModeCbr      = "cbr"
	ModeVbr      = "vbr"
	ModeAbr      = "abr"
...
// The quantization ranges a picture may be coded at: every code value carrying image data, or the
// 16-235 studio swing a broadcast chain expects.
//
// Declared here for the reason the modes are.
// The colour range is an option a Gap may take a value away from, and a table naming the axis
// without naming its values leaves every consumer to type the two strings itself, where the one
// that types them differently greys the wrong option.
const (
	ColorRangeFull    = "pc"
...
// Each is the JSON field name settings.Settings carries, so a gap and the form control it greys are
// the same identifier on both sides of the wire.
const (
	OptionChroma     = "chroma"
	OptionMode       = "mode"
...
// yuv444p" are what the message has to say.
...
// EveryEngine states one numeric limit under every publish engine, for the encoders whose knob is
// the library's rather than one wrapper's.
// It declares a shared fact once without collapsing the per-engine axis: the row still answers per
// engine, and a row that later diverges names the engines separately with no consumer changing.
...
//
// Which options exist is the Options list and nothing else, so a codec that cannot encode at a
// colour range is declared the same way as one that cannot encode a pixel format.
type Gap struct {
	// Engine is "ffmpeg", "gstreamer", or empty for both.
	// Empty says the format or the library has no such capability, rather than one builder failing to
	// reach it.
	...
	// Reason names which library or element lacks the capability, as a code rather than a sentence
	// about it (api/proto/screenshare/v1/text.proto).
	// One code per row and no arguments: the three fields above already say which codec, which engine
	// and which value, so a surface rendering the code has everything the sentence names and writes it
	// at its own length.
...
//
// The steps are the encoder's own identifiers rather than a normalized scale, because a normalized
// one would lie: x264's "slow" and SVT-AV1's preset 8 are not the same trade, and a number carried
// across a codec change would land on a different real setting than the one that was held.
// A shell names each step and renders one it has no name for as the identifier, which lets a codec
// gain a ladder with nothing edited there (docs/domain-model.md).
//
// Steps run from the most effort to the least, whatever direction the encoder's own numbering
// takes, so a control is drawn without knowing which way each ladder counts.
type Ladder struct {
	...
	// A mode with no entry leaves the knob unset, which is the encoder's own default rather than a
	// step of this ladder.
	...
	//
	// A fact about the encoder and not about the mode: NVENC pins its preset in CBR because a
...
// A ladder with no steps is an encoder without the knob, and a mode the Defaults leave out is one
// that default is right for.
//
// A held step wins wherever the ladder carries it, which is what makes the control a choice rather
// than a display of the row.
// It loses in a mode that pins the knob, and a pinned mode refuses nothing: the control is greyed
// there, so the value behind the greying is one nothing spends, and refusing a publish over it
// would name a field nobody can reach.
...
// ResolveSteps is what an encode of this codec in this mode spends on both ladders, and a refusal
// for a step the encoder does not take.
//
// Both builders read it, which is the point: the step is a fact about the encoder, and an encode
// whose look depended on which engine built its command would make a stream's picture follow the
// capture backend that produced it.
// Each engine still owns the spelling, since ffmpeg says -preset where the x264 element says
// speed-preset, and the empty step, which every builder answers by passing no such option at all.
//
// A refusal is an Umgebungsfehler and keeps a hand-edited settings file off an encoder's own error
// path: the field is free-form text, the repair walks a step off the ladder back onto it, and
// nothing between the two bounds a value that reached the settings by another route.
...
type Codec struct {
	...
	// Implemented is true once the argument builders (encoderArgs, gstEncoder) map this codec to a
	// working command.
	// A false row is offered greyed, so what is not built yet is visible, and BuildPublishArgs rejects
	// it.
	Implemented bool `json:"implemented"`
	// Chromas is every pixel format this codec may encode, on whichever publish engine reaches the
	// most of them.
	// A format the other engine's encoder will not take stays in the list and is declared as a Gap, so
	// it is offered where it works and greyed with the missing element where it does not.
	Chromas []string `json:"chromas"`
	// CqMax is the top of the constant-quality scale per publish engine, which is what the crf mode's
	// quantizer target counts on.
	// It follows the encoder rather than the format: the H.26x encoders reach 51, libvpx and the
	// software AV1 ones 63, and an encoder whose knob is a raw quantizer index counts to 127 or 255.
	//
	// Keyed by engine because the scale belongs to the property each engine sets and not to the
	// silicon underneath: ffmpeg's QSV encoders state a CQP quantizer on the H.26x scale for every
	// codec but AV1, where the qsv elements pass the format's own index through.
	...
	// live encode drops the lookahead and the frame reordering a quality one keeps, whatever effort it
	// is spending.
...
// named engine, and false where that value reaches its encoder there.
// It is the one gap lookup: chroma, rate-control mode, colour range and any option Options gains
// are asked about the same way.
//
// A chroma the codec encodes on neither engine is absent from Chromas instead, which Validate
// rejects on its own.
...
// engine has an encoder for it.
// It is the gap naming no option, since no value of any reaches an encoder that is not there.
...
// CqMaxOn is the top of this codec's constant-quality scale on the named engine, and zero where the
// row declares none for it.
...
// Validate rejects a codec and option-value combination this table forbids, so a settings object
// nothing repaired cannot reach an encoder.
//
// engine is the caller's own publish engine, and decides which codecs and option values are
// available.
// Both engines call this, so neither path accepts what the other rejects and a capability only one
// of them reaches is refused for the other rather than approximated.
//
...
func Validate(engine, codec string, options map[string]string, cq, bitrateM int) error {
	...
	// A mode outside the set matches no gap and would reach the builders, whose rate-control switches
	// end in the cbr branch, so the encode would run at a rate control the settings never asked for.
	...
	// built from, which is what keeps a greyed control and a refused publish in step: the offered
	// range, the greyed entry and the error here are three readings of one evaluation.
	//
	...
	// the first rather than the one an axis-by-axis validator happened to reach first.
	for _, option := range Options {
	...
	// The quantizer target reaches the encoder in crf mode alone, and each encoder's knob has its own
	...
	// A negative target is refused separately, since no rule states a floor and a scale starting at
	// zero is one no band below it can express.
...
// reachedElsewhere names the engines that do reach one value of one option, as a clause to hang off
// the refusal naming the engine that does not.
...
// A refusal naming only the engine the publish was attempted on states half the fact, and the half
...
// lossy: a fact nobody stated matches no rule that names a value, so an empty axis withholds
// nothing.
// What it must never do is guess, which would refuse a publish over a combination the caller never
// described.
//
// The axes it fills are the ones the codec table's own rules match on rather than every axis the
...
// codec's own scale.
// Derived rather than written down: a constant would either clamp a codec counting past it or offer
// numbers no encoder here reaches, and a row declaring a wider scale would leave the constant the
// one place that did not know.
...
// reports false here, matching what Validate accepts on the engine asked about.
...
// HasFormat is asked by the transport package before it narrows a watch choice by format, so a
// relay path in a format this app never encodes narrows nothing rather than narrowing to nothing.
