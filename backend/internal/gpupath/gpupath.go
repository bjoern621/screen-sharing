// Package gpupath declares which capture backend and encoder family pairs skip system memory,
// and resolves the memory setting against them.
//
// A capture backend producing GPU frames and an encoder reading GPU surfaces link directly:
// the conversion to the encoder's layout runs on the device and no frame crosses the bus.
// Where either end speaks system memory,
// every frame is downloaded, converted on the CPU and uploaded again for a surface encoder.
// A full round trip per frame at capture resolution,
// so the pair decides the shape of the whole capture chain rather than one filter in it.
//
// Which pairs have a device path is a fact about the two frameworks' elements and filters,
// declared once here and read by both publish engines.
...
// Whether the machine captures and encodes on the same GPU is a second precondition,
// checked by the engine that can name its devices,
// and a machine that cannot be held to one is refused rather than demoted (Undetermined).
package gpupath
...
const (
	...
	// Every combination satisfies it, which is what makes it the default:
	// a codec or capture backend with no device path is not a setting the user got wrong.
	MemoryAuto = "auto"
	// MemoryGpu demands the device path and refuses a pair that has none,
	// so a machine meant to publish without the round trip says so,
	// rather than paying a frame cost no control names.
	// It demands the colour with it: a pair whose only device path lets the encoder convert is refused,
	// a setting naming the memory alone being no consent to a colour nobody chose.
	MemoryGpu = "gpu"
	// MemoryGpuEncoderColor demands a device path and pays the encoder's colour for it,
	// the only way to ask for a path where the two cannot both be had.
	// A value of its own rather than a relaxation of MemoryGpu,
	// so the trade is recorded in settings that can be read back,
	// and a pair growing an exact-colour path stops trading without the setting changing meaning:
	// the demand is for a device path, and this one takes a cost where there is one.
	...
	// What a pair with no row does,
	// and the answer for a machine whose second GPU puts the import out of reach.
	MemorySystem = "system"
)
...
type Path struct {
	...
	// A code and not a sentence (api/proto/screenshare/v1/text.proto), one per row:
	// the mechanism is what differs between the rows,
	// and a statement general enough to cover all of them describes none.
	Import screensharev1.TextCode `json:"import"`
	// Colour is what this path does to the colour the settings name,
	// and so whether a memory setting may resolve to it without asking twice.
	...
	// so a greyed control states why at the control rather than in a log line.
	// A ColourExact row leaves it unset, having taken nothing.
...
// A pair absent from it encodes from system memory,
...
//
// The colour verdict rides on the row rather than following from membership,
// a pair being able to have a device path with no converter to put on it.
// Such a row belongs in this table too:
// leaving it out would hide a path that works,
// and adding it unlabelled would let a default take a colour nobody asked for.
var Paths = []Path{
	{
	...
	// Nothing converts between the two ends:
	// hwmap derives no CUDA and no Vulkan device from a Direct3D11 frame,
	// and scale_d3d11 builds no encoder layout out of the captured BGRA,
	// so nvenc reads the texture on its own device and converts it there.
	// It signals what it chose, so the stream is described truthfully,
	// and described as something other than the form shows, the whole of what this row costs.
	{
		Engine:  capabilities.EngineFfmpeg,
		Capture: "ddagrab",
		Family:  capabilities.FamilyNvenc,
...
// Resolve returns the memory the frames reach the encoder in,
// MemoryGpu, MemoryGpuEncoderColor or MemorySystem,
// and refuses a demand the pair cannot meet.
//
// The single arbiter: the pair table and the colour verdict on the row it finds are read here
// and nowhere else, so both publish engines and the form take one answer from the same two facts.
//
// The two demands differ in what they pay.
// MemoryGpu wants the device path and the colour,
// so a row offering only the first is refused,
// with what it would have cost and both values that get the user publishing again.
// MemoryGpuEncoderColor wants the device path and pays the colour for it,
// so it takes an encoder-colour row,
// and still answers MemoryGpu where the row converts on the device:
// there is nothing to trade, and refusing a run that gives more than was asked for has no cause.
//
// Auto answers with whichever of the two costs nothing.
// A pair whose only device path lets the encoder convert falls to system memory under auto:
// auto is what a settings file with no frame memory is filled with,
// and the colour fields it would override were set on purpose.
// Trading them is a choice, and a default does not make choices.
//
// A value outside Memories is refused, never read as auto.
// The setting decides whether the capture chain downloads every frame,
...
// The pair is why either fails, so they fail with one sentence:
// the colour the caller was willing to pay changes nothing for frames with no way onto the device.
...
// the question a capture chain has to answer.
// Both device values link capture to encoder with no download and differ only in who converts,
// so a check written against MemoryGpu alone builds the round trip for the other one in silence.
//
// Takes a resolved value, so MemoryAuto is not among them:
// Resolve answers that question before any chain is built,
// and reading it here means a caller asked where the frames live before deciding.
...
// The import is correct on one device only,
// so a machine offering several is refused with the candidates named, rather than guessed at.
