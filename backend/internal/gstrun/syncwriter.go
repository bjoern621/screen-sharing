...
//
// A run reports on its writer from goroutines that coordinate with nothing: the bus loop on the
// calling goroutine, the control server, the pointer reader and the delay reporter each write when
...
// these lines by luck of their length rather than by anything stated here, and a bytes.Buffer, which
// is what the tests pass, has no such property at all.
