...
// It names neither a stream nor a setting, so it is no field of Publish:
// a preset carries what the user chose about a stream,
// and a preset moved to another machine would carry a token no compositor there issued.
...
type portalStore struct {
	// RestoreToken is what SelectSources is given to skip the picker,
	// empty until a consent is persisted.
...
// PortalToken returns the stored ScreenCast restore token,
// empty where none is stored or the file cannot be used.
//
// A token that cannot be read costs a picker,
// so there is nothing to report and nothing to move aside:
// the next Open takes a consent and overwrites the file with the token it produced.
// The one store where the failure and the empty value have one remedy,
// so it does not follow Load's setAside path.
...
// which is why an empty one is no precondition here.
// Empty means the consent was not persisted, so the token on disk is spent,
// and keeping it would send the next session to SelectSources with a value no compositor honours.
