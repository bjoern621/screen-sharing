...
// populatedSettings is a settings.Settings with every field set, each to a value no other field
// holds.
//
// Distinctness is the point of the fixture and not tidiness.
// The failure this conversion has is not a field left out, which shows up as a zero, but a field
// mapped to its twin: BitrateM written into maxrate_mbps, the publish RTSP protocol read back out
// of the watch one, the player's watch leg landing in the tile's.
// Each of those survives a round trip unnoticed the moment two fields share a value, so no two here
// do, across the three groups and not only inside one, because the conversion writes all three.
...
// eachField visits every leaf field of a settings value, group by group, naming each the way a form
// key does: the group, a dot, the field.
//
// The walk stops two levels deep, which is the shape settings.Settings has: three groups of plain
// values.
...
// A settings draft crosses to a shell and comes back edited on every keystroke, so a field that
// loses its value on the way is a setting the user cannot change and one that silently reverts.
//
// A plain round trip, because the contract carries every settings field: nothing has to be merged
// onto a held draft to survive a shell that could not see it.
...
// The fixture catches a swapped twin only while no two fields share a value, and it is a
// hand-written table a later edit can quietly collide.
func TestTheFixtureGivesNoTwoFieldsTheSameValue(t *testing.T) {
	// Keyed by the value's rendering rather than by the value, because a settings field may be a
	// list and a list is not a map key.
	// The question is whether two fields are indistinguishable to a reader of the round trip, and
	// two that print alike are.
...
// Umgebungsfehler the app survives and the caller's to reject.
// A panic here would let a malformed message kill the process holding the encoder.
//
// The groups go through their own accessors for the same reason, so a message carrying one group
// and not another converts rather than panicking on the absent one.
...
// The empty output resolution is the capture's own size reaching the encoder unscaled, which is a
// value and not an absence: settings.Publish.OutputSize answers false for it rather than a zero
// size, and a conversion that turned it into anything else, a literal "0x0" or the monitor's
// dimensions filled in helpfully, would put a scaling stage in front of every stream that never
// asked for one.
//
// The round trip above covers a resolution that is set.
// This pins the meaning of the empty one, which is the case a well-meaning edit breaks.
...
// preset carrying either would move a setting the user never meant to save.
...
// result should not have to know which it holds.
...
// The store's order is the order the user saved in and the order a shell lists, so it is part of
// what crosses.
