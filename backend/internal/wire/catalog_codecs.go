...
//
// Beside the machine-side conversions rather than inside them, projecting one page
// (docs/domain-model.md), so reading them together shows whether the projection lost anything.
// Every row crosses whole:
// a row narrowed on the way over would be a second, quieter definition of the same codec.
...
// that has it, with no reason shown anywhere.
// Carrying the gap lets a form offer the format on that engine's capture backends and name who
// lacks it on the others.
...
//
// Nothing here is probed and nothing here forbids a choice:
...
// catalogAudioCodecs converts the audio table:
// what the second track can be coded in, and the element each engine codes it with.
//
// The sample rate and the bitrate are the row's rather than the user's.
// They cross so a shell can state what a track costs without a table of its own,
// and so the figure it shows is the one both engines build their branch from.
...
// catalogGaps converts a row's gaps,
// normalizing the option each names to the spelling a form control binds by (gapOptions).
//
// The reason crosses verbatim, naming the limit and which side has it,
// so a shell shows it where the option would have been and the reader learns what to change rather
// than only that the option is gone.
...
// A form control names it by the proto field name StreamSettings spells and form/keys.go binds
// a widget by ("color_range").
// docs/ipc-api.md promises a shell receiving a gap can grey the matching control with no mapping
// of its own, so the normalization happens here once,
// and the gaps and the capability_options list both leave through it rather than agreeing
// by coincidence.
...
// "" passes through: a gap naming no option takes the codec off an engine altogether,
// and has no control to point at.
...
// bounds the codec by.
//
// One typed row rather than a map keyed by engine name:
// the contract carries the engine as an enum everywhere else, and a map takes no enum key.
// Merging the columns makes the row a fact about an engine rather than a column heading,
// so a reader asks what GStreamer bounds this codec by and gets one answer.
//
// An engine that declares neither bound contributes no row,
// and a bound it does not declare stays absent on the row rather than crossing as nought.
// Absence is how the table says there is no quantizer scale and no bitrate ceiling.
// Zero is a number the encoder would have accepted.
