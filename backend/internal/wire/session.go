...
// settings; a viewer's identity is the pair the watch registry keys on.
...
// PublishSnapshot is what is publishing, at one instant, and the domain side of
// screenshare.v1.PublishState.
//
// It nests the way that message does, which is what makes the conversion below total.
// "A running stream has settings", "an attempt belongs to a retry" and "a retry belongs to a stream
// the user has not stopped" are each a nil inner struct here, so a snapshot that breaks one cannot
// be built rather than being caught on the way past.
type PublishSnapshot struct {
	// Live is the stream in force, nil while nothing publishes.
	// A pipeline waiting out a retry backoff is live: still the stream the user asked for, and
	// stopped by the same call.
	Live *LiveSnapshot
}

// LiveSnapshot is one stream the user asked for and has not stopped.
type LiveSnapshot struct {
	// Settings are what the running pipeline was built from, so they are what the viewers are
	// watching rather than what a form is showing.
...
// Everything after it is read off the running pipeline, as ReceiveStream's fields are: a chain
// falls back on a machine that cannot run its elements, and a hardware decoder may download its own
// frames.
...
// is.
// A method over the nil check rather than a field, so the answer cannot fall out of step with the
// settings beside it the way a stored flag could.
func (p PublishSnapshot) Publishing() bool { return p.Live != nil }

// Retry is the relaunch pending on the live stream, nil where nothing publishes and where a live
// pipeline is carrying frames.
// It saves a caller the two nil checks between it and an attempt count.
...
//
// Nothing is asserted: every invariant is a nil pointer in the shape read and an absent message in
// the shape written, so no state is left to reject.
// An idle snapshot has no settings to contradict, a live one cannot omit them, and an attempt count
// exists only inside a retry.
func PublishState(p PublishSnapshot) *screensharev1.PublishState {
	if p.Live == nil {
		return &screensharev1.PublishState{}
	}

	// The publish and relay groups, and not the viewer one: the running pipeline was built from
...
//
// A figure the run has no measurement for is left unset rather than sent as a zero, and proto3
// presence is what says so.
//
// Dup and Drop cross as duplicated_frames and dropped_frames, the contract spelling out what the
// domain abbreviates: frames the encoder repeated to hold the output rate, and input frames
// discarded for arriving faster than it took them.
// The frame counts carry no presence, because a run that has encoded no frames has encoded zero of
// them.
...
// The destination is a pointer so that a figure added to the sample costs one line here rather than
// a branch.
...
//
// An unreachable relay converts to a snapshot and not to a failure: it is an environment condition
// the screen has to say something about, and a failed call would leave the shell nothing to say it
// with.
...
//
// Every figure crosses through its presence rather than through its value, so a leg the relay times
// no round trip on arrives with rtt_ms absent instead of zero.
// SRT is the only leg it times and states a loss rate on.
// optional pointers on both sides make the copy the pointer itself, which is the one shape that
// cannot lose the distinction on the way.
//
// The strings are the relay's words rather than this code's, so none of them is asserted: a reader
// that arrived with no type is an Umgebungsfehler, and an empty one crosses as empty and reads as
// unknown, which is what an empty format on the path beside it already means
...
// Empty is absence and not a measured empty string: it is used on a host:port and on an RFC 3339
// timestamp, neither of which has a meaningful empty value.
...
// its own frames, so this is a fact about the run and not a copy of the viewer settings it was
// opened on.
type ReceiveStream struct {
	...
	// input pad carried, which read together are the evidence of a download or an upload in between.
	DecodeMemory string
	RenderMemory string
	// Decoder is the element the pipeline picked and Hardware whether it ran on silicon.
	// Hardware says where the decoding ran and nothing about where the frames went afterwards, which
	// is DecodeMemory's answer.
	Decoder  string
	Hardware bool
	// HasAudio is whether the decoder exposed an audio pad and the branch was built.
	// Until it did there is nothing to set a volume on, and a tile draws no meter rather than one
	// that measures nothing.
	HasAudio bool
	// Volume and Muted are the loudness in force, held by the receiver whether or not the branch
	// exists.
	// Reported rather than remembered by whoever set them, which is what lets two shells agree about
	// one decode's loudness.
	Volume float64
	Muted  bool
	// Transfer is the transfer characteristic the decoded frames carry, in GStreamer's spelling, and
	// HDR the verdict on it.
	// The verdict decides whether a viewer is offered anything at all; the characteristic is what a
	// reader is shown.
	Transfer string
	HDR      bool
	// ToneMap is whether the pipeline was built with the rung that rolls an HDR stream down, so it
	// reports what ran and not what was asked for.
	ToneMap bool
	// CanToneMap is whether this machine has an element that rolls the range down, and
	// ToneMapMissing the first one it needs and does not register.
...
// A decode with no audio branch has no entry at all, which is a different fact from a silent one
...
// A nil or empty slice crosses as an empty list, which is what "nothing is being decoded" looks
// like on the wire.
...
//
// A sample, where ReceiveStream is a state, which is why one decode has two shapes: what a decode
// is settles when the pipeline negotiates and is announced when it moves, and what it is doing has
// to be read off the running pipeline on a clock.
//
// A figure with no measurement yet is a pointer, because "not measured yet" is a state a sample has
// to be able to report: a rate is a delta between two samples and the first sample of a run has
// none behind it, and a latency bound or a position exists only once the pipeline can answer for
// it.
...
// A nil or empty slice crosses as an empty list, which is what a tick with nothing decoding looks
// like.
...
// receiveStatGroups carries the leg's own counters across, in the order the pipeline holds the
// elements.
...
// nothing encoded these frames, so there is no decoder to name, and nothing carried them, so there
// is no leg.
...
// A nil or empty slice crosses as an empty list, which is what "no screen is being previewed" looks
// like on the wire.
