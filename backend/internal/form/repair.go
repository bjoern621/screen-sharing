...
//
// The greying and the walk read one evaluation, optionState, so an option the form greys
// is one the repair never lands on and no control offers a value the publish refuses.
//
...
// A number stands against its slider range, a sane end rather than a refusal,
// and is clamped to the ceilings the capability table states, which are refusals:
// a bitrate over the codec's ceiling has no entry to grey, so the control looks live
// and only the launch says no.
func Repair(d Deps, draft settings.Settings) (settings.Settings, []string) {
	...
	// Run to a fixed point, because dependencies run both ways down the table:
	// the capture backend strands the codec below
	// it and the publish leg strands the audio codec above it.
	// A single pass would leave the upward kind for the next call,
	// so a form asked about the draft it had just returned would repair something new every time.
	//
	...
	//
	// Counted are the form's fields plus the settings the ladder repair moves with no control
	// of their own, the tune step being one,
	// or the bound sits a round under what the walk may legitimately do.
	...
	for changed := true; changed; rounds++ {
		...
		// Table order is screen order, so a round runs left to right and top to bottom:
		// a field is repaired against what the fields above it were left on,
		// and the chroma is held to the codec the capture backend just moved.
		...
		// The ladder steps and the ceilings last in the round: both are the codec's own facts
		// and the codec may have moved above.
...
// and no transport and a leg that cannot carry Opus makes a stored "opus" wrong about nothing.
// Repairing it would rewrite a choice for a stream it changes nothing about,
// and announce a repaired field no control on screen explains.
...
// Every entry here is a control the form owes: a setting the user cannot see
// and the repair rewrites is a change nobody can read.
...
// A step is the encoder's own identifier, x264 counting in names, SVT-AV1 in numbers to 13
// and NVENC from p1 to p7, so a draft that changed codec holds a step the new encoder never heard
// of and there is no position to carry across: "slow" is not preset 8.
// A step off the ladder is therefore reset to the one the codec's row declares for the mode rather
// than mapped, and the field is named in the repaired list,
// since the value is not one the user picked.
//
// A codec that declares no ladder leaves the field standing.
// There is no ladder to be off, the control is greyed with that reason,
// and clearing it would throw away the step the draft carries for whichever codec it came from.
func repairLadders(m *screensharev1.Settings) []string {
	...
	// A declared step can be the empty one, a tune ladder leaving most modes untuned
	// and a gapped mode declaring nothing at all, and the empty value is on no ladder.
...
// repairCeilings holds the numeric settings down to the limits the capability table states
// for the codec and engine the draft names, and returns the keys it moved.
//
...
// capabilities.Validate rejects, and neither has an entry to grey.
...
func repairCeilings(d Deps, m *screensharev1.Settings) []string {
	...
	// A draft naming a codec no table carries narrows nothing and stands until the codec field's
	// own repair moves it on a later round.
	v := verdictsOf(d, s)
	...
	if _, ceiling := v.Bounds(KeyBitrateM, 0, fieldRateCeiling); ceiling < fieldRateCeiling {
		...
		// The burst ceiling comes down with the target, where its control's range does not:
		// fieldMaxrateBounds offers the full scale, because a codec's limit bounds the target the encoder
		// is given rather than the headroom above it.
		// The two disagree, and this is the side that keeps the pair reachable:
		// a ceiling left where the target cannot follow is headroom nothing can ever use.
		// Not a refusal being anticipated, though: no rule bands this key
		// and capabilities.Validate weighs the target alone,
		// so a maxrate above the limit is a draft nothing rejects.
		// The move is named in the repaired keys, so the narrowing is never silent.
		if s.Publish.MaxrateM > ceiling {
			s.Publish.MaxrateM = ceiling
			moved = append(moved, KeyMaxrateM)
		}
	}
	// The target's own floor, which only the modes aiming at one have:
	// a draft carrying zero there arrived from a mode that sends no target, or from a file,
	// and a stream at no rate is what the control stopped offering (fieldBitrateBounds).
	...
	// The rate buffer is the rate times the window, and the pair has to stay inside the field
	// the encoder reads it into.
	// The window is the half that gives, the rate being the figure the user chose (fieldVbvBounds).
	...
	//
	// Constant quality has no target for it to sit above, so the pair cannot disagree there
	// and the bitrate is a figure from another mode: raising a 10 Mbit/s ceiling to a 40 Mbit/s target
	// would hand a quality encode four times the rate it was bounded to.
...
//
// Elsewhere the first legal entry is the replacement, an option list being written in the order
// a reader should reach for its entries.
...
// The 10-bit layout is last: more tonal resolution at 4:2:0 chroma is a different trade
// from the four above it, not a step down the same ladder.
var repairChromaOrder = []string{"gbrp", "yuv444p", "yuv422p", "yuv420p", "p010le"}

// Colour range prefers full, the range a desktop already carries.
// Limited range maps the captured values into a narrower window,
// a conversion the picture did not need,
// so the repair reaches for it only where full range is refused.
...
//
// A repair moves a backend nobody asked to move, so it reaches for the one that asks
// least of the machine it lands on.
...
// so landing there answers a Wayland session with a backend
// that fails for a reason no control names.
//
// An engine's own two backends sit together, since moving engine changes which codecs
// and transports the rest of the form offers and strands more than it fixes.
...
// The render chain a repair reaches for first:
// the one a stream renders through where nothing chose a chain.
//
// A row of its own because the offer order is the picker's, cheapest conversion first,
// and the default sits inside that list rather than at its head (receive.Chains).
// The walk below reaches for the first legal entry, so without this a chain that lost its elements
// would land on whatever the table happens to open with instead of on what the package answers
...
//
// av is the evaluation of s, taken by the caller: the walk asks it of every candidate,
// and a caller weighing several fields of one draft (presetStrands) asks it of every field too.
//
// A held value that is offered and not greyed stands, whatever else the list carries.
// Only an absent or greyed value walks,
// and it walks to the first entry the same evaluation leaves enabled.
...
func legalOption(av availability, d Deps, s settings.Settings, f *field, held string, entry int) (string, bool) {
	...
	if len(options) == 0 {
		...
		// The held value stays: there is nothing to walk to, and clearing the setting would invent
	...
	// Read as a sentinel it costs both directions at once, leaving a greyed value stranded
	// where the unscaled entry is the only legal one, and walking past the recommended entry
	// to whatever comes after it where a smaller monitor greys the rest.
	first, found := "", false
	...
	if !found {
		// Every entry greyed.
		// The value stands with the field's reason, the one case the contract allows a control to show
		// a value its own evaluation refuses.
		return held, false
	}
	return first, true
}

// repairWalk is the order a field's entries are tried in: its stated order narrowed
// to what the list offers, and the list's own order where it states none.
//
// A stated order is a preference among values, not a claim that every one is on offer here,
// so an entry the list left out is one no repair may reach for.
// An offered entry the order does not name is appended rather than dropped,
// since a value the machine offers stays reachable even
// where this file has no opinion about where it sits.
...
//
// An option value is a string on the wire whatever type the settings field holds,
// so this is where the two meet on the way in and repairValue where they meet on the way back.
// A select holding a number is ordinary, the monitor index being one:
// what makes a control a select is the machine having a list, not the value being a name.
...
// repairValue is an option's string back in the type its settings field holds,
// and false where the string does not fit that type.
//
// False is a broken contract rather than a condition to survive:
// every value reaching here came out of an option list this package built for this field.
...
// It is not in the settings, so nothing is stranded on it,
// and repairing it would write an entry the reader never made.
...
// repairAudioSources takes every entry that records nothing off the list,
// and returns the keys it moved.
//
// An entry naming no kind is how a reader turns a source off,
// and it is what the row past the end of the list holds until a kind is picked on it.
// Neither is a source, so leaving them in a stored draft would grow the list by one every time
// the form was asked about the draft it had just returned.
//
// The key named is the entry's own, the key a shell binds and can say moved.
// Entries after a dropped one shift up; a shell adopts the repaired draft whole
// and redraws from it, so no index survives the shift.
