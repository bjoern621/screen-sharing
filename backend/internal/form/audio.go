...
//
// A kind is declared and what is inside it is enumerated, so they are two controls:
// whether a machine serves desktop audio at all is a table's answer,
// and which outputs it plays into or which application is running is nothing a table can hold.
// platform.AudioSources is the declaration and this is the other half.
//
// The enumeration is the machine's answer and arrives in Deps, as the monitors
// and the encoder probe do.
// A resolve is a pure function of the draft and what the machine last answered,
// so it runs on every keystroke and pays for no probe of its own (internal/audiodev enumerates once
// and caches for the process).
...
//
// The kind's own default always is: it is what an entry naming no device takes,
// and no enumeration has to report it.
...
// every device the enumeration reported for it,
// then the selection itself where the enumeration reports no such device.
//
// A stranded selection stays for the reason a monitor index no enumeration reported does:
// an application not running is one that may be running when the stream starts,
// and dropping the entry would take a choice back without saying so.
// It carries the note naming it stranded, separating a device that is here from one that was.
...
// The entry decides nothing about the list: which kinds exist is the platform's answer
// and the same for every entry, so this ignores the entry and takes the declared table whole.
