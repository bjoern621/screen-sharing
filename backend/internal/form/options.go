...
//
// Every value comes from a domain table and never from a list typed here: publish.Captures for the
// capture backends, capabilities.Codecs for the encoders, capabilities.Modes for the rate controls,
// transport.PublishNames and transport.WatchNames for the legs, gpupath.Memories for the frame
// memory, ffmpeg.DrmMaps for the download strategies and Deps.Monitors for the screens.
...
// in one move, so a step dropped from either loses a shortcut where a value dropped from a domain
...
// Enabled and Reason are left alone, since which entries the current combination rules out is
// availability's single answer, spent in resolveField, so the evaluation that greys an option is
// the one that repairs a draft onto another.
// An option a neighbouring combination allows is present and greyed; one no combination should ever
// show is absent (docs/field-availability.md).
//
// No builder fills a label or a description.
// What an entry is called and what the paragraph under it says are the surface's, keyed by the
// field and this value (api/proto/screenshare/v1/text.proto).
...
// optionPlainList is the whole of a builder over one closed domain list: every value the table
// declares, in the table's own order, with no note and no recommendation.
//
// key feeds the assertion alone.
// A closed list with nothing in it is a domain table that came back empty, which would render as a
// control offering no choice rather than as the missing table it is.
...
// which is what tells the user the capture backend is the thing to change.
...
// The list then reads the same on every machine, so a user following a screenshot or a support
// thread meets the entry the other machine offered.
//
...
// Auto is marked as the one value every pair satisfies, which is a fact about the table rather than
// a preference: each of the others can be refused by some combination.
...
// optionCursors offers every pointer mode cursor.Modes declares, whatever the selected capture
// backend serves.
// One this backend cannot do is greyed with the backend's own reason, an entry that vanished having
// taken away the sentence naming what to change (docs/field-availability.md,
...
// optionDrmMaps offers the download strategies ffmpeg.DrmMaps declares, that being the table which
// refuses a name no row carries.
...
// optionMonitorOf is the enumerated monitor at a capture index, and false where enumeration
// reported none.
...
// The primary output is marked, emphasis being a verdict about this list rather than a property a
// reader derives from one row of it.
//
...
// off it, and a dropdown that dropped it would show another monitor above a capture that still runs
// on the missing one.
// That entry carries the note, nothing else on the wire describing an index no enumeration
// reported.
...
//
// Derived from the monitor rather than listed, so another screen produces another ladder with
// nothing here edited, and every scaled entry notes what it was derived from.
// A monitor enumeration reported no geometry for yields the unscaled entry alone, there being no
// source size to scale from and a ladder of absolute sizes being a claim about a screen this
// machine cannot measure.
//
// The unscaled entry is the empty value, which is what the settings hold for a capture that reaches
...
func optionOutputResolutions(d Deps, s settings.Settings) []*screensharev1.FieldOption {
	...
	for _, height := range optionScaleHeights {
		...
		// Even width: every chroma subsampling encoded here needs one, so an odd one is a scaler
...
// optionFpsLadder is the ladder the frame rate is offered on: the rates a film, a game and the
// panel refreshes this app has met run at.
//
// A ladder and not a domain, which is the whole of what separates this field from a select.
// The legal values are the fieldFpsBounds range entire; these are the answers worth reaching in one
// move, and a rate off the ladder is typed rather than refused
...
// once as a warning; disabling the entry would say it twice and make a legal rate look refused.
var optionFpsLadder = []int{30, 60, 90, 120, 144, 165, 240}

// optionFpsPresets offers the ladder, plus the held rate wherever the settings carry one the ladder
// does not.
//
// The held rate is present for the reason a stale monitor index is (optionMonitors): a ladder that
// dropped it would leave the dropdown claiming a rate the stream is not captured at.
...
// There is no list of pixel formats in Go: a chroma is a fact about a codec, so the union of the
// rows is the set and a row gaining a format puts it on the form with no edit here.
//
...
//
// Stated here because the capability rows declare chromas per codec and their union has no order to
// inherit, and because the order is itself the argument: a ladder from exact desktop pixels down to
// the cheapest format makes the trade legible where an arbitrary order makes it a list of names.
...
// No range is ever absent: a codec that cannot code one carries a gap, and a gap is a greying with
// a reason.
...
// optionModes offers every rate-control mode the capability table's validator accepts, in that
// table's order: the modes that aim at a bitrate, then the one that aims at a quality,
...
//
// The codec's and not one family's, because the step is the encoder's own identifier: x264 counts
// in names, SVT-AV1 in numbers to 13, NVENC from p1 to p7.
...
//
// The step this mode starts on is marked, that being the one the row declares rather than a
// preference.
...
// optionTunes offers the selected codec's tune ladder, read the way the effort one is: the steps
// are the encoder's own, and the step this mode starts on is marked.
//
// A ladder that leads with the untuned step says so as a value rather than by being short,
// which is what lets the control hold "tune for nothing" as a choice.
...
//
// The held value stays for the reason a stale monitor index does (optionMonitors): a list that
// dropped it would leave the dropdown claiming a step the draft is not on.
...
//
// Every declared source is offered and not only the served ones, which is the division every
// builder here makes: which entries exist at all is the builder's answer, and which of them this
// machine rules out is availability's single one.
...
// optionAudioCodecs offers every row of the audio capability table, in table order, each noting the
// rate and bitrate its track is coded at.
//
// Those figures are the table's and not the user's, stated once so both engines build the same
// branch, and the note is what stops the form from implying a choice about them.
...
// have, WebRTC on the GStreamer engine being the case that matters, so the entry stays and is
// greyed with the engine named, which tells the user the capture backend is what to change.
...
// optionRenderChains offers the chains a receive pipeline converts decoded frames with, in the
// receive package's own order: the default first, then the device chains, then the unconverted one.
//
// A chain this machine cannot run keeps its entry and is greyed, the entry plus its reason naming
// the element that is missing where a shorter list would say nothing.
// Which of them that is is availability's answer (availabilityOptionRules), for the reason every
// builder here leaves its verdict alone.
//
// The default is marked recommended, a hint about this machine rather than a value: what a draft
// holds is Field.value, and the mark says which chain a stream renders through when nothing chose
// one.
...
// One list for both, which transports carry RTP being a fact of the protocol rather than of a
