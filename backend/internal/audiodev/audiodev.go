...
// A form resolves on every keystroke,
// and a resolve is a pure function of the draft and what the machine last answered,
// so it cannot pay for a subprocess.
// The answer is taken once, cached for the process lifetime and read back through Cached
// (docs/development-principles.md).
//
// PipeWire answers, through pw-dump, and is the right server to ask rather than the convenient one.
...
// pwDumpExe writes every object the daemon holds as JSON,
// the one form carrying a node's properties rather than a line written for a reader.
const pwDumpExe = "pw-dump"

// pwDumpArgs drops the ANSI colouring pw-dump wraps its JSON in for a terminal,
...
// A sync.Once rather than a nil check: two surfaces may resolve at the same moment,
// and the answer is the machine's rather than one caller's.
var (
	once   sync.Once
	cached []platform.AudioDevice
)

// Cached is what this machine offers inside each kind,
// enumerated on the first call and answered from memory afterwards.
//
...
// and a machine whose devices changed since the first call is answered with what was there then.
// A shorter lifetime does not close that:
// the application that just launched is the one worth selecting,
// and a cache gets that case wrong every time.
// Only the daemon's add and remove events answer it.
...
// all yield nothing rather than an error.
// Each is an Umgebungsfehler and none fails the app:
// a kind with nothing enumerated keeps its own default.
func enumerate(ctx context.Context) []platform.AudioDevice {
	...
	// One order per machine, on every run:
