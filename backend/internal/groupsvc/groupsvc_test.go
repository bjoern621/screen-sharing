...
// service signs with a fresh signing key and reads a relay carrying these streams.
...
func TestCreatingAGroupHandsBackAKeyAndThePrefixItDerives(t *testing.T) {
	...
	groupKey, err := group.ParseKey(body["groupKey"].(string))
	if err != nil {
		t.Fatalf("the service handed back a group key it cannot read: %v", err)
	}
	if body["groupId"] != groupKey.ID() {
		t.Errorf("the service says the group is %v, where the group key derives %s", body["groupId"], groupKey.ID())
	}

	// Two creations are two groups: a service handing out one group key is one group everybody is in.
	_, second := call(t, s, "POST", "/groups", "")
	if second["groupKey"] == body["groupKey"] {
		t.Error("two creations handed back one group key")
	}
}
...
func TestAKeyBuysATokenForItsOwnPrefix(t *testing.T) {
	...
	if body["prefix"] != groupKey.Prefix() {
		t.Errorf("the token is for %v, where the group key derives %s", body["prefix"], groupKey.Prefix())
	}

	signed, _ := body["relayAccessToken"].(string)
...
// The relay fetches the signing key once and verifies every connection locally, so nothing here runs
// per stream.
