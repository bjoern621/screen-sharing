...
// placeInOrder slots stream i into the display order where the remembered order
// puts it: after every remembered stream that came before it, before the rest. A
// stream the remembered order does not list goes last, which is where a stream the
// window has never shown belongs.
func (s *Session) placeInOrder(i int) {
	...
	s.order = slices.Insert(s.order, pos, i)
}
