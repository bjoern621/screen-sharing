...
// threads, and the sink the player reads back by name.
func TestChainLaunchIsComplete(t *testing.T) {
	for _, c := range chains {
		line := c.launch("videotestsrc")
...
// TestDefaultChainStatesItsColour is the table's contract with a window that never
...
// spelling memoryOf reads back off a pad, since the overlay compares the two.
func TestChainMemoryFeaturesReadAsMemory(t *testing.T) {
	for _, c := range chains {
		...
		if c.device == player.MemorySystem {
