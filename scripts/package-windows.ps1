...
# task names both as dependencies; this script publishes the shell into the same
# directory and zips the result.
...
# Self-contained, so the machine needs no .NET install. Into the same directory as the
# backend rather than beside it: the loader and every lookup here start in the directory
...
# The archive carries one directory rather than loose files, so an extract lands in a
# folder named after the version instead of scattering several hundred files into
# whatever directory the reader unpacked it in.
$stage = Join-Path $dist $name
if (Test-Path $stage) { Remove-Item -Recurse -Force $stage }
New-Item -ItemType Directory -Force -Path $stage | Out-Null
Copy-Item -Recurse -Force (Join-Path $bin '*') $stage

# The one file a reader of this archive sees before running anything. Everything the app
# needs is in the archive, so this says which program to start and nothing else.
$readme = @"
...
# This archive is the one that redistributes ffmpeg and the GStreamer runtime, which are
# GPL and LGPL, so their notice travels with them rather than living only in the
# repository (THIRD-PARTY-NOTICES.md states what each one is and where its source is).
# ffmpeg's own license text is copied when the build that supplied the executables
# carried one, which the release workflow's download does.
...
# Compress-Archive refuses to overwrite, and a rebuilt package should replace the one
# built before it rather than fail here.
